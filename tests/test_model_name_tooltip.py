...
SRC2 = (Path(__file__).resolve().parent.parent / "web/lib/components/chat/models/picker/ModelRow.svelte").read_text(encoding="utf-8")
SRC3 = (Path(__file__).resolve().parent.parent / "web/lib/components/chat/models/picker/ModelPicker.svelte").read_text(encoding="utf-8")

def test_dropdown_item_has_title_tooltip():
    # The dropdown item name span must carry a title with the full display name.
    assert re.search(r"title=\{model.display\}", SRC2), \
...
def test_header_indicator_has_title_tooltip():
    ...
    assert re.search(r"id=\"model-picker-label\" title=\{sessionModel \?\? currentModelId\}", SRC3), \
