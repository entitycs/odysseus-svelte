...
def test_markdown_raw_html_sanitizer_checks_url_attr_edge_cases():
    src = (_REPO / "web" / "lib" / "legacy" / "markdown.js").read_text(encoding="utf-8")
    ...
    pattern = re.compile(
        r"candidate\s*=>\s*_isDangerousUrl\(candidate\)",
        re.MULTILINE
    )
    assert pattern.search(src), "dangerous url"

    pattern = re.compile(
        r"name === 'srcset'\s*\?\s*_isDangerousSrcset\(attr.value\)\s*:\s*_isDangerousUrl\(attr.value\)",
        re.MULTILINE
    )
    assert pattern.search(src), "dangerous source"
...
def test_email_rich_body_render_path_reuses_raw_html_sanitizer():
    markdown_src = (_REPO / "web" / "lib" / "legacy" / "markdown.js").read_text(encoding="utf-8")
    document_src = (_REPO / "web" / "lib" / "legacy" / "document.js").read_text(encoding="utf-8")
