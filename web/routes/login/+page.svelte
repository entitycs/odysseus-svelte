...
onMount(async () => {
  ...
  try {
    ...
    if (t && t.colors) {
      ...
      for (var k in ADV) {
        if (a[k]) s.setProperty(ADV[k], a[k]);
      }
    }
    ...
    var effColor =
      (t && t.bgEffectColor) || THEME_DEFAULT_EFFECT_COLOR[name] || '';
    var effInt =
      t && t.bgEffectIntensity !== undefined
        ? t.bgEffectIntensity
  ...
  try {
    const tm = await import('$lib/legacy/theme.js');
  ...
  function setMode(m) {
    ...
    if (m === 'setup') {
      ...
      confirmGroup.style.display = 'block';
      submitBtn.innerHTML =
        '<span style="position:relative;top:1px;">Create Account</span>';
  ...
  const policyPromise = fetch('/api/auth/policy', {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (mode === 'setup' || mode === 'signup') {
      const endpoint =
        mode === 'setup' ? '/api/auth/setup' : '/api/auth/signup';
      try {
    ...
    function finishLogin() {
      ...
      Promise.all([
        ...
        fetch('/api/auth/features', { credentials: 'same-origin' }).then((r) =>
          r.json(),
        ),
        fetch('/api/auth/settings', { credentials: 'same-origin' }).then((r) =>
          r.json(),
        ),
      ])
        .then(([sess, feat, sett]) => {
