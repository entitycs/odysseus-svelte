...
export function init() {
  ...
  _notifFired = new Set(
    JSON.parse(localStorage.getItem('cal-notif-fired') || '[]'),
  );
}
...
function _formatReminderBody(note) {
  ...
  if (dtstartRaw) {
    ...
    if (!isNaN(start.getTime())) {
      ...
      const when = start.toLocaleTimeString([], {
        hour: 'numeric',
        minute: '2-digit',
      });
      let when2 = '';
      ...
      if (mins >= 1 && mins <= 60)
        return `Starts in ${mins} min (${when}${when2})`;
      ...
      if (mins >= -60)
        return `Started ${Math.abs(mins)} min ago (${when}${when2})`;
...
async function _pollReminders() {
  try {
    const res = await fetch(`${API_BASE}/api/notes?label=calendar`, {
    ...
    for (const note of notes) {
      ...
      if (uiModule.showToast)
        uiModule.showToast(
          (note.title || 'Calendar Reminder') + (body ? ' — ' + body : ''),
        );
    }
