...
import { toTrimmedString } from '$lib/legacy/util/format';

export function getCurrentEndpointUrl(){
  return sessionModule.getCurrentEndpointUrl() || '';
}
...
export function shortEndpointLabel(label) {
  const value = toTrimmedString(label);
  if (!value) return '';
