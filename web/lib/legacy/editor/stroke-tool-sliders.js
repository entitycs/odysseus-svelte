...
function wireToolSliders(prefix, fields) {
  ...
  document
    .getElementById(`ge-${prefix}-opacity`)
    ?.addEventListener('input', (e) => {
      state[fields.opacity] = parseInt(e.target.value);
      document.getElementById(`ge-${prefix}-opacity-label`).textContent =
        state[fields.opacity] + '%';
      if (opPrev)
        opPrev.style.opacity = (state[fields.opacity] / 100).toFixed(2);
    });

  document
    .getElementById(`ge-${prefix}-flow`)
    ?.addEventListener('input', (e) => {
      state[fields.flow] = parseInt(e.target.value);
      document.getElementById(`ge-${prefix}-flow-label`).textContent =
        state[fields.flow] + '%';
      ...
      if (flPrev) {
        ...
        flPrev.style.opacity = (0.3 + (state[fields.flow] / 100) * 0.6).toFixed(
          2,
        );
      }
    });
  ...
  document
    .getElementById(`ge-${prefix}-softness`)
    ?.addEventListener('input', (e) => {
      state[fields.softness] = parseInt(e.target.value);
      document.getElementById(`ge-${prefix}-softness-label`).textContent =
        state[fields.softness] + '%';
