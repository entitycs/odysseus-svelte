...
export function wireImport({
  ...
  function handleImportedImage(img) {
    ...
    if (tb)
      tb.querySelectorAll('.ge-tool-btn').forEach((b) =>
        b.classList.toggle('active', b.dataset.tool === 'move'),
      );
  ...
  document
    .getElementById('ge-import-topbar')
    ?.addEventListener('click', () => importFileInput.click());
  document
    .getElementById('ge-import-file')
    ?.addEventListener('click', () => importFileInput.click());

  document
    .getElementById('ge-import-paste')
    ?.addEventListener('click', async () => {
      try {
        ...
        if (!blob) {
          if (uiModule) uiModule.showToast('No image found in clipboard');
          return;
        }
        ...
        img.onerror = () => {
          URL.revokeObjectURL(url);
          if (uiModule) uiModule.showToast('Failed to load clipboard image');
        };
        img.src = url;
      } catch (e) {
        if (uiModule)
          uiModule.showToast('Clipboard access denied or no image available');
      }
    });
  ...
  document
    .getElementById('ge-import-gallery')
    ?.addEventListener('click', async () => {
      try {
        const res = await fetch('/api/gallery/library?limit=50', {
          credentials: 'same-origin',
        });
        const data = await res.json();
        const items = data.items || [];
        if (!items.length) {
          if (uiModule) uiModule.showToast('No images in gallery');
          return;
        }
        ...
        overlay.style.cssText =
          'position:fixed;inset:0;z-index:10001;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;';
        const panel = document.createElement('div');
        panel.style.cssText =
          'background:var(--panel,#1e1e1e);border-radius:12px;padding:16px;max-width:500px;max-height:70vh;overflow-y:auto;width:90%;';
        panel.innerHTML =
          '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;"><span style="font-size:13px;font-weight:600;">Pick from Gallery</span><button id="ge-gallery-close" style="background:none;border:none;color:var(--fg);cursor:pointer;font-size:18px;">✕</button></div>';
        ...
        for (const item of items) {
          ...
          thumb.src = item.url;
          thumb.style.cssText =
            'width:100%;aspect-ratio:1;object-fit:cover;border-radius:6px;cursor:pointer;border:2px solid transparent;transition:border-color 0.15s;';
          thumb.addEventListener('mouseenter', () => {
            thumb.style.borderColor = 'var(--accent,#61afef)';
          });
          ...
          thumb.addEventListener('click', () => {
            ...
            img.onload = () => handleImportedImage(img);
            img.onerror = () => {
              if (uiModule) uiModule.showToast('Failed to load gallery image');
            };
            img.src = item.url;
          });
        ...
        panel
          .querySelector('#ge-gallery-close')
          .addEventListener('click', () => overlay.remove());
      } catch (e) {
        if (uiModule)
          uiModule.showToast('Failed to load gallery: ' + e.message);
      }
    });
