...
export function createApplyImageTool({
  ...
  return async function applyImageTool(
    endpoint,
    extraPayload,
    layerName,
    btn,
    ...
    if (!extraPayload._endpoint) {
      ...
      const type = m
        ? m[1].replace('upscale-ai', 'upscale').replace('remove-bg', 'rembg')
        : null;
    ...
    try {
      ...
      img.onerror = () => {
        if (uiModule) uiModule.showToast('Failed to load result', 6000);
      };
      ...
      for (const pkg of KNOWN_DEPS) {
        if (
          msg.includes(`${pkg} not installed`) ||
          msg.includes(`no module named '${pkg}'`)
        ) {
          depMatch = pkg;
          break;
        }
