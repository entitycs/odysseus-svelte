    ...
    if (btn && menu) {
      ...
      menu.addEventListener('click', (e) => {
        ...
        if (action === 'resize') resizeCustomPrompt();
          ...
          document.getElementById('ge-edge-menu-btn')?.click();
    ...
    if (btn && menu) {
      ...
      menu.querySelectorAll('[data-resize-w]').forEach((item) => {
        item.addEventListener('click', () => {
          menu.hidden = true;
          applyResize(
            parseInt(item.dataset.resizeW, 10),
            parseInt(item.dataset.resizeH, 10),
          );
        });
      });
      menu
        .querySelector('[data-resize-custom]')
