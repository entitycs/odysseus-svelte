...
export function wireSelectionControls({
  ...
  syncLassoFeather(0);
  document
    .getElementById('ge-lasso-feather')
    ?.addEventListener('input', (e) => {
      document.getElementById('ge-lasso-feather-label').textContent =
        e.target.value + 'px';
      syncLassoFeather(parseInt(e.target.value, 10));
      composite();
    });
  document.getElementById('ge-lasso-grow')?.addEventListener('input', (e) => {
    document.getElementById('ge-lasso-grow-label').textContent =
      e.target.value + 'px';
    composite();
  });
  ...
  document
    .getElementById('ge-lasso-invert')
  ...
  document.getElementById('ge-wand-feather')?.addEventListener('input', (e) => {
    ...
    if (prev)
      prev.style.setProperty('--feather-blur', Math.min(v / 14, 8) + 'px');
    composite();
  });
  document.getElementById('ge-wand-grow')?.addEventListener('input', (e) => {
    document.getElementById('ge-wand-grow-label').textContent =
      e.target.value + 'px';
    composite();
  });
  ...
  const retuneWand = () => {
    ...
    wandRetuneRaf = requestAnimationFrame(() => {
      wandRetuneRaf = null;
      runMagicWand(state.wandLastSeed.x, state.wandLastSeed.y, 'replace', {
        retune: true,
      });
    });
  };
  ...
  liveBtn?.addEventListener('click', () => {
    ...
    liveBtn.setAttribute(
      'aria-pressed',
      state.wandLiveRetune ? 'true' : 'false',
    );
    if (state.wandLiveRetune) retuneWand();
  });
  document
    .getElementById('ge-wand-tolerance')
    ?.addEventListener('input', (e) => {
  ...
  document.getElementById('ge-wand-vis')?.addEventListener('click', () => {
    ...
    if (btn) {
      ...
      btn.title = state.wandMaskVisible
  ...
  document
    .getElementById('ge-wand-clear')
    ?.addEventListener('click', wandClear);
  document
    .getElementById('ge-wand-invert')
    ?.addEventListener('click', invertSelection);
  document
    .getElementById('ge-wand-delete')
    ?.addEventListener('click', wandDeleteSelection);
  document
    .getElementById('ge-wand-copy')
    ?.addEventListener('click', wandCopyToNewLayer);
  document
    .getElementById('ge-wand-mask')
    ?.addEventListener('click', wandToMask);
  ...
  document
    .getElementById('ge-wand-rembg')
    ?.addEventListener('click', async () => {
      const btn = document.getElementById('ge-wand-rembg');
      const hint = buildSelectionHintMask();
      if (!hint) {
        if (uiModule)
          uiModule.showToast('Click to make a wand selection first');
        return;
      }
      await applyImageTool(
        '/api/image/remove-bg',
        { hint_mask: hint },
        'BG Removed',
        btn,
  ...
  if (wandTolPrev)
    wandTolPrev.style.opacity = (state.wandTolerance / 100).toFixed(2);
}
