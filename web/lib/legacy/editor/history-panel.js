...
import { historyPanelHTML } from '$lib/legacy/editor/build/popups.js';
import { HISTORY_ICON, relTime } from '$lib/legacy/editor/layer-helpers.js';
...
export function createHistoryPanel({ undo, redo }) {
    ...
    if (state.historyPanelEl) {
      if (state.historyPanelEl._escHandler) {
        document.removeEventListener(
          'keydown',
          state.historyPanelEl._escHandler,
          true,
        );
      }
      if (state.historyPanelEl._awayHandler) {
        document.removeEventListener(
          'pointerdown',
          state.historyPanelEl._awayHandler,
          true,
        );
      }
  ...
  function toggleHistoryPanel() {
    ...
    panel
      ...
      .addEventListener('click', minimiseHistoryPanel);
  ...
  function refreshHistoryPanelIfOpen() {
    ...
    for (let i = 0; i < state.undoStack.length; i++) {
      ...
      rows.push({
        offset: -(state.undoStack.length - i),
        label: s._label || 'Edit',
        ts: s._ts,
      });
    }
    ...
    for (let i = state.redoStack.length - 1; i >= 0; i--) {
      ...
      rows.push({
        offset: state.redoStack.length - i,
        label: s._label || 'Edit',
        ts: s._ts,
        future: true,
      });
