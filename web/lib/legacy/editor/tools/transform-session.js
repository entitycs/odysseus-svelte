...
export function createTransformSession({
  ...
  function openTransformPopup() {
    ...
    aspectBtn.setAttribute(
      'aria-pressed',
      state.transformAspectLock ? 'true' : 'false',
    );
    ...
    wInput.addEventListener('input', () => {
      if (state.transformAspectLock) {
        ...
        if (!Number.isNaN(w) && state.transformOrigW > 0) {
          ...
          const newH =
            Math.round(
              (Math.abs(w) / state.transformOrigW) * state.transformOrigH,
            ) * sign;
    ...
    hInput.addEventListener('input', () => {
      if (state.transformAspectLock) {
        ...
        if (!Number.isNaN(h) && state.transformOrigH > 0) {
          ...
          const newW =
            Math.round(
              (Math.abs(h) / state.transformOrigH) * state.transformOrigW,
            ) * sign;
    ...
    pop
      .querySelector('#ge-transform-apply')
      .addEventListener('click', () => confirmTransform());
    pop
      .querySelector('#ge-transform-cancel')
      .addEventListener('click', () => cancelTransform());
    pop
      .querySelector('#ge-transform-cancel-btn')
      ?.addEventListener('click', () => cancelTransform());
    ...
    pop
      .querySelector('#ge-transform-rot-90')
  ...
  function reapplyTransform() {
    ...
    tCtx.scale(
      state.transformPendingFlipH ? -1 : 1,
      state.transformPendingFlipV ? -1 : 1,
    );
