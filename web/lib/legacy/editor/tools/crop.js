...
import { canvasCoords } from '$lib/legacy/editor/canvas-coords.js';
import { drawCheckerboard } from '$lib/legacy/editor/checkerboard.js';
...
export function createCropTool({ composite, showCropApply }) {
  return {
    begin(e) {
      ...
      if (
        state.cropRect &&
        coords.x >= state.cropRect.x &&
        coords.x <= state.cropRect.x + state.cropRect.w &&
        coords.y >= state.cropRect.y &&
        coords.y <= state.cropRect.y + state.cropRect.h
      ) {
    ...
    drag(e) {
      ...
      if (state.cropMoving && state.cropRect && state.cropMoveStart) {
        ...
        nx = Math.max(
          0,
          Math.min(nx, state.mainCanvas.width - state.cropRect.w),
        );
        ny = Math.max(
          0,
          Math.min(ny, state.mainCanvas.height - state.cropRect.h),
        );
      ...
      if (e.shiftKey) {
        ...
        state.cropEnd = {
          x: state.cropStart.x + dx,
          y: state.cropStart.y + dy,
        };
      } else {
        state.cropAspectLock = null;
      }
