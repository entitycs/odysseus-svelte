...
export function syncOverlay(margin) {
  ...
  state.transformOverlay.style.left =
    Math.round(state.mainCanvas.offsetLeft - margin * state.zoom) + 'px';
  state.transformOverlay.style.top =
    Math.round(state.mainCanvas.offsetTop - margin * state.zoom) + 'px';
  state.transformOverlay.style.transform =
    state.mainCanvas.style.transform || 'none';
}
...
function knobPosition(cxh, cyh, rotRad, baseInnerR, rotOffset) {
  ...
  if (
    knobLocalX < 0 ||
    knobLocalY < 0 ||
    knobLocalX > state.mainCanvas.width ||
    knobLocalY > state.mainCanvas.height
  ) {
    rotInside = true;
  }
  ...
  try {
    ...
    if (area && !rotInside) {
      ...
      if (knobClientX < aRect.left + 6 || knobClientX > aRect.right - 6)
        rotInside = true;
    }
  } catch {}
...
export function drawHandles(margin) {
  ...
  ctx.clearRect(
    0,
    0,
    state.transformOverlay.width,
    state.transformOverlay.height,
  );
  ctx.save();
