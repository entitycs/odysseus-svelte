...
export function wireRembgAndSharpen({
  ...
  document
    .getElementById('ge-sharpen-amount')
    ?.addEventListener('input', (e) => {
      document.getElementById('ge-sharpen-label').textContent =
        e.target.value + '%';
      if (sharpenPrev)
        sharpenPrev.style.opacity = (
          parseInt(e.target.value, 10) / 100
        ).toFixed(2);
    });
  document.getElementById('ge-sharpen-run')?.addEventListener('click', () => {
    const amount = parseInt(
      document.getElementById('ge-sharpen-amount')?.value || '50',
    );
    applyImageTool(
      '/api/image/sharpen',
      { amount },
      'Sharpened',
      document.getElementById('ge-sharpen-run'),
    );
  });

  // ── Bg Remove ──
  document
    .getElementById('ge-rembg-install-link')
    ?.addEventListener('click', () => {
      openCookbookForDependency('rembg');
    });
  document
    .getElementById('ge-rembg-run')
    ?.addEventListener('click', async () => {
      ...
      const prevVisible = state.layers
        .filter((l) => l.visible)
        .map((l) => l.id);
      ...
      if (f) {
        f.value = 0;
        document.getElementById('ge-rembg-feather-label').textContent = '0px';
        syncRembgFeather(0);
      }
      if (g) {
        g.value = 0;
        document.getElementById('ge-rembg-grow-label').textContent = '0px';
        syncRembgGrow(0);
      }
    });
  ...
  function scheduleRembgApply() {
    if (rembgRaf) return;
    rembgRaf = requestAnimationFrame(() => {
      rembgRaf = null;
      rembgApplyEdgeNow();
    });
  }
  function rembgApplyEdgeNow() {
    ...
    const feather = parseInt(
      document.getElementById('ge-rembg-feather')?.value || '0',
      10,
    );
    const grow = parseInt(
      document.getElementById('ge-rembg-grow')?.value || '0',
      10,
    );
  ...
  function syncRembgGrow(v) {
    ...
    rembgGrowPrev.style.background =
      v < 0 ? 'color-mix(in srgb, var(--fg) 40%, transparent)' : 'var(--fg)';
  }
  syncRembgFeather(2);
  syncRembgGrow(0);
  document
    .getElementById('ge-rembg-feather')
    ?.addEventListener('input', (e) => {
  ...
  document.getElementById('ge-rembg-grow')?.addEventListener('input', (e) => {
    const v = parseInt(e.target.value, 10);
    document.getElementById('ge-rembg-grow-label').textContent =
      (v >= 0 ? '+' : '') + v + 'px';
    syncRembgGrow(v);
    scheduleRembgApply();
  });
