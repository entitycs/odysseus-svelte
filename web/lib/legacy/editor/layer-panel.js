...
import { applyAdjustment } from '$lib/legacy/editor//fx/pixel-pass.js';
import {
  ADJ_ICONS,
  adjLayerLabel,
  isLayerEmpty,
...
import { mergeLayerDownAtIndex } from '$lib/legacy/editor/wire-merge-buttons.js';

const EYE_OPEN =
  '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
const EYE_OFF =
  '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><line x1="8" y1="16" x2="16" y2="8"/><line x1="8" y1="8" x2="16" y2="16"/></svg>';
const EYE_OPEN_SM =
  '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>';
const EYE_OFF_SM =
  '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><line x1="8" y1="16" x2="16" y2="8"/><line x1="8" y1="8" x2="16" y2="16"/></svg>';
...
export function createLayerPanelRenderer(deps) {
  ...
  function render() {
    ...
    for (let i = state.layers.length - 1; i >= 0; i--) {
      ...
      const parentIsPaintTarget =
        layer.id === state.activeLayerId &&
        !(
          layer.masks &&
          layer.activeMaskId &&
          layer.masks.some((m) => m.id === layer.activeMaskId)
        );
      item.className =
        'ge-layer-item' +
        (parentIsPaintTarget ? ' active' : '') +
        (layer.id === state.activeLayerId && !parentIsPaintTarget
          ? ' active-parent'
          : '');
      ...
      item.addEventListener('click', (e) => {
        ...
        document
          .querySelectorAll('.ge-layers-list .ge-layer-item')
          .forEach((el) => {
            el.classList.toggle(
              'active',
              el.dataset.layerId === state.activeLayerId,
            );
          });
      });
      ...
      handle.innerHTML =
        '<svg width="8" height="14" viewBox="0 0 8 14" fill="currentColor"><circle cx="2" cy="2" r="1"/><circle cx="6" cy="2" r="1"/><circle cx="2" cy="7" r="1"/><circle cx="6" cy="7" r="1"/><circle cx="2" cy="12" r="1"/><circle cx="6" cy="12" r="1"/></svg>';
      ...
      fxBtn.className =
        'ge-layer-btn ge-layer-fx-btn' +
        (layerHasAdjustments(layer) ? ' active' : '');
      fxBtn.innerHTML =
        '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 3a9 9 0 0 1 0 18Z" fill="currentColor"/></svg>';
      fxBtn.title =
        'Adjust layer (Brightness, Contrast, Saturation, Hue, Levels, Color Balance)';
      ...
      fxBtn.addEventListener('pointerup', (e) => {
        ...
        const delay =
          e.pointerType === 'touch' || e.pointerType === 'pen' ? 120 : 0;
        openLayerFx(e, delay);
      });
      ...
      dupBtn.addEventListener('click', (e) => {
        ...
        if (Array.isArray(layer.masks) && layer.masks.length) {
          copy.masks = layer.masks.map((m) => {
            ...
            c.width = m.canvas.width;
            c.height = m.canvas.height;
      ...
      const hasLassoSelInitial =
        state.lassoPoints.length >= 3 && !state.lassoActive;
      const hasWandSelInitial = !!state.wandMask;
      const maskBtn = document.createElement('button');
      maskBtn.className =
        'ge-layer-btn ge-layer-mask-btn' +
        (hasLassoSelInitial || hasWandSelInitial ? ' from-selection' : '');
      maskBtn.innerHTML =
        '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 12c4 0 4-4 8-4s4 4 8 4-4 4-8 4-4-4-8-4z" fill="currentColor"/></svg>';
      maskBtn.title =
        hasLassoSelInitial || hasWandSelInitial
      ...
      if (i > 0) {
        ...
        mergeDownBtn.innerHTML =
          '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><polyline points="6 13 12 19 18 13"/></svg>';
      ...
      if (state.layers.length > 1) {
        ...
        delBtn.textContent = '×';
        delBtn.title = layer.isBase
          ? 'Delete original layer (Ctrl+Z to undo)'
          : 'Delete layer';
        delBtn.addEventListener('click', async (e) => {
          ...
          if (state.activeLayerId === layer.id) {
            state.activeLayerId =
              state.layers[Math.min(i, state.layers.length - 1)].id;
          }
          composite();
          render();
        });
      ...
      if (layer.adjLayers && layer.adjLayers.length) {
        for (const adj of layer.adjLayers) {
          ...
          sub.addEventListener('click', (e) => {
            ...
            if (
              e.target.closest(
                '.ge-layer-vis, .ge-layer-opacity, .ge-layer-btn',
              )
            )
              return;
      ...
      if (layer.masks && layer.masks.length) {
        for (let mi = 0; mi < layer.masks.length; mi++) {
          ...
          const sub = document.createElement('div');
          sub.className =
            'ge-layer-item ge-adj-sub-item ge-mask-sub-item' +
          ...
          const mkEmpty = isMaskCanvasEmpty(mk.canvas)
          ...
          if (mi > 0) {
            ...
            mergeBtn.innerHTML =
              '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="6 11 12 5 18 11"/></svg>';
            mergeBtn.addEventListener('click', (e) => {
              e.stopPropagation();
          ...
          delBtn.addEventListener('click', (e) => {
            ...
            if (layer.activeMaskId === mk.id) {
              layer.activeMaskId =
                layer.masks[layer.masks.length - 1]?.id || null;
            }
