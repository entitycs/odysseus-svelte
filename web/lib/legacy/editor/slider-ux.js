...
export function wireSliderUx({ registerDocClickAway }) {
  ...
  function bubbleRowFor(slider) {
    return slider.closest(
      '.ge-eraser-row, .ge-layer-item, .ge-control-row, .ge-adj-row',
    );
  }
  function bubbleText(slider) {
    ...
    const chip =
      row?.querySelector('.ge-slider-value') ||
  ...
  function showSliderBubble(slider, e) {
    if (sliderBubble.parentElement !== document.body)
      document.body.appendChild(sliderBubble);
    sliderBubble.textContent = bubbleText(slider);
    bubblePos(
      slider,
      e
        ? e.clientX
        : slider.getBoundingClientRect().left + slider.offsetWidth / 2,
    );
  ...
  container.addEventListener(
    ...
    (e) => {
      ...
      if (slider.matches('.ge-eraser-row input[type="range"]')) {
        ...
        const valFrac = Math.max(
          0,
          Math.min(1, 1 - (rect.right - e.clientX) / 140),
        );
  ...
  document.addEventListener('pointerup', () => {
    container
      .querySelectorAll('input[type="range"].is-using')
      .forEach(scheduleSliderRelease);
    hideSliderBubble();
  });
