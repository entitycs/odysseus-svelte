...
export function wireCanvasEvents({
  ...
  state.mainCanvas.addEventListener('mouseenter', (e) => {
    if (['brush', 'eraser', 'inpaint', 'lasso', 'clone'].includes(state.tool))
      updateBrushCursor(e);
  });
  ...
  state.mainCanvas.addEventListener(
    'touchstart',
    (e) => {
      e.preventDefault();
  ...
  canvasArea.addEventListener('pointerdown', (e) => {
    if (state.tool === 'lasso') return;
    if (e.target === state.mainCanvas || e.target === state.transformOverlay)
      return;
    if (
      e.target.closest(
        'button, input, .ge-adj-popup, .ge-transform-popup, .ge-fx-popup, .ge-inpaint-popup, .ge-controls, .ge-right-panel, .ge-fx-menu',
      )
    )
      return;
    ...
    try {
      canvasArea.setPointerCapture(pid);
    } catch {}
  ...
  const endPan = () => {
    ...
    panning = false;
    try {
      canvasArea.releasePointerCapture(pid);
    } catch {}
    pid = null;
