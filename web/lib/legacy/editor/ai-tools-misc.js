...
export function wireAIToolsMisc({
  ...
  document
    .getElementById('ge-harmonize-color')
    ?.addEventListener('input', (e) => {
      document.getElementById('ge-harmonize-color-label').textContent = (
        e.target.value / 100
      ).toFixed(2);
      if (harmColorPrev)
        harmColorPrev.style.opacity = (
          parseInt(e.target.value, 10) / 100
        ).toFixed(2);
    });
  document
    .getElementById('ge-harmonize-seam')
    ?.addEventListener('input', (e) => {
      document.getElementById('ge-harmonize-seam-label').textContent = (
        e.target.value / 100
      ).toFixed(2);
      if (harmSeamPrev)
        harmSeamPrev.style.opacity = (
          parseInt(e.target.value, 10) / 100
        ).toFixed(2);
    });
  ...
  document.getElementById('ge-harmonize-run')?.addEventListener('click', () => {
    const prompt =
      document.getElementById('ge-harmonize-prompt')?.value?.trim() ||
      'photorealistic, natural lighting, seamless blend';
    const color_match =
      parseInt(document.getElementById('ge-harmonize-color')?.value || '65') /
      100;
    const seam_fix =
      parseInt(document.getElementById('ge-harmonize-seam')?.value || '0') /
      100;
    const bodyFeather = Math.max(
      6,
      Math.round(Math.min(state.imgWidth, state.imgHeight) * 0.012),
    );
    const seamFeather = Math.max(
      8,
      Math.round(Math.min(state.imgWidth, state.imgHeight) * 0.015),
    );
    ...
    if (!body_mask) {
      if (uiModule)
        uiModule.showToast(
          'Harmonize needs a second layer pasted/imported over the base photo — nothing to color-match against.',
          6000,
        );
      return;
    }
    ...
    applyImageTool(
      ...
      payload,
      'Harmonized',
      document.getElementById('ge-harmonize-run'),
    );
  });
  ...
  document
    .getElementById('ge-upscale-2x')
    ?.addEventListener('click', () => canvasUpscale(2));
  document
    .getElementById('ge-upscale-4x')
    ?.addEventListener('click', () => canvasUpscale(4));

  // ── AI upscale (Real-ESRGAN, no diffusion server required) ──
  document
    .getElementById('ge-upscale-ai')
    ?.addEventListener('click', async () => {
      ...
      try {
        ...
        upWp.element.style.cssText =
          'display:inline-block;vertical-align:middle;position:relative;top:1px;margin-right:6px;width:14px;height:14px;';
        btn.innerHTML = '';
  ...
  // ── Style transfer ──
  document
    .getElementById('ge-style-strength')
    ?.addEventListener('input', (e) => {
      document.getElementById('ge-style-strength-label').textContent = (
        parseInt(e.target.value) / 100
      ).toFixed(2);
    });
  document
    .getElementById('ge-style-run')
    ?.addEventListener('click', async () => {
      const btn = document.getElementById('ge-style-run');
      const prompt = document.getElementById('ge-style-prompt').value.trim();
      if (!prompt) {
        uiModule.showToast('Enter a style prompt');
        return;
      }
      const strength =
        parseInt(document.getElementById('ge-style-strength').value) / 100;
      btn.disabled = true;
      ...
      try {
        ...
        const res = await fetch(`${apiBase}/api/gallery/style-transfer`, {
          method: 'POST',
          credentials: 'same-origin',
          body: fd,
        });
  ...
  document
    .getElementById('ge-add-layer')
    ?.addEventListener('click', addEmptyLayer);
