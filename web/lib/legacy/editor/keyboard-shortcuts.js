  ...
  document.addEventListener('keydown', (e) => {
    ...
    if ((e.ctrlKey || e.metaKey) && !isAltGrEvent(e)) {
      ...
      if ((e.key === 's' || e.key === 'S') && !e.altKey) {
        e.preventDefault();
        document
          .getElementById(e.shiftKey ? 'ge-export-gallery' : 'ge-save')
          ?.click();
      }
          ...
          tmp.toBlob((blob) => {
            if (blob && navigator.clipboard?.write) {
              navigator.clipboard
                ...
                .then(() => {
                  uiModule.showToast(
                    isCut ? 'Cut to clipboard' : 'Copied to clipboard',
                  );
                })
                .catch(() =>
                  uiModule.showToast(
                    isCut ? 'Cut (editor only)' : 'Copied (editor only)',
                  ),
                );
            }
          }, 'image/png');
      ...
      if ((e.key === 'x' || e.key === 'c') && state.lassoPoints.length >= 3) {
        ...
        const feather = parseInt(
          document.getElementById('ge-lasso-feather')?.value || '0',
        );
        const grow = parseInt(
          document.getElementById('ge-lasso-grow')?.value || '0',
        );
      ...
      if (
        ...
        state.lassoPoints = [
          { x: 0, y: 0 },
          { x: state.imgWidth, y: 0 },
          { x: state.imgWidth, y: state.imgHeight },
          { x: 0, y: state.imgHeight },
        ];
        state.lassoActive = false;
        composite();
        drawLassoOverlay();
    ...
    if (e.key === '[' || e.key === ']') {
      ...
      state.brushSize = Math.max(
        1,
        Math.min(800, Math.round(state.brushSize * factor)),
      );
      try {
        brushSizeSync(null);
      } catch {}
