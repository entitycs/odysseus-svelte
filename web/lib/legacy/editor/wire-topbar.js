...
const TOPBAR_MENU_IDS = [
  'ge-image-menu',
  'ge-filter-menu',
  'ge-resize-menu',
  'ge-save-menu',
];
const TOPBAR_TRIGGER_IDS = [
  'ge-image-menu-btn',
  'ge-filter-menu-btn',
  'ge-resize-menu-btn',
  'ge-save-menu-btn',
...
export function wireTopbar(deps) {
  ...
  // unchanged.
  {
    ...
    if (saveBtn && saveMenu) {
      ...
      window.addEventListener('resize', () => {
        if (!saveMenu.hidden) positionSaveMenu();
      });
  ...
  document.getElementById('ge-zoom-in')?.addEventListener('click', () => {
    state.zoom = Math.min(5, state.zoom * 1.25);
    applyZoom();
  });
  document.getElementById('ge-zoom-out')?.addEventListener('click', () => {
  ...
  document
    .getElementById('ge-export-gallery')
    ?.addEventListener('click', exportToGallery);
  document
    .getElementById('ge-download')
    ?.addEventListener('click', downloadPNG);
  document
    .getElementById('ge-save-project')
    ?.addEventListener('click', saveProject);
  document
    .getElementById('ge-load-project')
    ?.addEventListener('click', loadProjectPrompt);
  ...
  function applyEdgeAction(hardDelete) {
    ...
    composite();
    uiModule.showToast(
      hardDelete ? `Edges deleted ${width}px` : `Edges feathered ${width}px`,
    );
  }
  {
    ...
    if (btn && menu) {
      btn.addEventListener('click', (e) => {
        ...
        if (!menu.hidden) {
          ...
          setTimeout(
            () => document.getElementById('ge-edge-width')?.select(),
            0,
          );
        }
      });
      document
        .getElementById('ge-edge-feather')
        ?.addEventListener('click', () => {
          menu.hidden = true;
          applyEdgeAction(false);
        });
      document
        .getElementById('ge-edge-delete')
        ?.addEventListener('click', () => {
          menu.hidden = true;
          applyEdgeAction(true);
        });
      document
        .getElementById('ge-edge-width')
