...
export function wireInpaintControls({
  ...
  function scheduleInpaintEdgeRefresh() {
    ...
    requestAnimationFrame(() => {
      ...
      const feather = parseInt(
        document.getElementById('ge-feather-slider')?.value || '0',
        10,
      );
      const edge = parseInt(
        document.getElementById('ge-edgestroke-slider')?.value || '0',
        10,
      );
  ...
  document
    .getElementById('ge-inpaint-invert')
    ?.addEventListener('click', () => {
      if (!state.maskCtx || !state.maskCanvas) return;
      const imgData = state.maskCtx.getImageData(
        0,
        0,
  ...
  document
    .getElementById('ge-inpaint-mode-paint')
    ?.addEventListener('click', () => setInpaintMode(false));
  document
    .getElementById('ge-inpaint-mode-erase')
    ?.addEventListener('click', () => setInpaintMode(true));
  ...
  document
    .querySelector('.ge-inpaint-mask-color')
    ?.addEventListener('input', (e) => applyMaskTintFromHex(e.target.value));
  document
    .getElementById('ge-topbar-mask-color')
    ?.addEventListener('input', (e) => applyMaskTintFromHex(e.target.value));
  // Use the in-house HSV picker for the topbar swatch.
  const topbarMaskColor = document.getElementById('ge-topbar-mask-color');
  if (topbarMaskColor) {
    try {
      attachColorPicker(topbarMaskColor);
      topbarMaskColor.value = topbarMaskColor.value;
    } catch {}
  }
}
