...
import { drawHistogram } from '$lib/legacy/editor/fx/histogram.js';
import {
  ADJ_ICONS,
  adjLayerLabel,
  defaultAdjParams,
...
import modalManager from '$lib/legacy/modalManager.js';

export function createAdjPopupSystem({
  composite,
  saveState,
  renderLayerPanel,
}) {
  ...
  function openFxPopup(layer, anchorEl) {
    ...
    if (
      state.fxMenuEl &&
      document.body.contains(state.fxMenuEl) &&
      state.fxMenuEl._layerId === layer.id
    ) {
      closeFxMenu();
      return;
    }
    ...
    backdrop.style.cssText =
      'position:fixed;inset:0;z-index:10001;background:transparent;pointer-events:auto;touch-action:none;';
  ...
  function minimiseAdjPopup(pop) {
    ...
    const popupId =
      pop._modalId || `ge-fx-popup-${Math.random().toString(36).slice(2, 8)}`;
  ...
  function closeAdjPopup() {
    if (state.adjPopupEl) {
      ...
      if (state.adjPopupEl._escHandler) {
        document.removeEventListener(
          'keydown',
          state.adjPopupEl._escHandler,
          true,
        );
      }
      if (state.adjPopupEl._modalId) {
        try {
          modalManager.unregister(state.adjPopupEl._modalId);
        } catch {}
      }
  ...
  function openAdjPopup(layer, type, anchorEl, existingAdj) {
    ...
    pop
      .querySelector('.ge-adj-close')
      ?.addEventListener('click', closeAdjPopup);
    pop
      .querySelector('.ge-adj-min')
      ?.addEventListener('click', () => minimiseAdjPopup(pop));
    ...
    if (head) {
      ...
      head.addEventListener('pointerdown', (e) => {
        ...
        const onMove = (ev) => {
          const nx = Math.max(
            0,
            Math.min(window.innerWidth - 60, r0.left + (ev.clientX - startX)),
          );
          const ny = Math.max(
            0,
            Math.min(window.innerHeight - 30, r0.top + (ev.clientY - startY)),
          );
          setPos(nx, ny);
        };
        const onUp = () => {
    ...
    pop
      .querySelector('[data-adj-action="cancel"]')
    ...
    pop
      .querySelector('[data-adj-action="ok"]')
      ?.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        suppressLayerGhostTap();
        saveState(
          editing
            ? `Edit ${adjLayerLabel(type)}`
            : `Add ${adjLayerLabel(type)}`,
        );
        const params = layer._stagedAdj.params;
        layer._stagedAdj = null;
        if (editing) {
          const existing = (layer.adjLayers || []).find(
            (a) => a.id === existingAdj.id,
          );
  ...
  function buildAdjBody(layer, type, body, popEl) {
    ...
    `;
      ...
      body
        .querySelector('.ge-adj-hist-details')
    ...
    `;
      body
        .querySelector('.ge-adj-cb-tone-select')
      ...
      h.addEventListener('pointerdown', (e) => {
        ...
        const onMove = (ev) => {
          ...
          const updateRow = (key, displayVal) => {
            const sl = bodyEl.querySelector(
              `input[type="range"][data-key="${key}"]`,
            );
            if (sl)
              sl.value = String(
                key === 'gamma'
                  ? Math.round(layer._stagedAdj.params.gamma * 100)
                  : layer._stagedAdj.params[key],
              );
            const val = sl?.parentElement.querySelector('.ge-adj-value');
            if (val) val.textContent = displayVal;
          };
          if (which === 'inBlack')
            updateRow('inBlack', String(layer._stagedAdj.params.inBlack));
          if (which === 'inWhite')
            updateRow('inWhite', String(layer._stagedAdj.params.inWhite));
          if (which === 'gamma')
            updateRow('gamma', layer._stagedAdj.params.gamma.toFixed(2) + 'γ');
  ...
  function syncFxPanelToActiveLayerIfPresent() {
    /* no-op */
  }
