...
export function wireInpaintButtons({
  buildMergedMaskCanvas,
  dilateMask,
  applyInpaintFeather,
  getSelectedAIEndpoint,
  ensureActiveMaskLayer,
  saveState,
  createLayer,
  composite,
  renderLayerPanel,
  ...
  async function runInpaint({
    ...
    if (!preMerged) {
      if (uiModule)
        uiModule.showToast('Draw the area you want to inpaint first');
      return;
    }
    const pmCtx = preMerged.getContext('2d');
    const maskData = pmCtx.getImageData(
      0,
      0,
      preMerged.width,
      preMerged.height,
    ...
    try {
      const area =
        state.container && state.container.querySelector('.ge-canvas-area');
      ...
      if (area && mainRect.width && mainRect.height) {
        ...
        let cx = state.imgWidth / 2,
          cy = state.imgHeight / 2;
        try {
          const merged = buildMergedMaskCanvas();
          if (merged) {
            const d = merged
              .getContext('2d')
              .getImageData(0, 0, merged.width, merged.height).data;
            let minX = merged.width,
              maxX = 0,
              minY = merged.height,
              maxY = 0;
    ...
    try {
      ...
      flatCanvas.width = state.imgWidth;
      flatCanvas.height = state.imgHeight;
      ...
      const padPx = Math.min(
        ...
        Math.max(
          20,
          Math.round(Math.min(state.imgWidth, state.imgHeight) * 0.04),
        ),
      );
      ...
      if (!data.image)
        throw new Error('No image returned from inpaint endpoint');
      ...
      resultImg.onload = () => {
        ...
        try {
          ...
          aiSnap.width = state.imgWidth;
          aiSnap.height = state.imgHeight;
          ...
          if (uiModule)
            uiModule.showToast(
              'Inpaint complete — drag Edge feather / Edge stroke to blend',
              5000,
            );
        } catch (renderErr) {
          ...
          if (uiModule)
            uiModule.showToast(
              'Inpaint render failed: ' + (renderErr.message || renderErr),
              6000,
            );
        }
      };
      resultImg.onerror = (e) => {
        ...
        if (uiModule)
          uiModule.showToast('Inpaint result failed to decode', 6000);
      };
  ...
  // Generate.
  document
    .getElementById('ge-inpaint-run')
    .addEventListener('click', async () => {
      const prompt = document
        .getElementById('ge-inpaint-prompt')
        ?.value?.trim();
      if (!prompt) {
        if (uiModule) uiModule.showToast('Enter a prompt for inpainting');
        return;
      }
      const strength =
        parseInt(document.getElementById('ge-strength-slider')?.value || '75') /
        100;
      await runInpaint({
        prompt,
        strength,
  ...
  document
    .getElementById('ge-inpaint-remove')
    .addEventListener('click', async () => {
      const sel = getSelectedAIEndpoint('inpaint');
      ...
      if (isOpenAI) {
        ...
        strength =
          parseInt(
            document.getElementById('ge-strength-slider')?.value || '75',
          ) / 100;
      } else {
  ...
  document
    .getElementById('ge-inpaint-outpaint')
    .addEventListener('click', async () => {
      // 1) Flatten visible layers to detect alpha=0 (empty) regions.
      const flat = document.createElement('canvas');
      flat.width = state.imgWidth;
      flat.height = state.imgHeight;
      ...
      fctx.globalAlpha = 1;
      const flatData = fctx.getImageData(
        0,
        0,
        state.imgWidth,
        state.imgHeight,
      ...
      maskRaw.width = state.imgWidth;
      maskRaw.height = state.imgHeight;
      ...
      if (emptyCount === 0) {
        if (uiModule)
          uiModule.showToast(
            'No empty areas to outpaint — canvas is fully covered.',
          );
        return;
      }
      ...
      expanded.width = state.imgWidth;
      expanded.height = state.imgHeight;
      ...
      if (!mask) {
        if (uiModule) uiModule.showToast('No active layer for outpaint');
        return;
      }
      const savedMask = mask.ctx.getImageData(
        0,
        0,
        mask.canvas.width,
        mask.canvas.height,
      ...
      const prompt =
        userP ||
        'seamless natural continuation of the surrounding image, photorealistic, matching style, no objects, no people, no text';
      const strength = 0.99;
      try {
        await runInpaint({
          prompt,
