 ...
 * }} deps
 */
import { sortModelIds } from '$lib/legacy/modelSort';
...
function modelCaps(modelId, endpointName, endpointType) {
  ...
  if (textOnly.test(id) && !/image/i.test(id))
  ...
  if (
    ...
    const isInpaintModel =
      /inpaint|edit|fill/i.test(id) || /inpaint|edit|fill/i.test(name);
...
export function wireAIModelSelectors({
  container,
  apiBase,
  ...
  container.addEventListener(
    ...
    (e) => {
      const sel = e.target.closest('select');
      if (sel && sel.value !== '__serve_cookbook__')
        sel._prevServeValue = sel.value;
    },
    true,
  );
  ...
  if (
    !aiGenSelect &&
    !aiInpaintSelect &&
    !document.querySelector('select.ge-tool-model')
  )
    return;
  ...
  async function loadAIModels(opts = {}) {
    try {
      ...
      if (aiInpaintSelect)
        aiInpaintSelect.innerHTML = '<option value="">Auto</option>';
      const perToolSelects = Array.from(
        document.querySelectorAll('select.ge-tool-model'),
      );
      for (const ts of perToolSelects)
        ts.innerHTML = '<option value="">Auto</option>';
      let firstGen = null;
      ...
      for (const ep of endpoints) {
        ...
        for (const modelId of models) {
          ...
          const shortModel = modelId
            ...
            : ep.name || ep.base_url;
          const epHint =
            modelId && ep.name && ep.name !== modelId ? ` · ${ep.name}` : '';
          ...
          if (caps.inpaint && aiInpaintSelect) {
            ...
            if (
              ...
              selectedInpaint = value;
            // Prefer dedicated inpaint/edit models for default selection.
            if (
              epUsable &&
              !firstInpaint &&
              (!modelId ||
                /inpaint|edit|fill|gpt-image/i.test(modelId) ||
                /inpaint|edit|fill/i.test(ep.name || ''))
            ) {
              firstInpaint = value;
            }
          }
      ...
      const hasValue = (sel, value) =>
        !!value && [...sel.options].some((o) => o.value === value);
      if (aiGenSelect) {
        if (selectedGen) aiGenSelect.value = selectedGen;
        else if (hasValue(aiGenSelect, prevGenValue))
          aiGenSelect.value = prevGenValue;
        else if (firstGen) aiGenSelect.value = firstGen;
      }
      if (aiInpaintSelect) {
        if (selectedInpaint) aiInpaintSelect.value = selectedInpaint;
        else if (hasValue(aiInpaintSelect, prevInpaintValue))
          aiInpaintSelect.value = prevInpaintValue;
      ...
      for (const ts of perToolSelects) {
        ...
        ts.addEventListener('change', () => {
          ...
          prevValue = ts.value;
          try {
            localStorage.setItem(key, ts.value);
          } catch {}
        });
      }
    } catch (e) {
      ...
      const fallback =
        '<option value="">Auto</option><option value="" disabled>──────────</option><option value="__serve_cookbook__">+ Serve a model in Cookbook…</option>';
      if (aiGenSelect) aiGenSelect.innerHTML = fallback;
      if (aiInpaintSelect) aiInpaintSelect.innerHTML = fallback;
      document.querySelectorAll('select.ge-tool-model').forEach((ts) => {
        ts.innerHTML = fallback;
      });
      const wireServe = (sel) => {
        if (!sel) return;
        let prev = sel.value;
  ...
  const onModelEndpointsUpdated = (e) => {
    if (!container.isConnected) {
      window.removeEventListener(
        'ge:model-endpoints-updated',
        onModelEndpointsUpdated,
      );
      return;
    }
