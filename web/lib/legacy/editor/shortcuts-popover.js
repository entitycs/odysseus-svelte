 ...
 */
import { shortcutsPopupHTML } from '$lib/legacy/editor/build/popups.js';
...
export function createShortcutsPopover() {
  ...
  function ensurePopover() {
    ...
    el.querySelector('#ge-shortcuts-close').addEventListener('click', () =>
  ...
  function positionPopover(el, anchor) {
    ...
    top = Math.max(
      margin,
      Math.min(top, window.innerHeight - pr.height - margin),
    );
    el.style.left = left + 'px';
    el.style.top = top + 'px';
  }
  ...
  function toggleShortcuts(show) {
    ...
    if (open) {
      ...
      let saved = null;
      try {
        saved = JSON.parse(localStorage.getItem('ge-shortcuts-pos') || 'null');
      } catch {}
      if (saved && saved.left && saved.top) {
        ...
        requestAnimationFrame(() => {
          ...
          if (r.right > window.innerWidth)
            el.style.left = window.innerWidth - r.width - m + 'px';
          if (r.bottom > window.innerHeight)
            el.style.top = window.innerHeight - r.height - m + 'px';
