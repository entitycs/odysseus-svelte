...
import { snapModalToZone } from '$lib/legacy/tileManager.js';
...
export function saveCustomTheme(name, colors, opts) {
  ...
  if (opts) {
    ...
    if (opts.bgEffectIntensity !== undefined)
      entry.bgEffectIntensity = opts.bgEffectIntensity;
...
function _syncCustomThemesToServer(ct) {
  try {
    ...
    console.warn('Theme sync (custom) error:', e);
  }
}
...
export function applyBgEffectSize(v) {
  ...
  const n =
    v === undefined || v === null || isNaN(v)
      ? 1
      : Math.max(0.2, Math.min(3, Number(v)));
...
export function save(name, colors, opts) {
  ...
  if (opts) {
    ...
    if (opts.density && opts.density !== DEFAULT_DENSITY)
      obj.density = opts.density;
    if (opts.bgPattern && opts.bgPattern !== 'none')
      obj.bgPattern = opts.bgPattern;
    if (opts.bgEffectColor) obj.bgEffectColor = opts.bgEffectColor;
    if (opts.bgEffectIntensity !== undefined && opts.bgEffectIntensity !== 1)
      obj.bgEffectIntensity = opts.bgEffectIntensity;
    if (opts.bgEffectSize !== undefined && opts.bgEffectSize !== 1)
      obj.bgEffectSize = opts.bgEffectSize;
...
export function initThemeUI() {
  ...
  try {
    ...
    console.warn('Color picker init failed', e);
  }
  ...
  try {
    ...
    console.warn('syncAdvancedPickers on init failed', e);
  }
  ...
  if (themeTabs) {
    themeTabs.addEventListener('click', (e) => {
      ...
      document
        .querySelectorAll('.theme-tab-panel')
      ...
      if (opWrap)
        opWrap.classList.toggle('hidden', targetId !== 'theme-tab-customize');
  ...
  if (customEntries.length > 0 && userGrid && userCard) {
    ...
    userGrid.innerHTML = customEntries
      .map(
        ([name, c]) => `
    ...
    g.querySelectorAll('.theme-swatch').forEach((sw) => {
      sw.addEventListener('click', (e) => {
        ...
        const p =
          ct && ct.bgPattern
            ? ct.bgPattern
            : THEME_DEFAULT_PATTERN[name] || 'none';
        const ec =
          ct && ct.bgEffectColor
            ? ct.bgEffectColor
            : THEME_DEFAULT_EFFECT_COLOR[name] || '';
        const ei =
          ct && ct.bgEffectIntensity !== undefined
            ? ct.bgEffectIntensity
            : THEME_DEFAULT_INTENSITY[name] !== undefined
              ? THEME_DEFAULT_INTENSITY[name]
              : 1;
        const sz = ct && ct.bgEffectSize !== undefined ? ct.bgEffectSize : 1;
        const fr =
          ct && ct.frosted !== undefined
            ? !!ct.frosted
    ...
    g.querySelectorAll('.theme-delete-btn').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        if (uiModule && uiModule.styledConfirm) {
          if (
            !(await uiModule.styledConfirm(`Delete theme "${name}"?`, {
              confirmText: 'Delete',
              danger: true,
            }))
          )
    ...
    document.querySelectorAll('.color-reset-btn[data-reset]').forEach((btn) => {
      ...
      if (picker && refColors[key]) {
        btn.classList.toggle(
          'changed',
          picker.value.toLowerCase() !== refColors[key].toLowerCase(),
        );
      }
    });
    document
      .querySelectorAll('.color-reset-btn[data-reset-adv]')
      .forEach((btn) => {
        ...
        if (picker && ref) {
          btn.classList.toggle(
            'changed',
            picker.value.toLowerCase() !== ref.toLowerCase(),
          );
        }
      });
  }
    ...
    el.addEventListener('input', () => {
      ...
      if (_activeName && _customMap && _customMap[_activeName]) {
        ...
        saveCustomTheme(_activeName, colors, {
          font: _activeSaved.font,
          density: _activeSaved.density,
          bgPattern: _activeSaved.bgPattern,
          bgEffectColor: _activeSaved.bgEffectColor,
  ...
  if (saveGoBtnOld && saveNameInputOld) {
    ...
    const doSave = () => {
      ...
      const slug = name
        ...
        .replace(/[^a-z0-9-]/g, '');
      if (!slug) {
        saveError.textContent = 'Invalid name.';
        saveError.style.display = 'block';
        return;
      }
      if (THEMES[slug]) {
        saveError.textContent = 'Cannot overwrite a built-in theme.';
        saveError.style.display = 'block';
        return;
      }
      const colors = {};
      ...
      Object.entries(pickerIds2).forEach(([k, pid]) => {
        colors[k] = document.getElementById(pid).value;
      });
      const adv = {};
      const defaults = computeAdvancedDefaults(colors);
      ...
      if (result === 'limit') {
        saveError.textContent =
          'Max ' + MAX_CUSTOM_THEMES + ' custom themes. Delete one first.';
        saveError.style.display = 'block';
        return;
      }
      ...
      newGoBtn.innerHTML =
        '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg><span>Saved</span>';
      setTimeout(() => {
  ...
  function readCurrentColors() {
    ...
    const c = {};
    Object.entries(pickerIds2).forEach(([k, pid]) => {
      c[k] = document.getElementById(pid).value;
    });
    return c;
  }
  ...
  document
    .querySelectorAll('.color-reset-btn[data-reset-effect]')
    .forEach((btn) => {
      const newBtn = btn.cloneNode(true);
      btn.parentNode.replaceChild(newBtn, btn);
      newBtn.addEventListener('click', () => {
        ...
        if (ec) {
          ...
          applyBgEffectColor('');
          const s = getSaved();
          if (s) _saveFull(s.name, s.colors);
        }
      });
    });
  ...
  document
    ...
    .forEach((btn) => {
      ...
      newBtn.addEventListener('click', () => {
        ...
        if (picker) {
          picker.value =
            refDefaults[key] || computeAdvancedDefaults(refColors)[key];
          picker.dispatchEvent(new Event('input'));
        }
      });
    });
  ...
  const _initPattern =
    (saved && saved.bgPattern) ||
    (saved && THEME_DEFAULT_PATTERN[saved.name]) ||
    'none';
  const _initEffectColor =
    (saved && saved.bgEffectColor) ||
    (saved && THEME_DEFAULT_EFFECT_COLOR[saved.name]) ||
    '';
  const _initEffectIntensity =
    saved && saved.bgEffectIntensity !== undefined
      ? saved.bgEffectIntensity
      : saved && THEME_DEFAULT_INTENSITY[saved.name] !== undefined
        ? THEME_DEFAULT_INTENSITY[saved.name]
        : 1;
  const _initEffectSize =
    saved && saved.bgEffectSize !== undefined ? saved.bgEffectSize : 1;
  const _initFrosted =
    saved && saved.frosted !== undefined
      ? !!saved.frosted
  ...
  if (fontSelect) {
    const nf = fontSelect.cloneNode(true);
    fontSelect.parentNode.replaceChild(nf, fontSelect);
    nf.value = _initFont;
    nf.addEventListener('change', () => {
      applyFontDensity(
        nf.value,
        document.getElementById('theme-density-select').value,
      );
      const s = getSaved();
      if (s) _saveFull(s.name, s.colors);
    });
    ...
    fetch('/api/fonts/custom', { credentials: 'same-origin' })
      ...
      .then((data) => {
        ...
        nf.querySelectorAll('option[data-custom-font]').forEach((o) =>
          o.remove(),
        );
  ...
  if (densitySelect) {
    const nd = densitySelect.cloneNode(true);
    densitySelect.parentNode.replaceChild(nd, densitySelect);
    nd.value = _initDensity;
    nd.addEventListener('change', () => {
      applyFontDensity(
        document.getElementById('theme-font-select').value,
        nd.value,
      );
      const s = getSaved();
      if (s) _saveFull(s.name, s.colors);
  ...
  if (textSizeSelect) {
    const nts = textSizeSelect.cloneNode(true);
    textSizeSelect.parentNode.replaceChild(nts, textSizeSelect);
    let initScale = DEFAULT_UI_SCALE;
    try {
      initScale = localStorage.getItem(UI_SCALE_KEY) || DEFAULT_UI_SCALE;
    } catch (e) {}
    nts.value = initScale;
    ...
    nts.addEventListener('change', () => {
      ...
      try {
        localStorage.setItem(UI_SCALE_KEY, nts.value);
      } catch (e) {}
    });
  }
  if (patternSelect) {
    const np = patternSelect.cloneNode(true);
    patternSelect.parentNode.replaceChild(np, patternSelect);
  ...
  if (harmonyAccentEl) {
    try {
      attachColorPicker(harmonyAccentEl);
    } catch (_) {}
  }
  ...
  if (importGoEl && importAreaEl) {
    ...
    newGo.addEventListener('click', () => {
      ...
      let parsed;
      try {
        parsed = JSON.parse(importAreaEl.value.trim());
      } catch {
        saveError.textContent = 'Invalid JSON.';
        saveError.style.display = 'block';
        return;
      }
      ...
      if (missing.length) {
        saveError.textContent = 'Missing: ' + missing.join(', ');
        saveError.style.display = 'block';
        return;
      }
      const hexRe = /^#[0-9a-fA-F]{6}$/;
      for (const k of required) {
        if (!hexRe.test(colors[k])) {
          saveError.textContent = 'Bad hex for ' + k;
          saveError.style.display = 'block';
          return;
        }
      }
      ...
      if (colors.advanced && typeof colors.advanced === 'object')
        colorData.advanced = colors.advanced;
      const slug =
        name
          .toLowerCase()
          .replace(/\s+/g, '-')
      ...
      applyFontDensity(
        opts.font || DEFAULT_FONT,
        opts.density || DEFAULT_DENSITY,
      );
...
const _THEME_ZONE_MAP = {
  ...
  'adv-toggleActive':
    '.mode-toggle-btn.active, .admin-switch input:checked + .admin-slider',
...
function _clearThemeZoneHighlight() {
  document
    .querySelectorAll('.theme-zone-highlight')
...
function _flashAutosaved(label = 'Auto-saved') {
  ...
  if (!pill) {
    ...
    pill.innerHTML =
      '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg><span></span>';
...
function _initRain() {
  ...
  function draw() {
    ...
    if (drops.length < MAX_DROPS * inten && Math.random() < 0.6 * inten)
      spawn();

    for (let i = drops.length - 1; i >= 0; i--) {
      const d = drops[i];
      d.y += d.speed * speedMult;
      if (d.y > H + d.len * sizeMult) {
        drops.splice(i, 1);
        continue;
      }
...
function _bgSmoothNoise(x, y) {
  const ix = Math.floor(x),
    ...
    fx = x - ix,
    fy = y - iy;
  const a = _bgNoise2d(ix, iy),
    b = _bgNoise2d(ix + 1, iy),
    cc = _bgNoise2d(ix, iy + 1),
    d = _bgNoise2d(ix + 1, iy + 1);
...
function _initPerlinFlow() {
  ...
  function draw() {
    if (!document.body.classList.contains('bg-pattern-perlin-flow')) {
      window.removeEventListener('resize', _onResize);
      canvas.remove();
      return;
    }
...
function _initPetals() {
  ...
  function draw() {
    if (!document.body.classList.contains('bg-pattern-petals')) {
      window.removeEventListener('resize', _onResize);
      canvas.remove();
      return;
    }
...
function _initSparkles() {
  ...
  function draw() {
    if (!document.body.classList.contains('bg-pattern-sparkles')) {
      window.removeEventListener('resize', _onResize);
      canvas.remove();
      return;
    }
...
async function _initWithSync() {
  ...
  try {
    const res = await fetch('/api/prefs/custom-themes', {
      credentials: 'same-origin',
    });
    const data = await res.json();
    ...
    console.warn('Custom theme server sync failed:', e);
  }
  initThemeUI();
}
