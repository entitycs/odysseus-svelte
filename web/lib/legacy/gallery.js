...
import {
  ...
  isEditorOpen,
  openEditor,
} from '$lib/legacy/galleryEditor.js';
...
function _computeFetchLimit() {
  ...
  const gridW =
    (grid && grid.clientWidth) || Math.min(window.innerWidth * 0.9, 1100);
...
async function _fetchLibrary(append) {
  ...
  if (!append && _items.length === 0) {
    ...
    try {
      _knownEmpty = localStorage.getItem('gallery-known-empty') === '1';
    } catch (_) {}
  ...
  try {
    const res = await fetch(`${API_BASE}/api/gallery/library?${params}`, {
    ...
    try {
      ...
      if (_noFilters) {
        if (_items.length === 0)
          localStorage.setItem('gallery-known-empty', '1');
...
async function _bulkUpload(filesOrItems, fallbackAlbumId) {
  ...
  async function worker() {
    while (true) {
      ...
      if (status)
        status.textContent = `${done}/${total}${dupes ? ` (${dupes} duplicates)` : ''}`;
    }
  }
  await Promise.all(
...
async function _handleGalleryDrop(e) {
    ...
    if (entry.isDirectory) {
      ...
      if (!album) {
        try {
          ...
          const data = await res.json();
          if (data && data.id)
            album = { id: data.id, name: data.name || entry.name };
        } catch (err) {
          console.error('Failed to create album for', entry.name, err);
        }
      }
      if (!album) continue;
  ...
  if (looksLikeFolderUri) {
    uiModule.showError(
      'Browsers can’t read folders dropped from native file managers (Thunar/Nautilus). Use the "Upload album" tile in the Albums tab instead.',
    );
...
function _renderAlbums() {
  ...
  if (filterC) {
    ...
    filterC
      .querySelector('.gallery-chip[data-album=""]')
    ...
    filterC
      .querySelector('.gallery-chip-fav')
      ?.addEventListener('click', () => {
    ...
    filterC
      .querySelector('.gallery-chip-clear[data-clear="album"]')
    ...
    filterC
      .querySelectorAll('.gallery-chip-clear[data-clear-tag]')
      .forEach((x) => {
...
function _ensureAlbumsToolbar(container) {
  ...
  container
    .querySelector('#gallery-albums-select-btn')
    .addEventListener('click', () => {
      _setAlbumSelectMode(!_albumSelectMode);
    });
  container
    .querySelector('#gallery-albums-bulk-cancel')
    .addEventListener('click', () => {
      _setAlbumSelectMode(false);
    });
  container
    .querySelector('#gallery-albums-bulk-all')
  ...
  container
    .querySelector('#gallery-albums-bulk-delete')
    .addEventListener('click', (e) => {
      e.stopPropagation();
      if (!_albumSelected.size) {
        uiModule.showToast('Select albums first');
        return;
      }
...
function _setAlbumSelectMode(on) {
  ...
  const container = document.getElementById('gallery-albums-container');
  container.querySelector('#gallery-albums-select-btn span').textContent = on
    ? 'Cancel'
    : 'Select';
  container
    .querySelector('#gallery-albums-select-btn')
  ...
  container
    .querySelector('#gallery-albums-bulk-bar')
...
function _renderAlbumsGrid() {
  ...
  albums.forEach((a) => {
    ...
    const cls =
      'gallery-album-card' +
      (_albumSelectMode ? ' gallery-card-selectable' : '') +
      (isSel ? ' selected' : '');
    html += `
...
function _wireAlbumsEvents(scope) {
  ...
  container
    .querySelectorAll('.gallery-album-card[data-album]')
    .forEach((card) => {
      card.addEventListener('click', (e) => {
        ...
        const photosTab = modal?.querySelector(
          '.gallery-tab[data-tab="images"]',
        );
        photosTab?.click();
      });
    });
  ...
  container.querySelectorAll('.gallery-album-menu-btn').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      ...
      const pop = container.querySelector(
        `.gallery-album-menu-pop[data-album="${CSS.escape(id)}"]`,
      );
      const wasOpen = pop && !pop.hidden;
      container.querySelectorAll('.gallery-album-menu-pop').forEach((p) => {
        p.hidden = true;
      });
  ...
  document
    .getElementById('gallery-albums-new')
    ?.addEventListener('click', async () => {
      const name = uiModule.styledPrompt
        ? await uiModule.styledPrompt('Name your new album.', {
            title: 'New album',
  ...
  document
    .getElementById('gallery-albums-upload')
    ?.addEventListener('click', () => {
      ...
      picker.addEventListener('change', async () => {
        ...
        if (!images.length) {
          if (uiModule)
            uiModule.showToast('No images or videos in that folder');
          return;
        }
...
async function _bulkDeleteAlbums(ids) {
  ...
  if (failed)
    uiModule.showError(`Failed to delete ${failed} of ${ids.length} albums`);
  else if (uiModule)
    uiModule.showToast(
      `Deleted ${ids.length} album${ids.length > 1 ? 's' : ''}`,
    );
  _setAlbumSelectMode(false);
  await _fetchAlbums();
  _renderAlbumsTab();
...
async function _renderEditorDrafts() {
  ...
  try {
    const res = await fetch(`${API_BASE}/api/editor-drafts`, {
      credentials: 'same-origin',
    });
    if (res.ok) {
  ...
  for (const id of [..._draftsSelected])
    if (!present.has(id)) _draftsSelected.delete(id);
  _draftsPaint();
  _draftsWireOnce();
}
  ...
  grid.innerHTML = filtered
    .map((d) => {
      const updated = d.updated_at
        ? _humanRelativeDate(new Date(d.updated_at))
        : '';
...
function _draftsSyncBulkBar() {
  ...
  if (all) {
    const q = _draftsSearch.trim().toLowerCase();
    const visible = _draftsCache.filter(
      (d) =>
        !q ||
        String(d.name || '')
          .toLowerCase()
...
function _draftsWireOnce() {
  ...
  _draftsWired = true;
  document
    .getElementById('gallery-editor-drafts-search')
    ?.addEventListener('input', (e) => {
      _draftsSearch = e.target.value || '';
      _draftsPaint();
    });
  document
    .getElementById('gallery-editor-drafts-select')
    ?.addEventListener('click', () => {
  ...
  document
    .getElementById('gallery-editor-drafts-select-all')
    ?.addEventListener('change', (e) => {
  ...
  document
    .getElementById('gallery-editor-drafts-bulk-cancel')
    ?.addEventListener('click', () => {
      _draftsSelectMode = false;
      _draftsSelected.clear();
      _draftsPaint();
  ...
  document
    .getElementById('gallery-editor-drafts-bulk-delete')
    ?.addEventListener('click', async () => {
      ...
      if (grid)
        ids.forEach((id) =>
          grid
            .querySelector(
              `.gallery-editor-draft-card[data-draft-id="${window.CSS && CSS.escape ? CSS.escape(id) : id}"]`,
            )
            ?.classList.add('gallery-draft-removing'),
        );
...
function _renderEditorLanding() {
  ...
  _renderEditorDrafts();
  document
    .getElementById('gallery-editor-template')
    ?.addEventListener('change', (e) => {
  ...
  document
    .getElementById('gallery-editor-new')
    ?.addEventListener('click', async () => {
  ...
  document
    .getElementById('gallery-editor-pick')
    ?.addEventListener('click', () => {
      document
        .querySelector('#gallery-modal .gallery-tab[data-tab="images"]')
        ?.click();
    });
}
...
function _renderSkeletons(n) {
  ...
  for (let i = 0; i < count; i++)
    html +=
      '<div class="gallery-card gallery-card-skeleton" aria-hidden="true"></div>';
  grid.innerHTML = html;
...
function _renderGrid() {
  ...
  if (_items.length === 0) {
    grid.innerHTML =
      uploadTile +
      '<div class="gallery-empty">No photos yet. Click Upload or drag-and-drop to get started!</div>';
    _wireUploadTile();
  ...
  _items.forEach((img) => {
    ...
    const promptPreview =
      labelText.length > 60 ? labelText.substring(0, 58) + '...' : labelText;
...
function _openDetail(img) {
  ...
  const _relAgo = (d) => {
    ...
    if (secs < 86400 * 365) {
      const mo = Math.floor(secs / (86400 * 30));
  ...
  detail.innerHTML = `
    <div class="gallery-detail-header">
      ...
      <div class="gallery-detail-menu-wrap">
        ...
        <div class="gallery-detail-menu dropdown" id="gallery-detail-menu" hidden>
          ...
          ${
            img.album_id
              ? `<button class="dropdown-item-compact" id="gallery-set-cover-btn">
    ...
    <div class="gallery-detail-body">
      ...
      <div class="gallery-detail-sidebar">
        ...
        ${
          aiTags
            ? `<div class="gallery-detail-section"><label>AI Tags</label><div class="gallery-ai-tags">${aiTags
                .split(',')
                .map((t) => t.trim())
                .filter(Boolean)
                .map(
                  (t) =>
                    `<button class="gallery-ai-chip gallery-aitag-chip" data-tag-filter="${_esc(t)}" title="AI-generated tag — click to filter to photos tagged “${_esc(t)}”"><span class="gallery-aitag-mark" aria-hidden="true">✦</span>${_esc(t)}</button>`,
                )
                .join('')}</div></div>`
            : ''
        }
        <div class="gallery-detail-section">
          <label>Tags</label>
          <div class="gallery-ai-tags" id="gallery-user-tag-chips">${userTags
            .split(',')
            .map((t) => t.trim())
            .filter(Boolean)
            .map(
              (t) =>
                `<button class="gallery-ai-chip gallery-user-chip" data-tag-filter="${_esc(t)}" title="Filter to photos tagged “${_esc(t)}”">${_esc(t)}<span class="gallery-tag-x" title="Remove tag" aria-label="Remove tag">×</span></button>`,
            )
            .join('')}</div>
  ...
  document
    .getElementById('gallery-detail-back')
    .addEventListener('click', () => {
  ...
  const _removeUserTag = async (tag, chip) => {
    const existing = (img.user_tags || img.tags || '')
      .split(',')
      .map((t) => t.trim())
      .filter(Boolean);
    const remaining = existing.filter(
      (e) => e.toLowerCase() !== String(tag).toLowerCase(),
    );
    const cleaned = remaining.join(', ');
    const ok = await _patchImage(img.id, { tags: cleaned });
    if (!ok) {
      if (uiModule) uiModule.showError('Failed to remove tag');
      return;
    }
    img.tags = cleaned;
  ...
  detail.querySelectorAll('[data-tag-filter]').forEach((chip) => {
    chip.addEventListener('click', (e) => {
      ...
      if (e.target.closest('.gallery-tag-x')) {
        _removeUserTag(chip.dataset.tagFilter, chip);
        return;
      }
      ...
      const photosTab = document.querySelector(
        '#gallery-modal .gallery-tab[data-tab="images"]',
      );
      photosTab?.click();
      _fetchLibrary(false);
      _renderAlbums();
  ...
  const _toggleDetailFavorite = async () => {
    ...
    if (menuItem)
      menuItem.innerHTML = data.favorite
        ? '&#9829; Favorited'
        : '&#9825; Favorite';
    ...
    if (headerBtn) {
      ...
      if (svg)
        svg.setAttribute('fill', data.favorite ? 'currentColor' : 'none');
    }
  };
  document
    .getElementById('gallery-fav-detail')
    .addEventListener('click', _toggleDetailFavorite);
  document
    .getElementById('gallery-detail-fav-header')
    ?.addEventListener('click', _toggleDetailFavorite);

  document
    .getElementById('gallery-ai-tag-btn')
    .addEventListener('click', async (e) => {
      ...
      const stage =
        document.getElementById('gallery-detail-image-wrap') ||
        document.getElementById('gallery-detail-img')?.parentElement;
      let overlay = null,
        spinner = null;
      if (stage) {
        overlay = document.createElement('div');
        overlay.style.cssText =
          'position:absolute;inset:0;display:flex;flex-direction:column;gap:8px;align-items:center;justify-content:center;background:color-mix(in srgb, var(--bg) 55%, transparent);z-index:5;';
        try {
          ...
          overlay.textContent = clearMode ? 'Clearing…' : 'AI tagging…';
        }
      ...
      try {
        ...
        if (data.ok) {
          ...
          uiModule.showError(
            data.error || (clearMode ? 'Clear failed' : 'AI tagging failed'),
          );
        }
      } catch (e2) {
        cleanup();
  ...
  document
    .getElementById('gallery-download-btn')
    .addEventListener('click', async () => {
      try {
  ...
  if (_imgEl && _frame && _imgEl.tagName === 'IMG' && !_imgEl.complete) {
    ...
    ld.style.cssText =
      'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:color-mix(in srgb, var(--bg) 40%, transparent);z-index:4;pointer-events:none;border-radius:6px;';
    let _sp = null;
    try {
      _sp = spinnerModule.createWhirlpool(34);
      _sp.element.style.cssText = 'width:34px;height:34px;margin:0;';
      ld.appendChild(_sp.element);
    } catch (_) {}
  ...
  if (curIdx < 0 || curIdx >= _items.length - 1)
    nextBtn.classList.add('gallery-detail-nav-disabled');
  ...
  if (wrap) {
    ...
    wrap.addEventListener(
      ...
      (e) => {
        ...
        if (dx < 0 && curIdx < _items.length - 1)
          _openDetail(_items[curIdx + 1]);
  ...
  const _openInEditor = () => {
    try {
      ...
      if (uiModule)
        uiModule.showError(
          'Failed to open editor: ' + (e?.message || 'unknown'),
        );
    }
  };
  document
    .getElementById('gallery-edit-btn')
    ?.addEventListener('click', _openInEditor);
  document
    .getElementById('gallery-edit-direct-btn')
    ?.addEventListener('click', _openInEditor);
  ...
  const _rotate = async (angle) => {
    const stage =
      document.querySelector('.gallery-detail-img-stage') ||
      document.getElementById('gallery-detail-img')?.parentElement;
    let overlay = null;
    let spinner = null;
    if (stage) {
      ...
      overlay.style.cssText =
        'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;background:color-mix(in srgb, var(--bg) 55%, transparent);z-index:5;pointer-events:none;';
      try {
        spinner = spinnerModule.createWhirlpool(36);
        spinner.element.style.cssText = 'width:36px;height:36px;margin:0;';
        overlay.appendChild(spinner.element);
      } catch (_) {
        overlay.textContent = 'Rotating…';
      }
      if (getComputedStyle(stage).position === 'static')
        stage.style.position = 'relative';
      stage.appendChild(overlay);
    ...
    try {
      ...
      if (imgEl) {
        const newSrc =
          img.url + (img.url.includes('?') ? '&' : '?') + 't=' + Date.now();
  ...
  document
    .getElementById('gallery-rotate-btn')
    ?.addEventListener('click', () => _rotate(90));
  document
    .getElementById('gallery-rotate-ccw-btn')
    ?.addEventListener('click', () => _rotate(-90));

  // Set as album cover — only present if the photo is currently in an album.
  document
    .getElementById('gallery-set-cover-btn')
    ?.addEventListener('click', async () => {
      if (!img.album_id) return;
      try {
        const r = await fetch(
          `${API_BASE}/api/gallery/albums/${img.album_id}`,
          {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'same-origin',
  ...
  if (_tagInput) {
    ...
    const _addTags = async () => {
      ...
      const ok = await _patchImage(img.id, { tags: cleaned });
      if (!ok) {
        if (uiModule) uiModule.showError('Failed to save tags');
        return;
      }
      img.tags = cleaned;
      img.user_tags = cleaned;
    ...
    _tagInput.addEventListener('blur', () => {
      if (_tagInput.value.trim()) _addTags();
    });
  }

  document
    .getElementById('gallery-detail-album')
    .addEventListener('change', async (e) => {
      const albumId = e.target.value;
      const ok = await _patchImage(img.id, { album_id: albumId || '' });
      if (!ok) {
        uiModule.showError('Failed to update album');
        return;
      }
...
export function openGallery() {
  ...
  if (
    Modals.isRegistered('gallery-modal') &&
    Modals.isMinimized('gallery-modal')
  ) {
  ...
  if (editorTab) {
    editorTab.addEventListener('dblclick', (e) => {
      e.stopPropagation();
      const labelEl =
        editorTab.querySelector('.gallery-tab-label') || editorTab;
  ...
  modal.querySelectorAll('.gallery-tab').forEach((tab) => {
    tab.addEventListener('click', () => {
      ...
      if (albumsContainer)
        albumsContainer.style.display = target === 'albums' ? '' : 'none';
      if (editorContainer)
        editorContainer.style.display = target === 'editor' ? 'flex' : 'none';
      if (settingsContainer)
        settingsContainer.style.display = target === 'settings' ? '' : 'none';
      if (target === 'images') {
  ...
  document
    .getElementById('gallery-model-filter')
    .addEventListener('change', (e) => {
  ...
  const _maybeAutoLoad = () => {
    ...
    if (r.top <= window.innerHeight + 600) {
      // within 600px of the viewport bottom
      _loadingMore = true;
      _offset = _items.length;
      Promise.resolve(_fetchLibrary(true)).finally(() => {
        _loadingMore = false;
      });
    }
  };
      ...
      import('$lib/legacy/settings.js')
        .then((m) => {
          ...
          if (sm) {
            ...
            sm.style.setProperty(
              'z-index',
              String(Math.max(gz + 1, 10050)),
              'important',
            );
          }
        })
  ...
  if (tagAllBtn) {
    tagAllBtn.addEventListener('click', async () => {
      ...
      try {
        const r = await fetch(
          `${API_BASE}/api/gallery/ai-tag-batch?${params.toString()}`,
          {
            method: 'POST',
            credentials: 'same-origin',
          },
        ...
        uiModule.showError('Failed to fetch tag queue');
        return;
      }
      if (
        !listRes.ok ||
        !Array.isArray(listRes.image_ids) ||
        listRes.image_ids.length === 0
      ) {
      ...
      if (uiModule)
        uiModule.showToast(
          `Tagged ${done - failed} photo${(done - failed) !== 1 ? 's' : ''}`,
        );
    });
  }
  ...
  if (moreBtn && moreMenu) {
    ...
    const _setMore = (show) => {
      ...
      moreMenu.style.display = show ? 'block' : 'none';
    };
    _setMore(false);
  ...
  if (clearAiTagsBtn) {
    clearAiTagsBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        uiModule.showToast(
          `Cleared AI tags on ${d.cleared} photo${d.cleared === 1 ? '' : 's'}`,
        );
        await _fetchLibrary(false);
      } catch (e) {
  ...
  const _selectedDots = () => [
    ...document.querySelectorAll('.gallery-select-dot.selected'),
  ];
  const _selectedIds = () =>
    _selectedDots()
      .map((d) => d.closest('.gallery-card')?.dataset.id)
      .filter(Boolean);
  ...
  function _setSelectMode(on) {
    ...
    document.querySelectorAll('.gallery-select-dot').forEach((d) => {
      ...
      d.closest('.gallery-card')?.classList.toggle(
    ...
    if (!on)
      document
        .querySelectorAll('.gallery-bulk-menu')
        .forEach((m) => m.remove());
    _updateBulkCount();
  }
  ...
  document
    .getElementById('gallery-bulk-cancel')
    ?.addEventListener('click', () => _exitSelectMode());

  // Select all / none.
  document
    .getElementById('gallery-bulk-select-all')
    ?.addEventListener('change', (e) => {
      const on = e.target.checked;
      document
        .querySelectorAll('.gallery-select-dot')
        .forEach((d) => d.classList.toggle('selected', on));
      _updateBulkCount();
    });
  ...
  if ('ontouchstart' in window) {
    ...
    gridEl.addEventListener(
      ...
      (e) => {
        ...
        const t = e.touches[0];
        if (
          Math.abs(t.clientX - lpStartX) > MOVE_CANCEL_PX ||
  ...
  ['dragenter', 'dragover'].forEach((ev) => {
    imagesContainer.addEventListener(ev, (e) => {
      e.preventDefault();
      e.stopPropagation();
      grid.classList.add('gallery-dragover');
    });
  });
  ['dragleave', 'drop'].forEach((ev) => {
    imagesContainer.addEventListener(ev, (e) => {
      e.preventDefault();
      e.stopPropagation();
      grid.classList.remove('gallery-dragover');
    });
  });
  imagesContainer.addEventListener('drop', (e) => {
    _handleGalleryDrop(e).catch((err) =>
  ...
  function _showGalleryBulkMenu(anchor) {
    ...
    const _favIco =
      '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 21s-6.7-4.35-9.33-8.04C.9 10.3 1.4 6.9 4.1 5.6c1.9-.9 4 .03 5 1.7 1-1.67 3.1-2.6 5-1.7 2.7 1.3 3.2 4.7 1.43 7.36C18.7 16.65 12 21 12 21z"/></svg>';
    const _tagIco =
    ...
    const items = [
      {
        label: 'Favorite',
        icon: _favIco,
        action: () => _bulkFavorite(_selectedIds()),
      },
      {
        label: 'Add tag…',
        icon: _tagIco,
        action: () => _bulkTag(_selectedIds()),
      },
      {
        label: 'Download',
        icon: _dlIco,
        action: () => _bulkDownload(_selectedIds()),
      },
      {
        label: 'Delete',
        icon: _delIco,
        danger: true,
        action: () => _bulkDelete(_selectedIds()),
      },
      { separator: true },
    ...
    for (const a of items) {
      ...
      it.className =
        'dropdown-item-compact' + (a.danger ? ' dropdown-item-danger' : '');
  ...
  _bulkActionsBtn?.addEventListener('click', (e) => {
    ...
    if (!_selectedIds().length) {
      uiModule.showToast('Select photos first');
      return;
    }
  ...
  async function _bulkDelete(ids) {
    ...
    const deleted = [],
      failed = [];
    for (const id of ids) {
      const ok = await _deleteImage(id);
      (ok ? deleted : failed).push(id);
    }
    if (failed.length)
      uiModule.showError(
        `Failed to delete ${failed.length} of ${ids.length} photos`,
      );
    ...
    if (uiModule)
      uiModule.showToast(
        `${deleted.length} photo${deleted.length > 1 ? 's' : ''} deleted`,
      );
    ...
    if (
      _items.length === 0 &&
      (_activeTags.length || _activeAlbum || _favoritesOnly)
    ) {
      _activeTags = [];
      _activeAlbum = null;
      _favoritesOnly = false;
  ...
  async function _bulkDownload(ids) {
    ...
    if (ids.length > 5) {
      try {
        ...
        _exitSelectMode();
        if (uiModule)
          uiModule.showToast(`Downloaded ${ids.length} photos (zip)`);
      } catch (e) {
    ...
    _exitSelectMode();
    if (uiModule)
      uiModule.showToast(`Downloading ${n} photo${n === 1 ? '' : 's'}`);
  }
  ...
  async function _bulkTag(ids) {
    ...
    for (const id of ids) {
      const it = _items.find((i) => i.id === id);
      const existing = (it?.user_tags || it?.tags || '')
        .split(',')
        .map((t) => t.trim())
        .filter(Boolean);
    ...
    _exitSelectMode();
    if (uiModule)
      uiModule.showToast(`Tagged ${n} photo${n === 1 ? '' : 's'} “${tag}”`);
  }
  ...
  _escHandler = (e) => {
    ...
    const btn = document.getElementById(
      e.key === 'ArrowLeft' ? 'gallery-detail-prev' : 'gallery-detail-next',
    );
...
function _doCloseGallery() {
  const editorMounted = !!document.querySelector(
    '#gallery-editor-container .gallery-editor',
  );
  if (
    (window.__galleryEditLive || isEditorOpen() || editorMounted) &&
    !window.__galleryAllowCloseEditor
  ) {
