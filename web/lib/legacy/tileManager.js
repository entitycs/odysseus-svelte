...
function _viewportSafeRect() {
  ...
  const rail =
    document.querySelector('.icon-rail') ||
    document.querySelector('#icon-rail');
  let leftEdge = 0;
...
function _zoneForPointer(x, y) {
  ...
  if (y <= safe.top + TOP_FULL_STRIP_PX) {
    return {
      name: 'maximize',
      rect: { left: safe.left, top: safe.top, width: W, height: H },
    };
  }
  ...
  if (y <= safe.top + EDGE_THRESHOLD_PX)
    return {
      name: 'top-half',
      rect: { left: safe.left, top: safe.top, width: W, height: H / 2 },
    };
  if (x <= safe.left + EDGE_THRESHOLD_PX)
    return {
      name: 'left-half',
      rect: { left: safe.left, top: safe.top, width: W / 2, height: H },
    };
...
function _zoneForContent(content, x, y) {
  const modal =
    content && content.closest && content.closest('.modal, .research-overlay');
  ...
  if (modal && modal.id === 'settings-modal' && zone.name !== 'right-half')
    return null;
  if (
    modal &&
    (modal.id === 'cookbook-modal' || modal.id === 'theme-modal') &&
...
function _clearEdgeDockResidue(modal, content) {
  const hadDockState = !!(
    ...
    (content &&
      (content._preDockSnapshot || content._dockSide || content._dockSuspended))
  );
  if (modal) {
  ...
  if (hadDockState) {
    ['right', 'bottom', 'max-width', 'border-radius'].forEach((p) =>
...
function _applySnap(content, rect, zoneName) {
  ...
  const _modal =
    content.closest && content.closest('.modal, .research-overlay');
...
function _reclampAll(animate = false) {
  document
    .querySelectorAll(
      '.modal-content[data-_tile-zone], .research-pane[data-_tile-zone]',
    )
    .forEach((c) => {
      ...
      switch (name) {
        ...
        case 'maximize':
          ...
          break;
        case 'top-half':
          r = { left: safe.left, top: safe.top, width: W, height: H / 2 };
          break;
        case 'left-half':
          r = { left: safe.left, top: safe.top, width: W / 2, height: H };
          break;
        case 'right-half':
          r = {
            left: safe.left + W / 2,
        ...
        case 'bottom-half':
          ...
          break;
        case 'top-left':
          r = { left: safe.left, top: safe.top, width: W / 2, height: H / 2 };
          break;
        case 'top-right':
          r = {
            left: safe.left + W / 2,
...
export function snapModalToZone(modal, zone) {
  ...
  const content = modal.querySelector
    ? modal.querySelector('.modal-content, .research-pane') || modal
    : modal;
  if (!content) return;
