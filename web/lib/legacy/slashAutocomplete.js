...
import { COMMANDS, LEGACY_ALIASES } from '$lib/legacy/slashCommands.js';
...
async function _loadSkillEntries() {
  try {
    const res = await fetch('/api/skills/slash-catalog', {
...
function _exactCommandGroupItems(all, query) {
  ...
  const children = all.filter((entry) =>
    entry.token.toLowerCase().startsWith(prefix),
  );
...
function _render(popup, items, selectedIdx, query) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const usage =
      it.usage && it.usage !== it.token
        ? ` <span class="slash-ac-usage">${_esc(it.usage)}</span>`
        : '';
    html +=
      `<div class="slash-ac-row${sel}" role="option" data-idx="${i}" data-token="${_esc(it.token)}">` +
...
export function initSlashAutocomplete(textarea) {
  ...
  textarea.addEventListener('focus', () => {
    if (textarea.value.startsWith('/')) refresh();
  });
  ...
  textarea.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      const exactHit = items.find(
        (it) => it.token === v || it.aliases.includes(v),
      );
  ...
  window.addEventListener('resize', () => {
    if (visible) _position(popup, textarea);
  });
