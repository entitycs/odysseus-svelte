...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      uiData.ui_event === 'switch_model'
    ) {
      var modelDisplay = document.querySelector(
        '.current-model-name, #current-model',
      );
      ...
      if (tm && tm.THEMES && tm.applyColors && tm.save) {
        ...
        var colors =
          tm.THEMES[themeName] || customThemes[themeName] || uiData.colors;
        if (colors) {
      ...
      if (tm2 && tm2.applyColors && tm2.save) {
        ...
        if (colors2) {
          ...
          var opts = {};
          if (bg) {
            if (bg.pattern && tm2.applyBgPattern) {
              tm2.applyBgPattern(bg.pattern);
              opts.bgPattern = bg.pattern;
            }
            if (bg.effectColor && tm2.applyBgEffectColor) {
              tm2.applyBgEffectColor(bg.effectColor);
              opts.bgEffectColor = bg.effectColor;
            }
            if (bg.effectIntensity != null && tm2.applyBgEffectIntensity) {
              tm2.applyBgEffectIntensity(bg.effectIntensity);
              opts.bgEffectIntensity = bg.effectIntensity;
            }
            if (bg.effectSize != null && tm2.applyBgEffectSize) {
              tm2.applyBgEffectSize(bg.effectSize);
              opts.bgEffectSize = bg.effectSize;
            }
            if (bg.frosted != null && tm2.applyFrostedGlass) {
              tm2.applyFrostedGlass(bg.frosted);
              opts.frosted = bg.frosted;
            }
          }
          if (tm2.saveCustomTheme)
            tm2.saveCustomTheme(
              name,
              colors2,
      ...
      document.querySelectorAll('.odysseus-highlight').forEach(function (e) {
        e.classList.remove('odysseus-highlight');
      });
      document.querySelectorAll('.odysseus-hl-label').forEach(function (e) {
        e.remove();
      });
      ...
      uiData.ui_event === 'clear_highlight'
    ) {
      document.querySelectorAll('.odysseus-highlight').forEach(function (e) {
        e.classList.remove('odysseus-highlight');
      });
      ...
      uiEvent === 'research_started' ||
      uiData.ui_event === 'research_started'
    ) {
      ...
      if (rsid) {
        import('$lib/legacy/research/jobs.js')
          .then(function (mod) {
            var fn =
              mod.adoptSession || (mod.default && mod.default.adoptSession);
            if (fn) fn(rsid);
          })
          .catch(function () {});
      ...
      if (panel === 'documents') {
        import('$lib/legacy/documentLibrary.js')
          .then(function (mod) {
            var fn =
              mod.openLibrary || (mod.default && mod.default.openLibrary);
            if (fn) fn();
          })
          .catch(function () {});
        ...
        import('$lib/legacy/gallery.js')
          .then(function (mod) {
            var fn =
              mod.openGallery || (mod.default && mod.default.openGallery);
            if (fn) fn();
          })
          .catch(function () {});
      } else if (panel === 'email') {
        import('$lib/legacy/emailLibrary.js')
          .then(function (mod) {
            var fn =
        ...
        import('$lib/legacy/notes.js')
          .then(function (mod) {
            var fn =
              mod.openPanel ||
              mod.openNotes ||
              (mod.default && (mod.default.openPanel || mod.default.openNotes));
            if (fn) fn();
          })
          .catch(function () {});
      } else if (
      ...
      uiData.ui_event === 'open_email_reply'
    ) {
      try {
        var activeCtx =
          documentModule && documentModule.getActiveEmailComposerContext
            ? documentModule.getActiveEmailComposerContext()
            : null;
        var sameActiveDraft =
          activeCtx &&
          String(activeCtx.sourceUid || '') === String(uiData.uid || '') &&
          String(activeCtx.sourceFolder || 'INBOX') ===
            String(uiData.folder || 'INBOX');
        var existingDocId =
          sameActiveDraft && activeCtx.docId
            ? activeCtx.docId
        ...
        if (existingDocId && documentModule.replaceEmailReplyBody) {
          if (documentModule.loadDocument)
            documentModule.loadDocument(existingDocId);
          documentModule.replaceEmailReplyBody(
            existingDocId,
            uiData.body || '',
            { force: true },
          );
          if (uiModule && uiModule.showToast)
            uiModule.showToast('Wrote reply into the open email');
          return;
        }
      } catch (e) {
        console.warn('open_email_reply existing draft update failed:', e);
      }
      import('./emailInbox.js')
        .then(function (mod) {
          var fn =
            mod.openReplyDraft || (mod.default && mod.default.openReplyDraft);
          if (fn)
            fn(
              uiData.uid,
              uiData.folder || 'INBOX',
...
export function notifyStreamComplete(sessionId, query) {
  ...
  var isOtherSession =
    sessionModule && sessionModule.getCurrentSessionId() !== sessionId;
  if (!isHidden && !isOtherSession) return;
  if (!('Notification' in window) || Notification.permission !== 'granted')
    return;
  var body = query
    ? 'Response to "' + query.substring(0, 60) + '" is ready'
    : 'Your chat response has completed';
...
export function insertStreamDoneToast(sessionId, query) {
  ...
  div.innerHTML =
    ...
    '<span>Response ready in <strong>' +
    (name || 'session').replace(/</g, '&lt;') +
    '</strong>' +
...
export function notifyResearchComplete(sessionId, query) {
  ...
  if (!('Notification' in window) || Notification.permission !== 'granted')
    return;
  var body = query
    ? 'Research on "' + query.substring(0, 60) + '" is ready'
    : 'Your deep research has completed';
