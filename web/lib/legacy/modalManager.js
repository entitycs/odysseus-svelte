...
import { dismissOrRemove } from '$lib/legacy/escMenuStack.js';
import {
  applyEdgeDock,
  clearRightDock,
  resumeDock,
  suspendDock,
} from '$lib/legacy/modalSnap.js';
import {
  clearPreview,
  previewZoneAt,
  snapModalToZone,
} from '$lib/legacy/tileManager.js';
import { nextToolWindowZ } from '$lib/legacy/toolWindowZOrder.js';
...
export function init() {
  ...
  window.addEventListener('modal-dismissed', (e) => {
    ...
    if (!id) return;
    if (
      !_SWIPE_DOWN_MINIMIZES.has(id) &&
      !_SWIPE_DOWN_MINIMIZES_PREFIX.some((p) => id.startsWith(p))
    )
      return;
    ...
    if (modal) {
      const isEmailModal =
        id === 'email-lib-modal' || id.startsWith('email-reader-');
      if (
        ...
        try {
          ...
          console.warn('suspendDock on dismissed failed', err);
        }
      }
...
function _rememberDock(id, side) {
  if (!id || !side) return;
  try {
    localStorage.setItem(_rememberedDockKey(id), side);
  } catch (_) {}
}
function _forgetDock(id) {
  if (!id) return;
  try {
    localStorage.removeItem(_rememberedDockKey(id));
  } catch (_) {}
}
...
function _applyRememberedDock(id) {
  ...
  try {
    ...
    console.warn('apply remembered dock failed', e);
  }
}
...
function _emitModalOpened(id, modal) {
  try {
    window.dispatchEvent(
      new CustomEvent('odysseus:modal-opened', {
        detail: { id, modal },
      }),
    );
  } catch (_) {}
...
function _captureRestoreHeight(modal, state) {
  ...
  const minHeight =
    modal.id === 'email-lib-modal' && window.innerWidth > 768
...
function _applyRestoreHeight(modal, state) {
  ...
  const minHeight =
    modal.id === 'email-lib-modal' && window.innerWidth > 768
      ? Math.min(560, maxHeight)
      : 0;
  const height = Number.isFinite(requested)
    ? Math.max(minHeight, Math.min(requested, maxHeight))
    : null;
...
const _LABELS = {
  'cookbook-modal': {
    label: 'Cookbook',
    icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 7v14"/><path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"/></svg>',
  },
  'calendar-modal': {
    label: 'Calendar',
  ...
  'gallery-modal': {
    label: 'Gallery',
    icon: 'M3 3h18v18H3zM8.5 8.5l3 3M21 15l-5-5L5 21',
  },
  'tasks-modal': {
    label: 'Tasks',
  ...
  'notes-panel': {
    label: 'Notes',
    icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 3h10l4 4v14H5z"/><path d="M15 3v5h5"/><path d="M8 17.5 15.5 10l2.5 2.5L10.5 20H8z"/></svg>',
  },
  'email-lib-modal': {
    label: 'Email',
...
function _renderDock() {
  ...
  const persistentIds = isMobile
    ? [..._state.entries()]
        .filter(([id, _]) => _chipPositions.has(id))
        .map(([id]) => id)
    : [];
  ...
  document
    .querySelectorAll('body > .minimized-dock-chip')
    .forEach((c) => c.remove());
  ...
  for (const id of renderIds) {
    ...
    chip.addEventListener('click', (e) => {
      if (chip._wasDragging) {
        chip._wasDragging = false;
        return;
      }
...
function _initChainPhysics(grabbedChip, dock, startX, startY) {
  ...
  const chipEls = _dockOrder
    .map((id) =>
      document.querySelector(`.minimized-dock-chip[data-modal-id="${id}"]`),
    )
    .filter(Boolean);
  ...
  const dockChips =
    chipEls.length >= 2
      ? chipEls
      : [...dock.querySelectorAll('.minimized-dock-chip')];
...
function _wireChipDrag(chip, dock) {
    ...
    if (onTouch) {
      ...
      if (totalChips >= 2) {
        ...
        longPressTimer = setTimeout(() => {
          ...
          _chipPositions.set(chip.dataset.modalId, {
            left: chipStartLeft,
            top: chipStartTop,
          });
          _saveDockState();
  ...
  const onPointerMove = (e) => {
    ...
    const DRAG_THRESHOLD =
      e.pointerType === 'touch' || window.innerWidth <= 768 ? 14 : 5;
    ...
    if (dragMode === 'free') {
      ...
      chip.style.setProperty(
        'transform',
        `translate(${tx}px, ${ty}px) scale(${inZone ? 1.12 : 1.05})`,
        'important',
      );
    ...
    if (dragMode === 'reorder') {
      ...
      const siblings = [
        ...dock.querySelectorAll('.minimized-dock-chip:not(.dragging)'),
      ];
      ...
      let newLeft = Math.max(
        8,
        Math.min(window.innerWidth - dock.offsetWidth - 8, dockStartLeft + dx),
      );
      let newTop = Math.max(
        8,
        Math.min(window.innerHeight - dock.offsetHeight - 8, dockStartTop + dy),
      );
  ...
  const onPointerUp = () => {
    ...
    if (chipSnapZone) {
      ...
      if (modal)
        requestAnimationFrame(() =>
          requestAnimationFrame(() => snapModalToZone(modal, zone)),
        );
      return;
    }
    if (dragMode === 'chain' && chainState) {
      ...
      if (state.overTrash && dragging) {
        ...
        for (const l of state.links) {
          if (tz) {
            ...
            l.chip.style.transition =
              'transform 0.32s cubic-bezier(0.45, 0, 0.25, 1), opacity 0.3s ease-in, left 0.32s cubic-bezier(0.45, 0, 0.25, 1), top 0.32s cubic-bezier(0.45, 0, 0.25, 1)';
        ...
        for (const i of state.order) {
          ...
          const clampedLeft = Math.max(
            4,
            Math.min(window.innerWidth - l.width - 4, l.x),
          );
          const clampedTop = Math.max(
            4,
            Math.min(window.innerHeight - l.height - 4, l.y),
          );
          _chipPositions.set(l.chip.dataset.modalId, {
            left: clampedLeft,
            top: clampedTop,
          });
        }
    ...
    if (dragMode === 'free') {
      ...
      if (overTrash) {
        ...
        chip.style.setProperty(
          'transition',
          'transform 0.32s cubic-bezier(0.45, 0, 0.25, 1), opacity 0.3s ease-in',
          'important',
        );
        // Whirlpool: spin + shrink as the chip swirls into the X.
        chip.style.setProperty(
          'transform',
          `${cur} scale(0.15) rotate(720deg)`,
          'important',
        );
        ...
        if (nearest && nearestDist < myW + SNAP) {
          ...
          if (Math.abs(dx2) >= Math.abs(dy2)) {
            // Horizontal snap
            dropLeft =
              dx2 >= 0 ? nearest.right + gap : nearest.left - myW - gap;
            dropTop = nearest.top;
          } else {
            // Vertical snap
        ...
        const clampedLeft = Math.max(
          4,
          Math.min(window.innerWidth - myW - 4, dropLeft),
        );
        const clampedTop = Math.max(
          4,
          Math.min(window.innerHeight - myH - 4, dropTop),
        );
...
export function register(
  id,
  { restoreFn, closeFn, railBtnId, sidebarBtnId, label, icon } = {},
) {
  // railBtnId can be a single id or an array; we accept both rail and sidebar separately too.
  const btnIds = [];
  if (railBtnId)
    btnIds.push(...(Array.isArray(railBtnId) ? railBtnId : [railBtnId]));
  if (sidebarBtnId)
    btnIds.push(
      ...(Array.isArray(sidebarBtnId) ? sidebarBtnId : [sidebarBtnId]),
    );
  _state.set(id, {
    restoreFn: restoreFn || (() => {}),
    closeFn: closeFn || (() => {}),
  ...
  if (_modalEl && !_modalEl._mmAutoStackObs) {
    const _isVisible = () =>
      !_modalEl.classList.contains('hidden') &&
  ...
  if (_getRememberedDock(id)) {
    requestAnimationFrame(() =>
      requestAnimationFrame(() => _applyRememberedDock(id)),
    );
  }
}
...
export function isMinimized(id) {
  return _state.get(id)?.isMinimized === true;
}
  ...
  if (modal) {
    ...
    if (
      ...
      try {
        ...
        console.warn('suspendDock on minimize failed', e);
      }
    }
...
export function restore(id) {
  ...
  if (modal) {
    ...
    try {
      ...
      console.warn('resumeDock on restore failed', e);
    }
...
export function close(id) {
  ...
  const contentBeforeClose =
    modalBeforeClose?.querySelector?.('.modal-content');
  const suspendedDockSide =
    contentBeforeClose?._dockSuspended ||
    (modalBeforeClose?.classList?.contains('modal-left-docked')
      ? 'left'
      : modalBeforeClose?.classList?.contains('modal-right-docked')
        ? 'right'
        : null);
  ...
  if (modal) {
    ...
    if (
      ...
      try {
        clearRightDock(modal);
      } catch (e) {
        console.warn('clearRightDock on close failed', e);
      }
    }
    modal.classList.add('hidden');
  ...
  register(id, {
    ...
    closeFn: () => {
      ...
      const closeBtn = m.querySelector(
        '.close-btn, .modal-close, [data-close]',
      );
      if (closeBtn) {
        closeBtn.click();
      } else {
...
// const _scanTimer = setInterval(_scanAndWire, 1000); // unused
