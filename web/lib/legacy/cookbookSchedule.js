...
export function init() {
  try {
    function _safe(fn) {
      return function () {
        try {
          ...
          try {
            console.warn('[cookbookSchedule]', e);
          } catch (_) {}
        }
      };
    }
    ...
    function readPanelConfig(arrowBtn) {
      const item =
        arrowBtn.closest('.memory-item') ||
        arrowBtn.closest('.hwfit-cached-item');
    ...
    function wireForm(form, cfg) {
      ...
      form
        .querySelector('.hwfit-sched-cancel')
        .addEventListener('click', () => form.remove());
      form.querySelector('.hwfit-sched-save').addEventListener(
        'click',
        _safe(async () => {
          const startTime = form.querySelector('.hwfit-sched-start').value;
          const endTime = form.querySelector('.hwfit-sched-end').value;
          const days = Array.from(
            form.querySelectorAll('.hwfit-sched-day-chip.is-on'),
          ).map((c) => c.dataset.day);
          const mirrorToCalendar = !!form.querySelector(
            '.hwfit-sched-calendar-mirror',
          )?.checked;
          ...
          try {
            ...
            if (!r.ok || data.error) {
              ...
              toast(
                `Schedule save failed: ${data.error || data.detail || r.status}`,
              );
              return;
            }
            if (mirrorToCalendar) {
              ...
              try {
                const calsRes = await fetch('/api/calendar/calendars', {
                  credentials: 'same-origin',
                });
                const calsBody = calsRes.ok ? await calsRes.json() : {};
                let cookbookCal = (calsBody.calendars || []).find(
                  (c) => (c.name || '').toLowerCase() === 'cookbook',
                );
                if (!cookbookCal) {
                  const mk = await fetch(
                    '/api/calendar/calendars?name=Cookbook&color=%233b82f6',
                    {
                      method: 'POST',
                      credentials: 'same-origin',
                    },
                ...
                const evBody = {
                  ...
                  all_day: false,
                  description:
                    `Auto-mirrored from Cookbook schedule task ${data.id || ''}.\n` +
    ...
    document.addEventListener(
      ...
      _safe((e) => {
        const arrow =
          e.target.closest && e.target.closest('.hwfit-serve-schedule-arrow');
        if (!arrow) return;
        e.preventDefault();
        e.stopPropagation();
        openForm(arrow);
    ...
    try {
      console.warn('[cookbookSchedule] top-level error:', e);
    } catch (_) {}
  }
}
