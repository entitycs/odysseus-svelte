...
export function init() {
  _initSplitSeamIndicator();
  _initEdgeDockResizeHandles();
}

function _positionEdgeDockResizeHandles() {
  try {
    _edgeDockHandlePositioner && _edgeDockHandlePositioner();
  } catch (_) {}
}
...
function _hasAnyOtherDockedWindow(owner) {
  return (
    _hasOtherDockedWindow('left', owner) ||
    _hasOtherDockedWindow('right', owner)
  );
}
...
export function clearDockSide(side, owner = null) {
  ...
  document.body.classList.remove(
    side === 'left' ? 'left-dock-active' : 'right-dock-active',
  );
  document.documentElement.style.removeProperty(
    side === 'left' ? '--left-dock-w' : '--right-dock-w',
  );
  if (side === 'left') {
    try {
...
function _resolveRightDockWidth(modal, content) {
  return _clampRightDockWidth(
    content?._userDockWidth ||
      _storedDockWidth(modal, content, 'right') ||
      _defaultDockWidth(),
  );
}

function _resolveLeftDockWidth(content, left = _leftNavRight()) {
  return _clampLeftDockWidth(
    content?._userDockWidth ||
      _storedDockWidth(content?._dockOwner, content, 'left') ||
...
function _isEmailDockOwner(owner) {
  ...
  return (
    id === 'email-lib-modal' ||
    id.startsWith('email-reader-') ||
    owner?.classList?.contains('email-window-modal')
  );
}

function _showSnapHint(on, side = 'right') {
  const cls =
    side === 'left' ? 'modal-snap-hint-left' : 'modal-snap-hint-right';
...
function _shouldAutoCollapseSidebar(dockW) {
  ...
  const rl =
    rail && window.getComputedStyle(rail).display !== 'none'
      ? rail.getBoundingClientRect().width
      : 0;
  const remaining =
    window.innerWidth - sb - rl - _activeDockWidth('left') - dockW;
...
function _saveEmailDocSplitWidth(width) {
  try {
    localStorage.setItem(EMAIL_DOC_SPLIT_WIDTH_KEY, String(Math.round(width)));
  } catch (_) {}
}
...
function _disconnectLeftDockObservers(content) {
  ...
  try {
    obs.navObs && obs.navObs.disconnect();
  } catch (_) {}
  try {
    obs.bodyObs && obs.bodyObs.disconnect();
  } catch (_) {}
  try {
    obs.disconnectDocObs && obs.disconnectDocObs();
  } catch (_) {}
  try {
    window.removeEventListener('resize', obs.reanchor);
  } catch (_) {}
...
function _applyEmailDocSplitGeometry(left, emailWidth) {
  ...
  document.documentElement.style.setProperty(
    '--email-doc-split-left-x',
    `${left}px`,
  );
  document.documentElement.style.setProperty(
    '--email-doc-split-email-w',
    `${emailWidth}px`,
  );
  document.documentElement.style.setProperty(
    '--email-doc-split-right-x',
    `${x}px`,
  );
...
function _resolveEmailDocSplitWidth(content, left) {
  ...
  const requested =
    content?._emailDocSplitUserW || _storedEmailDocSplitWidth() || fallback;
...
function _anchorLeftDock(content) {
  ...
  const docOpen =
    ...
    _isEmailDockOwner(content._dockOwner);
  if (docOpen) {
...
export function applyEdgeDock(modal, side = 'right', dockClass) {
  if (!dockClass)
    dockClass = side === 'left' ? 'modal-left-docked' : 'modal-right-docked';
  ...
  if (side === 'left') {
    ...
    if (!content._leftDockNavObs && typeof MutationObserver !== 'undefined') {
      ...
      content._leftDockNavObs = {
        ...
        disconnectDocObs: () => {
          try {
            docResizeObs && docResizeObs.disconnect();
          } catch (_) {}
          try {
            chatResizeObs && chatResizeObs.disconnect();
          } catch (_) {}
        },
      };
    }
...
function _onDockedModalGone(modal, dockClass) {
  ...
  if (watcher) {
    try {
      watcher.obs && watcher.obs.disconnect();
    } catch (_) {}
    try {
      watcher.parentObs && watcher.parentObs.disconnect();
    } catch (_) {}
  ...
  if (
    _c?._preDockSnapshot?.collapsedSidebar &&
    !_hasAnyOtherDockedWindow(modal)
  ) {
...
export function clearRightDock(modal, cx, cy, dockClass) {
  ...
  if (snap && snap.collapsedSidebar && !_hasAnyOtherDockedWindow(modal))
    _expandSidebarFromRail();
  ...
  const refH =
    (r && r.height) || content.offsetHeight || window.innerHeight * 0.7;
  const targetLeft =
    typeof cx === 'number'
      ...
      : sty.left ||
        (r
          ? r.left + 'px'
          : Math.max(8, (window.innerWidth - refW) / 2) + 'px');
  const targetTop =
    typeof cy === 'number'
      ? Math.max(8, cy - 20)
      : sty.top ||
        (r
          ? r.top + 'px'
          : Math.max(8, (window.innerHeight - refH) / 3) + 'px');
  content.style.left =
    typeof targetLeft === 'number' ? targetLeft + 'px' : targetLeft;
  content.style.top =
    typeof targetTop === 'number' ? targetTop + 'px' : targetTop;
...
export function suspendDock(modal) {
  ...
  if (modal._dockCloseWatcher) {
    try {
      modal._dockCloseWatcher.obs && modal._dockCloseWatcher.obs.disconnect();
    } catch (_) {}
    try {
      modal._dockCloseWatcher.parentObs &&
        modal._dockCloseWatcher.parentObs.disconnect();
    } catch (_) {}
    delete modal._dockCloseWatcher;
  }
  ...
  if (
    content._preDockSnapshot?.collapsedSidebar &&
    !_hasAnyOtherDockedWindow(modal)
  ) {
    _expandSidebarFromRail();
  }
...
export function makeRightDockController(
  modal,
  dockClass = 'modal-right-docked',
) {
...
function _initEdgeDockResizeHandles() {
  ...
  if (!document.body) {
    document.addEventListener('DOMContentLoaded', _initEdgeDockResizeHandles, {
      once: true,
    });
    return;
  }
  ...
  const _hasVisibleFloatingModal = (owner) => {
    const all = Array.from(
      document.querySelectorAll('.modal:not(.hidden):not(.modal-minimized)'),
    );
  ...
  new MutationObserver(_positionEdgeDockResizeHandles).observe(document.body, {
    attributes: true,
    attributeFilter: ['class'],
  });
  new MutationObserver(_positionEdgeDockResizeHandles).observe(
...
function _initSplitSeamIndicator() {
  ...
  stripe.addEventListener('pointerdown', (e) => {
    ...
    const onUp = (ev) => {
      try {
        stripe.releasePointerCapture?.(e.pointerId);
      } catch (_) {}
      document.removeEventListener('pointermove', onMove, true);
      document.removeEventListener('pointerup', onUp, true);
      document.removeEventListener('pointercancel', onUp, true);
  ...
  new MutationObserver(_position).observe(document.body, {
    attributes: true,
    attributeFilter: ['class'],
  });
  new MutationObserver(_position).observe(document.documentElement, {
    attributes: true,
