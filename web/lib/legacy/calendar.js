...
import {
  ...
  WEEKDAYS,
  WEEKDAYS_SUN,
} from '$lib/legacy/calendar/utils.js';
...
let WEEK_HOUR_PX = WK_PX_DEFAULT;
...
export async function init() {
  ...
  _filtersCollapsed = localStorage.getItem('cal-filters-collapsed') === '1';
  // Week-start preference: 'mon' (default, Mon=first col) or 'sun' (Sun=first col).
  _weekStartSun = localStorage.getItem('cal-week-start') === 'sun';
...
async function _renderMonth() {
  ...
  for (const wd of _weekStartSun ? WEEKDAYS_SUN : WEEKDAYS)
    h += `<div class="cal-weekday">${wd}</div>`;
  h += '</div>';
  ...
  if (_qaInput && !_qaInput._wired) {
    ...
    const _submitQA = async () => {
      ...
      if (_qaStatus) {
        ...
        try {
          const sp = (await import('$lib/legacy/spinner.js')).default;
...
async function _showCalSettings() {
  ...
  const _activeStyle =
    'color-mix(in srgb, var(--accent,var(--red)) 18%, var(--panel))';
  const _inactiveStyle = 'var(--panel)';
  const _applyWeekStartActive = () => {
    if (_monBtn)
      _monBtn.style.background = _weekStartSun ? _inactiveStyle : _activeStyle;
    if (_sunBtn)
      _sunBtn.style.background = _weekStartSun ? _activeStyle : _inactiveStyle;
  };
  ...
  overlay.querySelectorAll('.cal-settings-row').forEach((row) => {
    ...
    const save = () => {
      ...
      saveTimer = setTimeout(async () => {
        await fetch(
          `${API_BASE}/api/calendar/calendars/${id}?name=${encodeURIComponent(nameInput.value)}&color=${encodeURIComponent(colorInput.value)}`,
          { method: 'PUT' },
        );
        ...
        <div class="note-color-picker" id="cal-f-colors">
          ${CAL_COLORS.map((c) => {
            ...
            if (isCustom) {
              ...
              bg = url
                ? `center/cover no-repeat url('${_cssUrlEscape(url)}')`
                : _CAL_CUSTOM_GRADIENT;
            } else {
              bg = c.hex || 'var(--border)';
            }
...
export { closeCalendar, isCalendarOpen, openCalendar, openCalendarTo };
