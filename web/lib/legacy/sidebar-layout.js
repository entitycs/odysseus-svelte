...
export function initSidebarLayout(Storage, opts) {
  ...
  window._odyOpenSidebar = function (side) {
    ...
    if (
      ...
      cc &&
      cc.classList.contains('compare-active')
    )
      return;
    ...
    if (side === 'left' || side === 'right') {
      ...
      if (sidebar.classList.contains('right-side') !== wantRight) {
        ...
        try {
          Storage.set(Storage.KEYS.SIDEBAR_SIDE, side);
        } catch (_) {}
    ...
    if (window.innerWidth < 768 && iconRail) {
      iconRail.classList.remove('mobile-mini');
      iconRail.style.cssText = '';
    }
    sidebar.classList.remove('hidden');
  ...
  if (iconRail) {
    iconRail.addEventListener('click', (e) => {
      ...
      if (
        !btn ||
        btn.id === 'rail-new-session' ||
        btn.id === 'rail-delete-session' ||
        btn.id === 'rail-search-btn' ||
        btn.id === 'rail-settings' ||
        btn.id === 'rail-admin'
      )
        return;
  ...
  function checkSidebarAutoCollapse() {
    ...
    const hasTileSnapped = document.querySelector(
      '.modal-content[data-_tile-zone], .research-pane[data-_tile-zone]',
    );
    const chatTooNarrow =
      chatContainer &&
      chatContainer.offsetWidth < MIN_CHAT_WIDTH &&
      !isHidden &&
      !hasTileSnapped;

    if (
      (window.innerWidth < AUTO_COLLAPSE_WIDTH || chatTooNarrow) &&
      !isHidden
    ) {
      sidebar.classList.add('hidden');
  ...
  mobileBackdrop.addEventListener('click', (e) => {
    ...
    const openDD = document.querySelector(
      '.session-dropdown-menu[style*="display: block"], .session-dropdown-menu[style*="display:block"]',
    );
    const openSub = document.querySelector(
      '.session-folder-submenu[style*="display: block"], .session-folder-submenu[style*="display:block"]',
    );
    if (openDD || openSub) {
  ...
  _syncRailSideFn = function () {
    _origSyncRailSideCore();
  ...
  document.addEventListener('click', (e) => {
    ...
    if (
      e.target.closest(
        '.session-dropdown, .folder-submenu, #styled-prompt-overlay, #styled-confirm-overlay',
      )
    )
      return;
  ...
  if (window.innerWidth < 700) {
    const _restoreSidebar = () => {
      ...
      const anyDocked =
        document.querySelectorAll('.minimized-dock-chip').length > 0;
      ...
      if (
        _railWasOpenBeforeTool &&
        rail &&
        !rail.classList.contains('mobile-mini')
      ) {
    ...
    _modalObs.observe(document.body, {
      subtree: true,
      attributes: true,
    ...
    (e) => {
      ...
      if (!decided) {
        ...
        if (ady > adx) {
          track = false;
          return;
        } // vertical-dominant → let it scroll
