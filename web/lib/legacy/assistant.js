...
import { selectSession } from '$lib/legacy/sessions.js';
...
function _ensureModalEl() {
  ...
  modal
    .querySelector('#assistant-settings-close')
...
function _renderSettingsBody(body, data, tzList) {
  ...
  _fetchEndpoints().then((endpoints) => {
    ...
    for (const ep of endpoints) {
      ...
      const sel =
        crew.endpoint_url &&
        url.includes(crew.endpoint_url.replace('/v1', '').replace(/\/$/, ''))
          ? ' selected'
          : '';
    ...
    epSelect.addEventListener('change', async () => {
      ...
      try {
        ...
        let mHTML = '';
        const modelIds = (models.models || models || [])
        ...
        modelSelect.innerHTML =
          mHTML || '<option value="">(no models)</option>';
      } catch {
  ...
  body.querySelector('#assistant-tools-all')?.addEventListener('click', () => {
    body.querySelectorAll('.assistant-tool-cb').forEach((cb) => {
      cb.checked = true;
    });
  });
  ...
  if (charPick && personalityEl) {
    ...
    charPick.addEventListener('change', () => {
      ...
      let name = '';
      if (type === 'preset') {
        const p = (charPick._presets || []).find(
          (x) => (x._key || x.name || x.id) === id,
        );
        if (p) {
          prompt = p.system_prompt || p.personality || '';
          name = p.character_name || p.name || p._key || '';
        }
      } else if (type === 'template') {
        const t = (charPick._templates || []).find(
          (x) => (x.id || x.name) === id,
        );
        if (t) {
          prompt = t.system_prompt || t.personality || '';
          name = t.character_name || t.name || '';
        }
      }
  ...
  // ── Event wiring ──
  body
    .querySelector('#assistant-settings-cancel')
    .addEventListener('click', _closeModal);
  body
    .querySelector('#assistant-settings-save')
    .addEventListener('click', async () => {
      const selectedTools = [];
      body
        .querySelectorAll('.assistant-tool-cb:checked')
        .forEach((cb) => selectedTools.push(cb.value));
      const payload = {
        ...
        check_ins: Array.from(
          body.querySelectorAll('.assistant-checkin-row'),
        ).map((row) => ({
      ...
      const _poll = setInterval(async () => {
        try {
          const res = await fetch(
            `${API}/run-status/${encodeURIComponent(taskId)}`,
            { credentials: 'same-origin' },
          );
          if (!res.ok) return;
...
export async function openAssistantSettings() {
  ...
  try {
    ...
    console.error(e);
    body.innerHTML =
      '<div style="padding:12px;opacity:0.6;">Could not load assistant settings.</div>';
  }
}
...
async function _ensureHeaderAffordances(sessionId) {
  ...
  const headerRight = document.querySelector(
    '.chat-header-right, #chat-header .actions, .chat-header',
  );
...
function _watchForAssistantActivation() {
  ...
  const interval = setInterval(async () => {
    ...
    const activeSessionId =
      window.sessionModule?.getActiveSession?.()?.id ||
