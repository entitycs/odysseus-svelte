...
function _saveDismissed(set) {
  try {
    localStorage.setItem(_DISMISSED_KEY, JSON.stringify([...set]));
  } catch {}
}
...
async function _reconnectActive() {
  try {
    ...
    const res = await fetch(`${_apiBase}/api/research/active`, {
    ...
    const libRes = await fetch(
      `${_apiBase}/api/research/library?sort=recent&limit=20`,
      { credentials: 'same-origin' },
    );
    if (libRes.ok) {
...
export async function cancelJob(id) {
  ...
  try {
    await fetch(`${_apiBase}/api/research/cancel/${id}`, {
      method: 'POST',
      credentials: 'same-origin',
    });
  } catch {}
...
export function formatPhase(progress, maxRounds) {
  ...
  switch (p.phase) {
    ...
    case 'searching':
      return `${rn}Searching (${p.queries || 0} queries)`;
    case 'reading':
      return `${rn}Reading ${p.total_sources || 0} sources`;
    case 'analyzing':
      return `${rn}Analyzing ${p.total_findings || 0} findings`;
    case 'writing':
      return `Writing report -- ${p.total_sources || 0} sources`;
    default:
      return p.phase;
  }
}
...
async function _launchJob(job) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      try {
        job.errorMsg = JSON.parse(txt).detail || txt;
      } catch {
        job.errorMsg = txt;
      }
      job.status = 'error';
...
async function _pollFallback(job) {
  ...
  try {
    const res = await fetch(`${_apiBase}/api/research/status/${job.id}`, {
      credentials: 'same-origin',
    });
    if (!res.ok) {
      _finishJob(job, 'error');
...
function _finishJob(job, status) {
  ...
  if (status === 'done') {
    if ('Notification' in window && Notification.permission === 'granted') {
      try {
        new Notification('Research Complete', { body: job.query.slice(0, 80) });
      } catch {}
    }
