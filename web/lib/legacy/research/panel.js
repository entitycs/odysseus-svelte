...
import * as jobs from '$lib/legacy/research/jobs.js';
...
let _synapseMinimized;
const _vizCollapseIcon =
  '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="18 15 12 9 6 15"/></svg>';
const _vizExpandIcon =
  '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>';

export function init() {
  try {
    _synapseMinimized = localStorage.getItem(_SYNAPSE_MIN_KEY) === '1';
  } catch {
    _synapseMinimized = false;
  }
  try {
    _settingsCollapsed = localStorage.getItem(_COLLAPSE_KEY) === '1';
  } catch {}
}
...
function _toggleSynapseMinimized() {
  ...
  try {
    localStorage.setItem(_SYNAPSE_MIN_KEY, _synapseMinimized ? '1' : '0');
  } catch {}
  // Apply live to all rendered cards without forcing a full rebuild.
  document
    .querySelectorAll('.research-job-synapse-host')
  ...
  try {
    localStorage.setItem(
      ...
      JSON.stringify({
        ...
        search_provider:
          document.getElementById('research-search-provider')?.value || '',
...
function _syncResearchRail() {
  ...
  if (railBtn) {
    railBtn.classList.remove(
      'rail-notify',
      'rail-notify-success',
      'rail-notify-error',
      'research-notif-active',
    );
    if (active) {
      railBtn.classList.add(
        'rail-notify',
        errored ? 'rail-notify-error' : 'rail-notify-success',
        'research-notif-active',
      );
    }
  }
  if (toolBtn) {
    ...
    if (running > 0) {
      if (!wrap) {
        ...
        wrap.innerHTML =
          '<span class="research-sb-status"></span><span class="research-sb-dot"></span>';
        toolBtn.appendChild(wrap);
      }
      const round =
        runningJob && runningJob.progress && runningJob.progress.round;
      ...
      if (round)
        wrap.querySelector('.research-sb-status').textContent = `R${round}`;
    } else if (wrap) {
      wrap.remove();
    }
  }
...
async function _updateResearchCount() {
  ...
  if (!el) return;
  try {
    const res = await fetch('/api/research/library?limit=1', {
...
export function initLegacy(apiBase, markdownMod, sessionMod) {
  _apiBase = apiBase;
  _markdownModule = markdownMod;
  _sessionModule = sessionMod;
  jobs.initLegacy(apiBase);
...
function _buildPanelHTML() {
  ...
  const providerOpts = searchProviders
    .map((p) => `<option value="${p}">${p || 'Default'}</option>`)
    .join('');
...
function _wireEvents(pane) {
  pane
    .querySelector('#research-panel-close')
    .addEventListener('click', closePanel);
  pane
    .querySelector('#research-panel-minimize')
  ...
  pane
    .querySelector('#research-start-btn')
    .addEventListener('click', _handleStart);
  pane.querySelector('#research-add-btn').addEventListener('click', _handleAdd);

  pane
    .querySelector('#research-settings-toggle')
    .addEventListener('click', () => {
      ...
      try {
        localStorage.setItem(
          'odysseus-research-settings-collapsed',
          _settingsCollapsed ? '1' : '0',
        );
      } catch {}
    });
...
function _readSettings() {
  const category =
    document.getElementById('research-category')?.value || undefined;
  const settings = {
    max_rounds: parseInt(
      document.getElementById('research-rounds')?.value || '0',
      10,
    ),
    search_provider:
      document.getElementById('research-search-provider')?.value || undefined,
    endpoint_id:
      document.getElementById('research-endpoint')?.value || undefined,
...
async function _loadEndpoints() {
  try {
    const res = await fetch(`${_apiBase}/api/model-endpoints`, {
      credentials: 'same-origin',
    });
    if (!res.ok) return;
    ...
    if (!sel) return;
    _endpoints
      .filter((e) => e.is_enabled && e.model_type === 'llm')
      .forEach((ep) => {
...
function _renderJobs() {
  ...
  if (!allJobs.length) {
    ...
    if (noPastHint) {
      ...
      if (!noPastHint.dataset._wired) {
        ...
        noPastHint
          .querySelector('.research-library-link')
  ...
  const recentDone = allJobs
    .filter((j) => j.status === 'done' && !j._fromLibrary)
    .reverse();
  ...
  const liveIds = new Set(
    allJobs.filter((j) => j.status === 'running').map((j) => j.id),
  );
  for (const [jobId, entry] of _jobSynapses) {
    if (liveIds.has(jobId)) continue;
    try {
      entry.synapse.complete();
    } catch {}
    setTimeout(() => {
      try {
        entry.synapse.destroy();
      } catch {}
    }, 800);
  ...
  const _addSection = (key, title, arr) => {
    ...
    let dotColor,
      dotPulse = false;
    if (key === 'active') {
      const failed = arr.some(
        (j) => j.status === 'error' || j.status === 'cancelled',
      );
      if (failed) {
        dotColor = '#f44336';
      } else {
        dotColor = 'var(--accent, var(--red))';
        dotPulse = true;
      }
    } else {
    ...
    const clearAllHtml =
      '<button class="research-section-clear" title="Clear all research">' +
      _cancelIcon +
      ' Clear all</button>';
    header.innerHTML =
      '<span class="research-section-title">' +
      title +
      '</span>' +
      '<span class="research-section-count memory-count">' +
      arr.length +
      ' research</span>' +
      '<span class="research-section-right">' +
      clearAllHtml +
      '<span class="research-section-dot' +
      (dotPulse ? ' pulsing' : '') +
      '" style="background:' +
      dotColor +
      ';"></span>' +
    ...
    if (key === 'past') {
      ...
      hint.innerHTML =
        '<span>Multi-step web research with an LLM-in-the-loop agent</span> <button type="button" class="research-library-link">Library, Research</button>';
      hint
        .querySelector('.research-library-link')
        .addEventListener('click', (e) => {
          e.stopPropagation();
    ...
    header.addEventListener('click', () => {
      ...
      if (nowCollapsed) _collapsedSections.add(key);
      else _collapsedSections.delete(key);
    });
    header
      .querySelector('.research-section-clear')
      ?.addEventListener('click', (e) => {
        e.stopPropagation();
...
function _promptParallelOrSequential(count, anchorBtn) {
  ...
  const ICON_SEQUENTIAL =
    '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="8" y1="6" x2="20" y2="6"/><line x1="8" y1="12" x2="20" y2="12"/><line x1="8" y1="18" x2="20" y2="18"/><circle cx="4" cy="6" r="1.5" fill="currentColor"/><circle cx="4" cy="12" r="1.5" fill="currentColor"/><circle cx="4" cy="18" r="1.5" fill="currentColor"/></svg>';
  pop.innerHTML =
    '<button class="research-run-mode-row" data-mode="parallel">' +
    ICON_PARALLEL +
    '<span class="rrm-title">Parallel</span></button>' +
    '<button class="research-run-mode-row" data-mode="sequential">' +
    ICON_SEQUENTIAL +
    '<span class="rrm-title">Sequential</span></button>';
...
function _buildJobCard(job) {
  ...
  const modelTag =
    job.modelName || job.settings?._modelName
  ...
  if (job.status === 'queued') {
    ...
    `;
    card
      .querySelector('.research-job-cancel')
      .addEventListener('click', (e) => {
        e.stopPropagation();
        jobs.cancelJob(job.id);
      });
    card
      .querySelector('.research-synapse-toggle')
      ?.addEventListener('click', (e) => {
        e.stopPropagation();
        _toggleSynapseMinimized();
      });
    ...
    if (job.progress) {
      ...
      if (typeof job.progress.round === 'number')
        entry.synapse.setRound(job.progress.round);
      if (typeof job.progress.total_sources === 'number')
        entry.synapse.setSourceCount(job.progress.total_sources);
    }
    ...
    const doneBadge = failed
      ...
      : job.category
        ? `<span class="research-cat-badge">${_esc(job.category)}</span>`
        : `<span class="research-cat-badge research-cat-standard">standard</span>`;
      ...
      .addEventListener('click', async (e) => {
        ...
        if (window.styledConfirm) {
          const ok = await window.styledConfirm(
            'Delete this research? This permanently removes it from disk.',
            { confirmText: 'Delete', danger: true },
          );
          if (!ok) return;
        }
        try {
          await fetch(`${_apiBase}/api/research/${job.id}`, {
            method: 'DELETE',
            credentials: 'same-origin',
          });
        } catch {}
    ...
    const errMsg = job.errorMsg
      ? `<div class="research-job-error">${_esc(job.errorMsg)}</div>`
      : '';
    card.innerHTML = `
      <div class="research-job-header">
        <span class="research-job-query">${_esc(job.query)}</span>${job.category ? `<span class="research-cat-badge">${_esc(job.category)}</span>` : ''}
...
const _CAT_ICONS = {
  product:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="8" width="18" height="13" rx="2"/><path d="M7 8V5a5 5 0 0 1 10 0v3"/></svg>',
  comparison:
    '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3v18"/><path d="M16 3v18"/><path d="M3 8h5"/><path d="M16 16h5"/></svg>',
  howto:
...
function _renderResult(job) {
  if (!job.result)
    return '<div class="research-job-loading">Loading result...</div>';
  const cat = job.category || '';
  ...
  if (job.sources?.length) {
    ...
    if (job.sources.length > 10)
      html += `<span class="research-source-more">+${job.sources.length - 10} more</span>`;
    html += '</div>';
  }
...
async function _copyResult(job, btn) {
  ...
  if (btn) {
    ...
    if (ok) {
      ...
      setTimeout(() => {
        btn.innerHTML = orig;
        btn.classList.remove('research-job-action-copied');
      }, 2000);
    } else {
...
async function _chatAboutResearch(researchId, btn) {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = `${_chatIcon} Creating…`;
  }
  try {
    ...
    if (_sessionModule && _sessionModule.selectSession && payload.session_id) {
      if (_sessionModule.loadSessions)
        await _sessionModule.loadSessions().catch(() => {});
