...
} from '$lib/legacy/model/endpoint.js';
...
import {
  formatCompactNumber,
  toTrimmedString,
} from '$lib/legacy/util/format.js';
import {
  safeDisplayImageSrc,
  safeHref,
  safeToolScreenshotSrc,
} from '$lib/legacy/util/safeString.js';
...
async function loadExecFenceRegex() {
  try {
    ...
    console.warn(
      'chatRenderer: /api/tools fetch failed; live exec-fence stripping disabled until reload',
      err,
    );
  }
}
  ...
  editBtn.addEventListener('click', async (e) => {
    ...
    try {
      const [galleryMod, editorMod] = await Promise.all([
        import('$lib/legacy/gallery'), // TODO IMPORT (why 'not found when .js?')
        import('$lib/legacy/galleryEditor.js'), // TODO IMPORT
      ]);
...
export function renderAskUserCard(payload, options) {
  ...
  otherInput.placeholder = multi
    ? 'Other (added to selection)…'
    : 'Other… (type your own answer)';
  otherInput.setAttribute(
    'aria-label',
    multi ? 'Add a custom option' : 'Type a custom answer',
  );
  ...
  const submit = () => {
    ...
    if (multi) {
      const picked = Array.from(
        card.querySelectorAll('.ask-user-option input:checked'),
