...
import { _clearDiagnosis, _diagnose, _showDiagnosis } from '$lib/legacy/cookbook-diagnosis.js';
import { nextFreePort, portOf } from '$lib/legacy/cookbookPorts.js';
import { computeProgressSignal } from '$lib/legacy/cookbookProgressSignal.js';
...
async function _openDownloadForGgufTask(task) {
  ...
  setTimeout(async () => {
    ...
    try {
      const hwfit = await import('$lib/legacy/cookbook-hwfit.js');
...
async function _openServeEditForTask(task, cmdOverride, fieldOverrides = null) {
  ...
  try {
    const { openServePanelForRepo } = await import('$lib/legacy/cookbookServe.js');
