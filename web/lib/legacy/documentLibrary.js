...
import {
  dismissOrRemove,
  registerMenuDismiss,
} from '$lib/legacy/escMenuStack.js';
import { langIcon } from '$lib/legacy/langIcons.js';
  ...
  if (!_libScrollBound) {
    ...
    const _maybeAutoLoad = () => {
      ...
      for (const btn of document.querySelectorAll('.doclib-inline-load-more')) {
        ...
        if (btn.getBoundingClientRect().top > window.innerHeight + 600)
          continue;
...
function _collapseExpandedCard(card) {
  ...
  if (grid && !instant) {
    const siblings = [...grid.querySelectorAll('.doclib-card')].filter(
      (c) => c !== card,
    );
    siblings.forEach((s) => {
      s.style.opacity = '0';
...
function _showLibDropdown(anchor, items, opts) {
  ...
  for (const item of items) {
    ...
    row.className =
      'dropdown-item-compact' + (item.danger ? ' dropdown-item-danger' : '');
    const iconKey = item.icon || item.label.toLowerCase();
    const iconSvg = _LIB_DD_ICONS[iconKey] || '';
    row.innerHTML =
      (iconSvg ? '<span class="dropdown-icon">' + iconSvg + '</span>' : '') +
      '<span>' +
      item.label +
      '</span>';
  ...
  const close = (e) => {
    if (!dd.contains(e.target)) teardown();
  };
...
function libraryRenderLangChips() {
  ...
  allChip.className =
    'memory-cat-chip' + (!_libraryActiveLanguage ? ' active' : '');
  ...
  for (const [lang, count] of sorted) {
    ...
    chip.className =
      'memory-cat-chip' + (_libraryActiveLanguage === lang ? ' active' : '');
...
function libraryRemoveDocumentFromState(docId) {
  ...
  if (lang && Object.hasOwn(_libraryLanguages, lang)) {
...
function libraryRenderGrid() {
  ...
  if (grid.parentElement)
    grid.parentElement
      .querySelectorAll(':scope > .doclib-inline-load-more')
      .forEach((b) => b.remove());

  if (_libraryDocs.length === 0) {
    if (_librarySearch || _libraryActiveLanguage) {
      grid.innerHTML =
        '<div class="doclib-empty">No documents match your search.</div>';
    } else {
      const _impIco =
        '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin:0 4px;"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>';
      grid.innerHTML =
        '<div class="doclib-empty" style="display:flex;align-items:center;justify-content:center;gap:8px;flex-wrap:wrap;">' +
        '<span>No documents yet</span>' +
        '<span style="opacity:0.7;font-size:11px;">' +
        '<a href="#" data-doclib-import style="color:var(--accent,var(--red));text-decoration:underline;">Import' +
        _impIco +
        '</a>' +
        ...
        '</div>';
      grid
        .querySelector('[data-doclib-import]')
  ...
  if (shownCount < _libraryTotal) {
    ...
    btn.addEventListener('click', async () => {
      ...
      if (
        _docsVisibleLimit > _libraryDocs.length &&
        _libraryDocs.length < _libraryTotal
      ) {
...
function libraryCreateCard(doc) {
  ...
  if (_librarySelectMode) {
    ...
    cb.addEventListener('change', () => {
      ...
      const selectAllEl = document.getElementById('doclib-select-all');
      if (selectAllEl)
        selectAllEl.checked = _libraryDocs.every((d) =>
          _librarySelectedIds.has(d.id),
        );
    });
    card.appendChild(cb);
  }
  ...
  const _langSvg =
    doc.language && doc.language !== 'text'
      ? langIcon(doc.language, 12, {
          style:
            'vertical-align:-2px;margin-right:4px;opacity:0.55;flex-shrink:0;color:currentColor;',
        })
      : '';
  titleEl.innerHTML =
    (_langSvg || _GEN_DOC_ICON) + _hlSearch(doc.title || 'Untitled');
  titleRow.appendChild(titleEl);
  const verBadge = document.createElement('span');
  verBadge.style.cssText =
    'font-size:9px;padding:1px 6px;border-radius:8px;background:color-mix(in srgb, var(--red) 15%, transparent);border:1px solid color-mix(in srgb, var(--red) 40%, transparent);color:var(--red);flex-shrink:0;';
  ...
  meta.style.cssText =
    'font-size:10px;opacity:0.55;margin-top:2px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;';
  ...
  menuBtn.addEventListener('click', (e) => {
    ...
    if (window.innerWidth <= 768) {
      const items = [];
      if (doc.session_id)
        items.push({ label: 'Open', action: () => libraryOpenInSession(doc) });
      items.push({ label: 'Clone', action: () => libraryImportDocument(doc) });
      _showLibDropdown(menuBtn, items, {
        onSelect: () => {
    ...
    const dropdown =
      menuWrap.querySelector('.doclib-card-dropdown') ||
      document.body.querySelector(
        '.doclib-card-dropdown[data-owner="' + CSS.escape(doc.id) + '"]',
      );
    if (dropdown) {
      const isOpen =
      ...
      if (isOpen) {
        ...
        requestAnimationFrame(() => {
          ...
          if (mr.bottom > window.innerHeight - 8)
            dropdown.style.top = rect.top - mr.height - 4 + 'px';
          if (mr.left < 8) {
        ...
        setTimeout(
          () => document.addEventListener('click', _cardDocClick, true),
          0,
        );
  ...
  dropdown.style.cssText =
    'display:none;position:absolute;top:100%;right:0;z-index:1000;min-width:0;width:max-content;padding:4px;background:var(--panel);border:1px solid var(--border);border-radius:8px;box-shadow:0 8px 24px rgba(0,0,0,0.3);backdrop-filter:blur(12px);font-size:12px;';
  ...
  function hideCardDropdown() {
    ...
    _cardUnreg = () => {};
    if (_cardDocClick) {
      document.removeEventListener('click', _cardDocClick, true);
      _cardDocClick = null;
    }
    dropdown.style.display = 'none';
  ...
  exportItem.addEventListener('click', async (e) => {
    ...
    try {
      ...
      const blob = new Blob([full.current_content || ''], {
        type: 'text/plain',
      });
      ...
      if (uiModule) uiModule.showError('Failed to export document');
    }
  });
  ...
  archiveItem.innerHTML =
    _di(_archiveIco) +
    `<span>${_libraryArchivedView ? 'Restore' : 'Archive'}</span>`;
  archiveItem.title = _libraryArchivedView
    ? 'Restore to active documents'
  ...
  archiveItem.addEventListener('click', async (e) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/document/${doc.id}/archive?archived=${toArchived}`,
      ...
      if (uiModule)
        uiModule.showError('Failed to ' + (toArchived ? 'archive' : 'restore'));
    }
  });
  ...
  deleteItem.addEventListener('click', (e) => {
    e.stopPropagation();
    hideCardDropdown();
    libraryDeleteSingle(doc.id, card);
  });
  ...
  try {
    if (
      ...
      code.innerHTML = window.hljs.highlight(doc.preview || '', {
  ...
  openBtn.innerHTML =
    '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><path d="M5 12h14M13 5l7 7-7 7"/></svg>Open';
  if (doc.session_id) {
  ...
  cloneBtn.innerHTML =
    '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>Clone';
  ...
  archiveBtn.innerHTML =
    '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px;margin-right:3px;"><polyline points="21 8 21 21 3 21 3 8"/><rect x="1" y="3" width="22" height="5"/><line x1="10" y1="12" x2="14" y2="12"/></svg>' +
    (_libraryArchivedView ? 'Restore' : 'Archive');
  archiveBtn.title = _libraryArchivedView
    ? 'Restore to active documents'
    : 'Archive (hide from the main list)';
...
async function libraryExpandCard(card, doc) {
  ...
  if (grid) {
    grid
      ...
      .forEach((c) => _collapseExpandedCard(c));
  }

  // Fade siblings out before the CSS display:none kicks in
  const siblings = grid
    ? [...grid.querySelectorAll('.doclib-card')].filter((c) => c !== card)
    : [];
  ...
  if (!instant) {
    ...
    siblings.forEach((s) => {
      s.style.transition = 'opacity 0.12s ease';
      s.style.opacity = '0';
    });
  }
  ...
  const toolbar = grid
    ? grid.closest('.admin-card')?.querySelector('.memory-toolbar')
    : null;
  ...
  try {
    ...
    const isPdfDoc = /<!--\s*pdf_(?:form_)?source\s+upload_id="[^"]+"/.test(
      content,
    );
    ...
    if (isPdfDoc) {
      ...
      frame.style.cssText =
        'width:100%;height:60vh;border:1px solid var(--border);border-radius:6px;background:var(--bg);opacity:0;transition:opacity 0.15s ease;';
      if (existingPre) existingPre.remove();
      if (existingFrame) existingFrame.remove();
      preview.insertBefore(frame, preview.firstChild);
      if (actionsBar && !preview.contains(actionsBar))
        preview.appendChild(actionsBar);
      requestAnimationFrame(() => {
    ...
    if (!existingPre) {
      preview.innerHTML =
        '<div style="padding:8px;color:var(--color-error);font-size:10px;">Failed to load</div>';
    }
    if (actionsBar && !preview.contains(actionsBar))
      preview.appendChild(actionsBar);
  }
...
function libraryUpdateBulkCount() {
  ...
  if (actionsBtn)
    actionsBtn.style.color = _librarySelectedIds.size > 0 ? 'var(--fg)' : '';
...
async function libraryDeleteSingle(docId, card) {
  ...
  try {
    ...
    if (uiModule)
      uiModule.showError(`Failed to delete document: ${e.message || e}`);
  }
}
...
async function libraryBulkDelete() {
  ...
  for (const id of _librarySelectedIds) {
    try {
      ...
      if (res.ok) {
        ...
        failed++;
        console.warn('Delete failed for', id, 'status', res.status);
      }
    } catch (e) {
      failed++;
  ...
  for (const id of deletedIds) {
    const card = document.querySelector(
      `.doclib-card[data-doc-id="${CSS.escape(String(id))}"]`,
    );
...
async function libraryBulkExport() {
  ...
  if (_librarySelectedIds.size > 5) {
    ...
    try {
      ...
      if (uiModule)
        uiModule.showToast(`Exported ${ids.length} documents (zip)`);
    } catch (e) {
  ...
  const docs = await Promise.all(
    [..._librarySelectedIds].map(async (id) => {
      try {
        const res = await fetch(`${API_BASE}/api/document/${id}`);
  ...
  for (const doc of docs) {
    ...
    const filename =
      (doc.title || 'document') +
      (doc.title && doc.title.includes('.') ? '' : ext);
  ...
  if (uiModule)
    uiModule.showToast(
      `Exported ${_librarySelectedIds.size} document${_librarySelectedIds.size !== 1 ? 's' : ''}`,
    );
}
...
export function openLibrary(opts) {
        ...
        if (saved && saved.fullscreen) {
          ...
          requestAnimationFrame(() => {
            ...
            if (r.right > window.innerWidth)
              content.style.left =
                Math.max(0, window.innerWidth - r.width - 8) + 'px';
            if (r.bottom > window.innerHeight)
              content.style.top =
                Math.max(0, window.innerHeight - r.height - 8) + 'px';
  ...
  function _switchLibTab(tab) {
    _activeLibTab = tab;
    _tabBtns.forEach((b) =>
      b.classList.toggle('active', b.dataset.doclibTab === tab),
    );
  ...
  function _renderLibChats() {
    ...
    fetch(API_BASE + '/api/sessions', { credentials: 'same-origin' })
      .then((r) => r.json())
      .then((data) => {
      ...
      .catch(() => {
        grid.innerHTML = '<div class="doclib-empty">Failed to load</div>';
      });
  }
  ...
  async function _toggleChatPreview(card, session) {
    ...
    preview.innerHTML =
      '<div style="opacity:0.4;font-size:11px;padding:8px 4px;">Loading…</div>';
    try {
      ...
      const msgsHtml = recent.length
        ? recent
            .map((m) => {
              ...
              const msgModel =
                (m.metadata && (m.metadata.model || m.metadata.model_name)) ||
                '';
              const modelTag =
                !isUser && (msgModel || sessionModel)
      ...
      const copyHtml = isArchive
        ? ''
        : '<button class="doclib-chat-copy-btn">' +
      ...
      if (openBtn)
        openBtn.addEventListener('click', (e) => {
          ...
          if (window.sessionModule)
            window.sessionModule.selectSession(session.id);
          closeLibrary();
      ...
      if (restoreBtn)
        restoreBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          await fetch(API_BASE + '/api/session/' + session.id + '/unarchive', {
            method: 'POST',
          });
          _renderLibArchive();
        });
      ...
      if (deleteBtn)
        deleteBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          if (
            !(await window.styledConfirm('Delete this chat?', {
              confirmText: 'Delete',
              danger: true,
            }))
          )
            return;
          await fetch(API_BASE + '/api/session/' + session.id, {
            method: 'DELETE',
          });
          ...
          if (isArchive) _renderLibArchive();
          else _renderLibChats();
        });
    } catch (e) {
      preview.innerHTML =
        '<div style="opacity:0.5;font-size:11px;padding:6px 4px;color:var(--color-error);">Failed to load preview</div>';
    }
  }
  ...
  function _renderChatsGrid() {
    ...
    if (_csb) {
      _csb.classList.toggle('active', _chatsSelectMode);
      _csb.textContent = _chatsSelectMode ? 'Cancel' : 'Select';
    }
    ...
    if (_chatsModelFilter)
      filtered = filtered.filter((s) => s.folder === _chatsModelFilter);
    if (_chatsSort === 'oldest')
      filtered.sort((a, b) =>
        (a.updated_at || '') > (b.updated_at || '') ? 1 : -1,
      );
    else if (_chatsSort === 'most-messages')
      filtered.sort((a, b) => (b.message_count || 0) - (a.message_count || 0));
    else if (_chatsSort === 'alpha')
    ...
    if (!filtered.length) {
      ...
      const _sadIco =
        '<span style="vertical-align:-3px;margin-left:6px;">' +
        uiModule.emptyStateIcon('sad') +
        '</span>';
      grid.innerHTML =
        '<div class="doclib-empty">No chats' + _sadIco + '</div>';
    ...
    for (const s of visible) {
      ...
      const cbHtml = _chatsSelectMode
        ? '<input type="checkbox" class="memory-select-cb"' +
          (_chatsSelected.has(s.id) ? ' checked' : '') +
          '>'
        : '';
      const chatIconSvg =
        '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px;opacity:0.4;flex-shrink:0;"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>';
      const chevronSvg =
        '<span class="doclib-card-chevron"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></span>';
      ...
      const msgCountHtml =
        _chatMsgs > 0
          ? '<span style="opacity:0.45;font-weight:normal;font-size:0.9em;margin-left:6px;">\u00b7 ' +
            _chatMsgs +
            ' msg' +
            (_chatMsgs === 1 ? '' : 's') +
            '</span>'
      ...
      if (cb) {
        cb.addEventListener('click', (e) => e.stopPropagation());
        cb.addEventListener('change', () => {
          if (cb.checked) _chatsSelected.add(s.id);
          else _chatsSelected.delete(s.id);
          _updateChatsCount();
        });
      }
      card.querySelector('._chat-menu').addEventListener('click', (e) => {
        e.stopPropagation();
        _showLibDropdown(
          e.currentTarget,
          [
            {
              ...
              action: () => {
                if (window.sessionModule)
                  window.sessionModule.selectSession(s.id);
              },
            },
            { label: 'Copy', action: () => _copyChatById(s.id) },
            {
              label: 'Archive',
              action: async () => {
                await fetch(API_BASE + '/api/session/' + s.id + '/archive', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                });
                _renderLibChats();
            ...
            },
            {
              ...
              action: async () => {
                if (
                  ...
                  return;
                await fetch(API_BASE + '/api/session/' + s.id, {
                  method: 'DELETE',
                });
                card.style.maxHeight = `${Math.max(card.getBoundingClientRect().height, card.scrollHeight)}px`;
                card.classList.add('memory-tidy-removing');
          ...
          ],
          {
            onSelect: () => {
              ...
              document
                .getElementById('doclib-chats-bulk')
      ...
      card.addEventListener('click', (e) => {
        ...
        if (_chatsSelectMode) {
          const c = card.querySelector('.memory-select-cb');
          if (c) {
            c.checked = !c.checked;
            if (c.checked) _chatsSelected.add(s.id);
            else _chatsSelected.delete(s.id);
            _updateChatsCount();
          }
          return;
        ...
        if (
          e.target.closest('._chat-menu') ||
          e.target.closest('.memory-select-cb') ||
          e.target.closest('.doclib-chat-open-btn')
        )
          return;
  ...
  function _renderChatsChips() {
    ...
    const mk = (label, val, count) => {
      ...
      c.className =
        'memory-cat-chip' + (_chatsModelFilter === val ? ' active' : '');
      c.textContent = label + ' (' + count + ')';
      c.addEventListener('click', () => {
        _chatsModelFilter = _chatsModelFilter === val ? '' : val;
        _renderChatsGrid();
        _renderChatsChips();
      });
      el.appendChild(c);
  ...
  function _updateChatsCount() {
    const el = document.getElementById('doclib-chats-selected-count');
    if (el) el.textContent = _chatsSelected.size + ' Selected';
  }

  // Chats event listeners
  document
    .getElementById('doclib-chats-sort')
    .addEventListener('change', (e) => {
      _chatsSort = e.target.value;
      _renderChatsGrid();
    });
  document
    .getElementById('doclib-chats-search')
    .addEventListener('input', (e) => {
      _chatsSearch = e.target.value.trim();
      _renderChatsGrid();
    });
  document
    .getElementById('doclib-chats-select-btn')
    .addEventListener('click', () => {
      _chatsSelectMode = !_chatsSelectMode;
      _chatsSelected.clear();
      document
        .getElementById('doclib-chats-bulk')
        .classList.toggle('hidden', !_chatsSelectMode);
      _renderChatsGrid();
    });
  document
    .getElementById('doclib-chats-bulk-cancel')
    ?.addEventListener('click', () => {
  ...
  function _chatsToggleAll() {
    ...
    document
      .querySelectorAll('#doclib-chats-grid .memory-select-cb')
      .forEach((cb) => {
        cb.checked = newState;
      });
    _chatsSessions.forEach((s) => {
      if (newState) _chatsSelected.add(s.id);
      else _chatsSelected.delete(s.id);
    });
    _updateChatsCount();
  }
  document
    .getElementById('doclib-chats-select-all')
    .addEventListener('change', _chatsToggleAll);
  document
    .getElementById('doclib-chats-bulk')
    .addEventListener('click', (e) => {
  ...
  document
    .getElementById('doclib-chats-bulk-archive')
    .addEventListener('click', async () => {
      ...
      const results = await Promise.all(
        ids.map((sid) =>
          fetch(API_BASE + '/api/session/' + sid + '/archive', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          })
            .then((r) => ({ sid, ok: r.ok }))
      ...
      if (failed.length && grid) {
        ...
        if (window.uiModule)
          window.uiModule.showError(
            `Failed to archive ${failed.length} of ${ids.length} chat${ids.length > 1 ? 's' : ''}`,
          );
      }
      _chatsSelected.clear();
      _chatsSelectMode = false;
  ...
  document
    .getElementById('doclib-chats-bulk-delete')
    .addEventListener('click', async () => {
      const count = _chatsSelected.size;
      if (!count) return;
      if (
      ...
      const results = await Promise.all(
        ids.map((sid) =>
          fetch(API_BASE + '/api/session/' + sid, { method: 'DELETE' })
            .then((r) => ({ sid, ok: r.ok }))
            .catch(() => ({ sid, ok: false })),
        ),
      );
      ...
      if (failed.length && grid) {
        ...
        if (window.uiModule)
          window.uiModule.showError(
            `Failed to delete ${failed.length} of ${ids.length} chat${ids.length > 1 ? 's' : ''}`,
          );
      }
      _chatsSelected.clear();
      _chatsSelectMode = false;
  ...
  document
    .getElementById('doclib-chats-tidy-btn')
    .addEventListener('click', async () => {
      ...
      sp.start();
      try {
        const res = await fetch(API_BASE + '/api/sessions/auto-sort', {
          method: 'POST',
          credentials: 'same-origin',
        });
        const data = await res.json();
        ...
        if (data.status === 'ok') {
          ...
          if (window.uiModule)
            window.uiModule.showToast(data.reason || 'Nothing to tidy');
        }
      } catch (e) {
  ...
  function _renderLibArchive() {
    ...
    Promise.all([
      fetch(API_BASE + '/api/sessions/archived?limit=100&sort=recent', {
        credentials: 'same-origin',
      })
        .then((r) => r.json())
        .catch(() => ({})),
      fetch(API_BASE + '/api/documents/library?archived=true&limit=50', {
        credentials: 'same-origin',
      })
        .then((r) => r.json())
        .catch(() => ({})),
      fetch('/api/research/library?archived=true', {
        credentials: 'same-origin',
      })
        .then((r) => r.json())
        .catch(() => ({})),
      ...
      .then(([s, d, r]) => {
        ...
        _arcSessions = (s.sessions || []).map((x) => ({
          ...x,
          archived: true,
        }));
        _arcDocs = d.documents || [];
        _arcResearch = (r.research || []).map((x) => ({
          ...x,
          archived: true,
        }));
        _renderArcGrid();
  ...
  async function _toggleArcDocPreview(card, d) {
    ...
    preview.innerHTML =
      '<div style="opacity:0.4;font-size:11px;padding:8px 4px;">Loading…</div>';
    try {
      const res = await fetch(`${API_BASE}/api/document/${d.id}`, {
        credentials: 'same-origin',
      });
      if (!res.ok) throw new Error('failed');
      const full = await res.json();
      ...
      const pre = document.createElement('pre');
      pre.style.cssText =
        'white-space:pre-wrap;word-break:break-word;font-size:11px;margin:6px 4px;max-height:50vh;overflow:auto;';
      ...
      actions
        .querySelector('.doclib-chat-delete-btn')
        .addEventListener('click', async (ev) => {
          ev.stopPropagation();
          if (
            !(await window.styledConfirm('Delete this document?', {
              confirmText: 'Delete',
              danger: true,
            }))
          )
            return;
          await fetch(`${API_BASE}/api/document/${d.id}`, {
            method: 'DELETE',
            credentials: 'same-origin',
          });
          _renderLibArchive();
      ...
      actions
        .querySelector('.doclib-chat-restore-btn')
        .addEventListener('click', async (ev) => {
          ev.stopPropagation();
          await fetch(
            `${API_BASE}/api/document/${d.id}/archive?archived=false`,
            { method: 'POST', credentials: 'same-origin' },
          );
          _renderLibArchive();
        });
      ...
      actions
        .querySelector('.doclib-chat-open-btn')
      ...
      preview.innerHTML =
        '<div style="opacity:0.4;font-size:11px;padding:8px 4px;">Failed to load preview</div>';
    }
  }
  ...
  function _renderArcGrid() {
    ...
    if (_asb) {
      _asb.classList.toggle('active', _arcSelectMode);
      _asb.textContent = _arcSelectMode ? 'Cancel' : 'Select';
    }
    ...
    if (_arcModelFilter)
      filtered = filtered.filter(
        (s) => (s.model || '').split('/').pop() === _arcModelFilter,
      );
    if (_arcSort === 'oldest')
      filtered.sort((a, b) =>
        (a.updated_at || '') > (b.updated_at || '') ? 1 : -1,
      );
    else if (_arcSort === 'most-messages')
      filtered.sort((a, b) => (b.message_count || 0) - (a.message_count || 0));
    else if (_arcSort === 'alpha')
      filtered.sort((a, b) => (a.name || '').localeCompare(b.name || ''));
    else
      filtered.sort((a, b) =>
        (b.updated_at || '') > (a.updated_at || '') ? 1 : -1,
    ...
    let filtDocs = _aq
      ? _arcDocs.filter((d) => (d.title || '').toLowerCase().includes(_aq))
      : _arcDocs;
    let filtResearch = _aq
      ? _arcResearch.filter((r) => (r.query || '').toLowerCase().includes(_aq))
      : _arcResearch;
    ...
    if (stats)
      stats.textContent =
        filtered.length + filtDocs.length + filtResearch.length + ' archived';

    if (!filtered.length && !filtDocs.length && !filtResearch.length) {
      // Neutral / no-smile face for "nothing archived here".
      const _neutralIco =
        '<span style="vertical-align:-3px;margin-left:6px;">' +
        uiModule.emptyStateIcon('neutral') +
        '</span>';
      grid.innerHTML =
        '<div class="doclib-empty">No archived items' + _neutralIco + '</div>';
    ...
    for (const s of visible) {
      ...
      const model = (s.model || '').split('/').pop();
      const cbHtml = _arcSelectMode
        ? '<input type="checkbox" class="memory-select-cb" data-arckey="chats:' +
          s.id +
          '"' +
          (_arcSelected.has('chats:' + s.id) ? ' checked' : '') +
          '>'
        : '';
      const arcIconSvg =
        '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;margin-right:4px;opacity:0.5;flex-shrink:0;"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>';
      card.innerHTML =
        '<div class="doclib-chat-header" style="display:flex;align-items:center;width:100%;gap:6px;">' +
        cbHtml +
        '<div style="flex:1;min-width:0;">' +
      ...
      if (cb) {
        cb.addEventListener('click', (e) => e.stopPropagation());
        cb.addEventListener('change', () => {
          if (cb.checked) _arcSelected.add('chats:' + s.id);
          else _arcSelected.delete('chats:' + s.id);
          _updateArcCount();
        });
      }
      card.querySelector('._arc-menu').addEventListener('click', (e) => {
        e.stopPropagation();
        _showLibDropdown(
          e.currentTarget,
          [
            ...
            { label: 'Copy', action: () => _copyChatById(s.id) },
            {
              label: 'Restore',
              action: async () => {
                await fetch(API_BASE + '/api/session/' + s.id + '/unarchive', {
                  method: 'POST',
                });
                _renderLibArchive();
              },
          ...
          ],
          {
            onSelect: () => {
              ...
              document
                .getElementById('doclib-arc-bulk')
                ?.classList.remove('hidden');
              _renderArcGrid();
            },
          },
      ...
      card.addEventListener('click', (e) => {
        ...
        if (_arcSelectMode) {
          ...
          if (c) {
            c.checked = !c.checked;
            if (c.checked) _arcSelected.add('chats:' + s.id);
            else _arcSelected.delete('chats:' + s.id);
            _updateArcCount();
          }
          return;
    ...
    for (const d of filtDocs) {
      ...
      const _dcb = _arcSelectMode
        ? '<input type="checkbox" class="memory-select-cb" data-arckey="documents:' +
          d.id +
          '"' +
          (_arcSelected.has('documents:' + d.id) ? ' checked' : '') +
          '>'
        : '';
      card.innerHTML =
        '<div class="doclib-chat-header" style="display:flex;align-items:center;width:100%;gap:6px;">' +
        ...
        '<div class="memory-item-title">' +
        _arcDocIco +
        _esc(d.title || 'Untitled') +
        '</div>' +
        '<div class="memory-item-meta" style="font-size:10px;opacity:0.4;margin-top:2px;">' +
        ['Document', d.language || 'text', _relTime(d.updated_at)]
          .filter(Boolean)
          .join(' · ') +
        '</div>' +
        '</div>' +
      ...
      if (_dcbEl) {
        _dcbEl.addEventListener('click', (e) => e.stopPropagation());
        _dcbEl.addEventListener('change', () => {
          if (_dcbEl.checked) _arcSelected.add('documents:' + d.id);
          else _arcSelected.delete('documents:' + d.id);
          _updateArcCount();
        });
      }
      card.addEventListener('click', (e) => {
        if (
          e.target.closest('._arc-doc-menu') ||
          e.target.closest('.memory-select-cb')
        )
          return;
        if (_arcSelectMode) {
          ...
          if (c) {
            c.checked = !c.checked;
            if (c.checked) _arcSelected.add('documents:' + d.id);
            else _arcSelected.delete('documents:' + d.id);
            _updateArcCount();
          }
          return;
      ...
      card.querySelector('._arc-doc-menu').addEventListener('click', (e) => {
        e.stopPropagation();
        _showLibDropdown(
          e.currentTarget,
          [
    ...
    for (const r of filtResearch) {
      ...
      const _rcb = _arcSelectMode
        ? '<input type="checkbox" class="memory-select-cb" data-arckey="research:' +
          r.id +
          '"' +
          (_arcSelected.has('research:' + r.id) ? ' checked' : '') +
          '>'
        : '';
      card.innerHTML =
        '<div class="doclib-chat-header" style="display:flex;align-items:center;width:100%;gap:6px;">' +
        ...
        '<div class="memory-item-title">' +
        _arcResIco +
        _esc(r.query || 'Research') +
        '</div>' +
        '<div class="memory-item-meta" style="font-size:10px;opacity:0.4;margin-top:2px;">' +
        [
          'Research',
          r.source_count ? r.source_count + ' sources' : '',
          _relTime(
      ...
      if (_rcbEl) {
        _rcbEl.addEventListener('click', (e) => e.stopPropagation());
        _rcbEl.addEventListener('change', () => {
          if (_rcbEl.checked) _arcSelected.add('research:' + r.id);
          else _arcSelected.delete('research:' + r.id);
          _updateArcCount();
        });
      }
      card.addEventListener('click', (e) => {
        if (
          e.target.closest('._arc-res-menu') ||
          e.target.closest('.memory-select-cb')
        )
          return;
        if (_arcSelectMode) {
          ...
          if (c) {
            c.checked = !c.checked;
            if (c.checked) _arcSelected.add('research:' + r.id);
            else _arcSelected.delete('research:' + r.id);
            _updateArcCount();
          }
          return;
      ...
      card.querySelector('._arc-res-menu').addEventListener('click', (e) => {
        e.stopPropagation();
        _showLibDropdown(
          e.currentTarget,
          [
            {
              ...
              action: () => {
                const a = document.createElement('a');
                a.href = '/api/research/report/' + r.id;
                a.target = '_blank';
                a.rel = 'noopener';
                document.body.appendChild(a);
                a.click();
            ...
            },
            {
              ...
              action: async () => {
                await fetch(
                  '/api/research/' + r.id + '/archive?archived=false',
                  { method: 'POST', credentials: 'same-origin' },
                );
                _renderLibArchive();
              },
            },
            {
              ...
              action: async () => {
                if (
                  !(await window.styledConfirm('Delete this research?', {
                    confirmText: 'Delete',
                    danger: true,
                  }))
                )
  ...
  function _renderArcChips() {
    ...
    const mk = (label, val, count) => {
      const c = document.createElement('button');
      c.className =
        'memory-cat-chip' + (_arcTypeFilter === val ? ' active' : '');
      c.textContent = label + ' (' + count + ')';
      c.addEventListener('click', () => {
        _arcTypeFilter = _arcTypeFilter === val ? '' : val;
        _renderArcGrid();
        _renderArcChips();
      });
      el.appendChild(c);
  ...
  function _updateArcCount() {
    const el = document.getElementById('doclib-arc-selected-count');
    if (el) el.textContent = _arcSelected.size + ' Selected';
  }

  // Archive event listeners
  document.getElementById('doclib-arc-sort').addEventListener('change', (e) => {
    _arcSort = e.target.value;
    _renderArcGrid();
  });
  document
    .getElementById('doclib-arc-search')
    .addEventListener('input', (e) => {
      _arcSearch = e.target.value.trim();
      _renderArcGrid();
    });
  document
    .getElementById('doclib-arc-select-btn')
    .addEventListener('click', () => {
      _arcSelectMode = !_arcSelectMode;
      _arcSelected.clear();
      document
        .getElementById('doclib-arc-bulk')
        .classList.toggle('hidden', !_arcSelectMode);
      _renderArcGrid();
    });
  document
    .getElementById('doclib-arc-bulk-cancel')
    ?.addEventListener('click', () => {
  ...
  function _arcToggleAll() {
    ...
    cbs.forEach((cb) => {
      ...
      if (k) {
        if (newState) _arcSelected.add(k);
        else _arcSelected.delete(k);
      }
    });
    _updateArcCount();
  }
  document
    .getElementById('doclib-arc-select-all')
    .addEventListener('change', _arcToggleAll);
  ...
  function _arcRestoreOne(key) {
    ...
    if (type === 'documents')
      return fetch(
        API_BASE + '/api/document/' + id + '/archive?archived=false',
        { method: 'POST', credentials: 'same-origin' },
      );
    if (type === 'research')
      return fetch('/api/research/' + id + '/archive?archived=false', {
        method: 'POST',
        credentials: 'same-origin',
      });
    return fetch(API_BASE + '/api/session/' + id + '/unarchive', {
      method: 'POST',
      credentials: 'same-origin',
    });
  }
  function _arcDeleteOne(key) {
    const i = key.indexOf(':'),
      ...
      id = key.slice(i + 1);
    if (type === 'documents')
      return fetch(API_BASE + '/api/document/' + id, {
        method: 'DELETE',
        credentials: 'same-origin',
      });
    if (type === 'research')
  ...
  document
    .getElementById('doclib-arc-bulk-restore')
    .addEventListener('click', async () => {
  ...
  document
    .getElementById('doclib-arc-bulk-delete')
    .addEventListener('click', async () => {
      const count = _arcSelected.size;
      if (!count) return;
      if (
        !(await window.styledConfirm(
          `Delete ${count} archived item${count > 1 ? 's' : ''} permanently?`,
          { confirmText: 'Delete', danger: true },
        ))
      )
        return;
  ...
  async function _renderLibResearch() {
    ...
    grid.innerHTML = '';
    try {
      const _spm = (await import('$lib/legacy/spinner.js')).default;
      const _sp = _spm.createWhirlpool(22);
      _sp.element.style.cssText = 'margin:18px auto;display:block;';
      grid.appendChild(_sp.element);
    } catch {
      grid.innerHTML = '<div class="hwfit-loading">Loading…</div>';
    }
    try {
      const res = await fetch(
        '/api/research/library' +
          (_researchArchivedView ? '?archived=true' : ''),
        { credentials: 'same-origin' },
      );
      if (!res.ok) throw new Error(res.statusText);
      const data = await res.json();
  ...
  async function _toggleResearchPreview(card, item) {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/research/detail/${item.id}`, {
        credentials: 'same-origin',
      });
    ...
    preview.innerHTML =
      ...
      (summaryHtml ||
        sourcesHtml ||
        '<div style="opacity:0.4;font-size:11px;padding:6px 4px;">No preview available</div>') +
    ...
    if (discussBtn)
      discussBtn.addEventListener('click', async (e) => {
        ...
        try {
          ...
          const res = await fetch(`${API_BASE}/api/research/spinoff/${_sid}`, {
            method: 'POST',
            credentials: 'same-origin',
          });
          if (!res.ok) {
          ...
          if (uiModule)
            uiModule.showError(
              'Could not start discussion: ' + (err.message || err),
            );
        }
      });
    const openBtn = preview.querySelector('.doclib-chat-open-btn');
    ...
    if (delBtn)
      delBtn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const ok =
          uiModule && uiModule.styledConfirm
            ? await uiModule.styledConfirm('Delete this research report?', {
                confirmText: 'Delete',
                danger: true,
              })
        ...
        if (!ok) return;
        try {
          const res = await fetch(`${API_BASE}/api/research/${item.id}`, {
            method: 'DELETE',
            credentials: 'same-origin',
          });
          ...
          if (uiModule && uiModule.showError)
            uiModule.showError('Failed to delete: ' + err.message);
        }
      });
    const arcBtn = preview.querySelector('.doclib-chat-archive-btn');
    if (arcBtn)
      arcBtn.addEventListener('click', async (e) => {
        e.stopPropagation();
        ...
        try {
          await fetch(
            `${API_BASE}/api/research/${item.id}/archive?archived=${toArchived}`,
            { method: 'POST', credentials: 'same-origin' },
          );
          if (fromArchiveTab) {
            _renderLibArchive();
          ...
          if (uiModule)
            uiModule.showToast(toArchived ? 'Archived' : 'Restored');
        } catch {
          if (uiModule)
            uiModule.showError(
              'Failed to ' + (toArchived ? 'archive' : 'restore'),
            );
        }
      });
  }
  ...
  function _renderResearchGrid() {
    ...
    if (_rsb) {
      _rsb.classList.toggle('active', _researchSelectMode);
      _rsb.textContent = _researchSelectMode ? 'Cancel' : 'Select';
    }
    ...
    // Sort
    const _rSort =
      document.getElementById('doclib-research-sort')?.value || 'recent';
    if (_rSort === 'recent')
      items.sort((a, b) => (b.completed_at || 0) - (a.completed_at || 0));
    else if (_rSort === 'oldest')
      items.sort((a, b) => (a.completed_at || 0) - (b.completed_at || 0));
    else if (_rSort === 'most-sources')
      items.sort((a, b) => (b.source_count || 0) - (a.source_count || 0));
    else if (_rSort === 'alpha')
      items.sort((a, b) => (a.query || '').localeCompare(b.query || ''));
    if (stats)
      stats.textContent =
        items.length + ' research' + (items.length !== 1 ? 'es' : '');
    if (!items.length) {
      grid.innerHTML =
        ...
        '</div>';
      grid
        .querySelector('[data-doclib-open-research]')
        ?.addEventListener('click', (e) => {
          e.preventDefault();
    ...
    for (const r of items) {
      const date = r.completed_at
        ? new Date(r.completed_at * 1000).toLocaleDateString(undefined, {
            month: 'short',
            day: 'numeric',
            year: 'numeric',
          })
      ...
      const time = r.completed_at
        ? new Date(r.completed_at * 1000).toLocaleTimeString(undefined, {
            hour: '2-digit',
            minute: '2-digit',
          })
        : '';
      ...
      if (_researchSelectMode)
        html += `<input type="checkbox" class="memory-select-cb _res-cb" data-rid="${r.id}"${selected ? ' checked' : ''}>`;
      ...
      html += `</div>`;
      if (!_researchSelectMode)
        html += `<div class="memory-item-actions"><button class="memory-item-btn doclib-research-delete" data-rid="${r.id}" title="Delete"><svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg></button></div>`;
      html += `</div>`;
    ...
    grid.querySelectorAll('._res-cb').forEach((cb) => {
      cb.addEventListener('click', (e) => e.stopPropagation());
      cb.addEventListener('change', () => {
        if (cb.checked) _researchSelected.add(cb.dataset.rid);
        else _researchSelected.delete(cb.dataset.rid);
    ...
    grid.querySelectorAll('.doclib-research-card').forEach((card) => {
      card.addEventListener('click', (e) => {
        ...
        if (
          e.target.closest('.doclib-research-delete') ||
          e.target.closest('._res-cb') ||
          e.target.closest('.doclib-chat-open-btn')
        )
          return;
      ...
      btn.addEventListener('click', (e) => {
        ...
        _showLibDropdown(
          ...
          [
            ...
            {
              label: _researchArchivedView ? 'Restore' : 'Archive',
              action: async () => {
                ...
                try {
                  await fetch(
                    '/api/research/' + rid + '/archive?archived=' + toArchived,
                    { method: 'POST', credentials: 'same-origin' },
                  );
                } catch {}
          ...
          ],
          {
            onSelect: () => {
              ...
              document
                .getElementById('doclib-research-bulk')
                ?.classList.remove('hidden');
              _renderResearchGrid();
            },
          },
  ...
  if (researchSortEl)
    researchSortEl.addEventListener('change', () => _renderResearchGrid());
  ...
  document
    .getElementById('doclib-research-select-btn')
    ?.addEventListener('click', () => {
      _researchSelectMode = !_researchSelectMode;
      _researchSelected.clear();
      document
        .getElementById('doclib-research-bulk')
        .classList.toggle('hidden', !_researchSelectMode);
      _renderResearchGrid();
    });
  ...
  document
    .getElementById('doclib-research-tidy-btn')
      ...
      try {
        ...
        const results = await Promise.all(
          needFetch.map(async (r) => {
            try {
              const res = await fetch('/api/research/detail/' + r.id, {
                credentials: 'same-origin',
              });
              if (!res.ok) return null;
  ...
  document
    .getElementById('doclib-research-bulk-cancel')
    ?.addEventListener('click', () => {
      _researchSelectMode = false;
      _researchSelected.clear();
      document.getElementById('doclib-research-bulk').classList.add('hidden');
  ...
  // Research select all
  document
    .getElementById('doclib-research-select-all')
    ?.addEventListener('change', () => {
      const allCb = document.getElementById('doclib-research-select-all');
      const newState = allCb?.checked;
      _researchItems.forEach((r) => {
        if (newState) _researchSelected.add(r.id);
        else _researchSelected.delete(r.id);
      });
      _updateResearchCount();
      _renderResearchGrid();
    });
  ...
  document
    .getElementById('doclib-research-bulk-delete')
    ?.addEventListener('click', async () => {
      const count = _researchSelected.size;
      if (!count) return;
      if (
        !(await window.styledConfirm(
          `Delete ${count} research report${count > 1 ? 's' : ''} permanently?`,
          { confirmText: 'Delete', danger: true },
        ))
      )
        return;
      ...
      await new Promise((r) => setTimeout(r, 250));
      await Promise.all(
        [..._researchSelected].map((rid) =>
          fetch('/api/research/' + rid, {
            method: 'DELETE',
            credentials: 'same-origin',
          }),
        ),
      );
      _researchItems = _researchItems.filter(
        (r) => !_researchSelected.has(r.id),
      );
      _researchSelected.clear();
      _researchSelectMode = false;
      document.getElementById('doclib-research-bulk').classList.add('hidden');
  ...
  document
    .getElementById('doclib-research-bulk-archive')
    ?.addEventListener('click', async () => {
      const count = _researchSelected.size;
      if (!count) return;
      const toArchived = !_researchArchivedView;
      ...
      await Promise.all(
        [..._researchSelected].map((rid) =>
          fetch('/api/research/' + rid + '/archive?archived=' + toArchived, {
            method: 'POST',
            credentials: 'same-origin',
          }),
        ),
  ...
  if (importFileBtn && fileInput) {
    ...
    fileInput.addEventListener('change', async () => {
      ...
      try {
        ...
        _sp.element.style.cssText =
          'width:12px;height:12px;margin:0 4px 0 0;display:inline-block;vertical-align:middle;position:relative;top:-2px;';
  ...
  if (archivedBtn)
    archivedBtn.addEventListener('click', () => {
      ...
      archivedBtn.title = _libraryArchivedView
  ...
  if (tidyBtn)
    tidyBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        try {
          const res2 = await fetch(`${API_BASE}/api/documents/ai-tidy`, {
            method: 'POST',
          });
          if (res2.ok) {
        ...
        if (totalDeleted === 0 && totalFixed === 0) {
          ...
          const msg =
            aiMessage ||
            `Removed ${totalDeleted} document${totalDeleted !== 1 ? 's' : ''}`;
  ...
  if (bulkActionsBtn)
    bulkActionsBtn.addEventListener('click', (e) => {
      ...
      _showLibDropdown(
        ...
        [
          {
            label: _libraryArchivedView ? 'Restore' : 'Archive',
            icon: _libraryArchivedView ? 'restore' : 'archive',
            action: libraryBulkArchive,
          },
  ...
  if (bulkCancelBtn)
    bulkCancelBtn.addEventListener('click', libraryExitSelectMode);
  ...
  _libraryEscHandler = (e) => {
    if (e.key === 'Escape') {
      ...
      const expanded = document.querySelector(
        '#doclib-grid .doclib-card-expanded',
      );
      if (expanded) {
...
export function closeLibrary() {
  ...
  if (modal) {
    const content = modal.querySelector(
      '.modal-content, .doclib-modal-content',
    );
    if (content) {
