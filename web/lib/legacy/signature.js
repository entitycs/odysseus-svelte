...
export function setLastUsed(sig) {
  _lastUsed = sig || null;
}
...
function _modal(innerHtml) {
  ...
  overlay.style.cssText =
    'pointer-events:auto;background:rgba(0,0,0,0.45);z-index:10100;';
...
export function capture(opts = {}) {
  return new Promise((resolve) => {
    ...
    overlay.querySelector('.sig-clear').onclick = () => {
      pad.clear();
      refreshSaveBtn();
    };
    overlay.querySelector('.sig-undo').onclick = () => {
      pad.undo();
      refreshSaveBtn();
    };
  ...
  return new Promise(async (resolve) => {
    ...
    overlay.querySelectorAll('.sig-tile-del').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        if (
          !(await window.styledConfirm('Delete this signature?', {
            confirmText: 'Delete',
            danger: true,
          }))
        )
