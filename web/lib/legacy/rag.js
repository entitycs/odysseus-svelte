...
export function init() {
  initLegacy(window.location.origin);
}
  ...
  try {
    ...
    if (files.length === 0) {
      ...
      placeholder.style.cssText =
        'color:var(--color-muted);font-size:12px;padding:4px 0;';
    ...
    files.forEach((f) => {
      ...
      size.style.cssText =
        'color:var(--color-muted);font-size:11px;flex-shrink:0;';
      ...
      del.style.cssText =
        'background:none;border:none;color:var(--color-error);cursor:pointer;padding:2px 4px;font-size:12px;flex-shrink:0;';
...
async function _deleteFile(filepath, displayName) {
  if (
    !(await uiModule.styledConfirm(`Remove "${displayName}" from RAG?`, {
      confirmText: 'Remove',
      danger: true,
    }))
  )
  ...
  try {
    const res = await fetch(
      `${API_BASE}/api/personal/file?filepath=${encodeURIComponent(filepath)}`,
      {
        method: 'DELETE',
        credentials: 'same-origin',
      },
