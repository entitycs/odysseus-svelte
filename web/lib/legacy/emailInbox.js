...
import {
  buildReplyAllCc,
  extractEmail,
} from '$lib/legacy/emailLibrary/replyRecipients.js';
import {
  closeEmailLibrary,
  initEmailLibrary,
  isOpen as isLibOpen,
  openEmailLibrary,
  prewarmEmailLibrary,
} from '$lib/legacy/emailLibrary.js';
import { emailApiUrl, emailAccountQuery } from '$lib/legacy/emailShared.js';
import { bindMenuDismiss, dismissOrRemove } from '$lib/legacy/escMenuStack.js';
import * as Modals from '$lib/legacy/modalManager.js';
import { applyEdgeDock } from '$lib/legacy/modalSnap.js';
...
const _emailSetupHint = () =>
  '<div style="margin-top:6px;opacity:0.72;font-size:11px;">Setup: <span style="color:var(--accent,var(--red));">Settings &rsaquo; Integrations</span></div>';
...
export function init() {
  ...
  window.addEventListener('email-answered', (e) => {
    ...
    document
      .querySelectorAll(
        '.email-item[data-uid="' + CSS.escape(String(uid)) + '"]',
      )
      .forEach((item) => {
...
function _withoutMyAddresses(raw, myAddresses) {
  return _splitEmailAddresses(raw).filter(
    (addr) => !_isMyEmailAddress(addr, myAddresses),
  );
}
...
function _openEmailTagFilter(tag) {
  ...
  setTimeout(() => {
    document.dispatchEvent(
      new CustomEvent('odysseus:email-filter-tag', {
...
function _emailTagPillHtml(tag, em) {
  ...
  const eventUid =
    normalized === 'calendar' && Array.isArray(em?.calendar_event_uids)
...
function _emailTagGroupHtml(tags, em) {
  ...
  if (visible.length === 1)
    return `<span class="email-tags">${visible[0]}</span>`;
  const extra = visible
    .slice(1)
    .map((html) => `<span class="email-tag-extra">${html}</span>`)
    .join('');
...
export async function openReplyDraft(
  uid,
  folder = 'INBOX',
  mode = 'reply',
  prefilledBody = '',
...
function _watchDocOpenToReDockEmail() {
  ...
  _docOpenObs = new MutationObserver(() => {
    ...
    if (cur && !last) {
      if (window.innerWidth > 768) {
        ...
        document
          .querySelectorAll('.modal[id^="email-reader-"]')
          .forEach((m) => {
            if (m.classList.contains('hidden')) return;
            if (m.classList.contains('modal-left-docked')) return;
            try {
              applyEdgeDock(m, 'left');
            } catch (_) {}
          });
      }
    }
  ...
  _docOpenObs.observe(document.body, {
    attributes: true,
...
async function _refreshUnreadCount() {
  ...
  try {
    ...
    const [listRes, urgRes] = await Promise.all([
      fetch(
        `${API_BASE}/api/email/list?folder=INBOX&limit=50&filter=unread${_acct()}`,
      ),
      fetch(`${API_BASE}/api/email/urgency-state`, {
    ...
    const lastSeen = parseInt(
      localStorage.getItem('odysseus-email-last-seen-uid') || '0',
      10,
    );
      ...
      .then((data) => {
        ...
        if (emails.length > 0) {
          const maxUid = Math.max(
            ...emails.map((e) => parseInt(e.uid, 10) || 0),
          );
...
export async function loadEmails(append = false) {
  ...
  try {
    ...
    if (!append && list) {
      const msg =
        e && e.message ? `Failed to load: ${e.message}` : 'Failed to load';
...
export function sortedFolders(folders) {
  const roleOf = (folder) => {
    ...
    if (f.includes('trash') || f.includes('bin') || f.includes('deleted'))
      return 'trash';
    return '';
  };
  const roleOrder = [
  ...
  return {
    priority: roleOrder.map((role) => found.get(role)).filter(Boolean),
    others,
  };
}
...
function _renderList() {
  ...
  if (_senderFilter) {
    ...
    chip
      .querySelector('.email-filter-chip-clear')
      .addEventListener('click', () => _clearSenderFilter());
  ...
  if (_emails.length === 0) {
    ...
    empty.textContent = _senderFilter
      ? `No emails from ${_senderFilterLabel || _senderFilter}`
      : 'No emails';
...
function _createEmailItem(em) {
  ...
  item.className =
    'list-item email-item' + (em.is_spam_verdict ? ' email-item-spam' : '');
    ...
    if (us && us.per_uid && em.uid != null) {
      ...
      for (const k of Object.keys(us.per_uid)) {
        if (k.endsWith(suffix)) {
          ...
          if (score >= 3) {
            _unreadColor = 'var(--color-error, #e06c75)';
            _unreadTitle = 'Urgent — ' + (v.reason || 'needs reply now');
          } else if (score === 2) {
            _unreadColor = '#f0ad4e';
            _unreadTitle = 'Reply soon — ' + (v.reason || '');
          }
          break;
        }
      }
...
async function _openEmail(em, itemEl, preloadedData = null, mode = 'reply', noteHint = '', prefilledBody = '', mailboxContext = null) {
  ...
  let aiSuggestedBody =
    typeof prefilledBody === 'string' && prefilledBody.trim()
      ? prefilledBody.trim()
      : null;
  if (wantsAiReply) {
  ...
  try {
    ...
    if (wantsAiReply) {
      ...
      if (data.cached_ai_reply && !noteHint && !activeReplyAccount) {
        ...
        draftToastTimer = setTimeout(() => {
          import('$lib/legacy/ui.js')
            .then(
              (m) =>
                m.showToast &&
                m.showToast('Drafting AI reply', {
                  duration: 3000,
                  leadingIcon: 'spinner',
                }),
            )
        ...
        try {
          ...
          if (result.success && result.reply) {
            ...
            import('$lib/legacy/ui.js')
              .then(
                (m) => m.showError && m.showError('AI reply failed: ' + _msg),
              )
              .catch(() => {});
            return;
          }
          ...
          import('$lib/legacy/ui.js')
            .then(
              (m) =>
                m.showError &&
                m.showError('AI reply failed: ' + (e.message || e)),
            )
            .catch(() => {});
          return;
        }
    ...
    const myAddresses =
      Array.isArray(window._myEmailAddresses) && window._myEmailAddresses.length
        ? window._myEmailAddresses
        : window._myEmailAddress
          ? [window._myEmailAddress]
          : [];
    ...
    let toAddress = fromIsMe
      ? originalToWithoutMe.join(', ') ||
        originalCcWithoutMe[0] ||
        data.from_address
      : data.from_address;
    let ccAddresses = '';
    ...
    if (mode === 'reply-all') {
      if (fromIsMe) {
        ...
        toAddress =
          ...
          originalCcWithoutMe[0] ||
          data.from_address;
        ccAddresses = originalCcWithoutMe
          .filter(
            (addr) =>
              !originalToWithoutMe.some(
                (t) => extractEmail(t) === extractEmail(addr),
              ),
          )
          .join(', ');
      } else {
    ...
    if (mode !== 'forward' && data.message_id)
    ...
    let _origBody =
      typeof data.body === 'string' && data.body.length ? data.body : '';
      ...
      if (existingDocId) {
        ...
        if (!activeSid) {
          ...
          import('./ui.js')
            .then(
              (m) =>
                m.showError && m.showError('Could not start a reply chat.'),
            )
            .catch(() => {});
          return;
        }
          ...
          import('$lib/legacy/ui.js')
            .then(
              (m) =>
                m.showError &&
                m.showError(
                  'Failed to create reply draft (' + docRes.status + ')',
                ),
            )
            .catch(() => {});
          return;
    ...
    import('$lib/legacy/ui.js')
      .then((m) => m.showError && m.showError('Reply failed: ' + msg))
      .catch(() => {});
  } finally {
    if (spinner) {
      spinner.destroy();
...
function _showEmailMenu(em, anchor, itemEl) {
  ...
  const close = bindMenuDismiss(
    ...
    (ev) => !dropdown.contains(ev.target) && !anchor.contains(ev.target),
  );
}
...
function _showRemindSubmenu(em, parentDropdown) {
  ...
  header.style.cssText =
    'opacity:0.5;font-size:10px;pointer-events:none;text-transform:uppercase;letter-spacing:0.5px;padding-top:6px;';
  ...
  const presets = [
    {
      label: 'Later today',
      sub: laterToday.toLocaleTimeString([], {
        hour: 'numeric',
        minute: '2-digit',
      }),
      date: laterToday,
    ...
    {
      label: 'Tomorrow',
      sub: tomorrow.toLocaleTimeString([], {
        hour: 'numeric',
        minute: '2-digit',
      }),
      date: tomorrow,
    },
    {
      label: 'Next week',
      sub:
        nextWeek.toLocaleDateString([], { weekday: 'short' }) +
        ' ' +
        nextWeek.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }),
      date: nextWeek,
    },
  ];
  ...
  customItem.addEventListener('click', async (e) => {
    ...
    tmp.style.cssText =
      'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:99999;padding:8px;background:var(--bg);border:1px solid var(--border);border-radius:6px;font-size:13px;';
...
async function _createReplyReminder(em, dueDate) {
  ...
  try {
    ...
    const { showToast } = await import('$lib/legacy/ui.js');
    ...
    const { showError } = await import('$lib/legacy/ui.js');
...
async function _archiveEmail(em) {
  try {
    await fetch(
      `${API_BASE}/api/email/archive/${em.uid}?folder=${encodeURIComponent(_currentFolder)}${_acct()}`,
      { method: 'POST' },
    );
...
async function _deleteEmail(em) {
  ...
  const { styledConfirm } = await import('$lib/legacy/ui.js');
  const ok = await styledConfirm(`Delete "${subject}"?`, {
    confirmText: 'Delete',
    cancelText: 'Cancel',
    danger: true,
  });
  ...
  try {
    await fetch(
      `${API_BASE}/api/email/delete/${em.uid}?folder=${encodeURIComponent(_currentFolder)}${_acct()}`,
      { method: 'DELETE' },
    );
    _emails = _emails.filter((e) => e.uid !== em.uid);
    _renderList();
...
async function _toggleDone(em, itemEl) {
  ...
  if (itemEl) {
    if (newState) {
      ...
      itemEl
        .querySelectorAll('.email-unread-dot, [data-unread-dot]')
        .forEach((n) => n.remove());
    }
  ...
  try {
    if (newState) {
      await fetch(
        `${API_BASE}/api/email/mark-answered/${em.uid}?folder=${encodeURIComponent(_currentFolder)}${_acct()}`,
        { method: 'POST' },
      );
      await fetch(
        `${API_BASE}/api/email/mark-read/${em.uid}?folder=${encodeURIComponent(_currentFolder)}${_acct()}`,
        { method: 'POST' },
      );
    } else {
      await fetch(
        `${API_BASE}/api/email/clear-answered/${em.uid}?folder=${encodeURIComponent(_currentFolder)}${_acct()}`,
        { method: 'POST' },
      );
    }
  } catch (e) {
    ...
    if (!sid) {
      ...
      import('$lib/legacy/ui.js')
        .then(
          (m) =>
            m.showError &&
            m.showError('Could not start a new email (no session).'),
        )
        .catch(() => {});
      return;
    }
    ...
    if (!res.ok) {
      ...
      import('$lib/legacy/ui.js')
        .then(
          (m) =>
            m.showError &&
            m.showError('Failed to create new email (' + res.status + ')'),
        )
        .catch(() => {});
      return;
    }
