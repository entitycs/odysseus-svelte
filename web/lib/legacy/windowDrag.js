...
import { makeEdgeDockController } from '$lib/legacy/modalSnap.js';
import { makeWindowResizable } from '$lib/legacy/windowResize.js';
...
export function makeWindowDraggable(modal, options = {}) {
  ...
  const leftDock =
    enableDock && options.enableLeftDock !== false
      ? makeEdgeDockController(modal, 'left')
      : null;
  ...
  const _onMove = (cx, cy) => {
    ...
    if (
      Math.abs(cx - startX) > MOVE_THRESHOLD ||
      Math.abs(cy - startY) > MOVE_THRESHOLD
    ) {
  ...
  header.addEventListener('mousedown', (e) => {
    ...
    const onUp = (ev) => {
      ...
      if (movedDuringDrag) {
        ...
        header.addEventListener('click', swallow, {
          capture: true,
          once: true,
        });
        // Safety: if no click fires (some browsers), drop the listener.
        setTimeout(
          () => header.removeEventListener('click', swallow, { capture: true }),
          50,
        );
      }
    };
