...
const _MEMORY_SORT_ICONS = {
  ...
  oldest:
    '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 3-6.7L3 8"/><polyline points="3 3 3 8 8 8"/><polyline points="12 7 12 12 16 14"/></svg>',
  alpha:
    '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 4h6"/><path d="M3 10h6"/><path d="M3 16h4"/><path d="M14 4l4 12"/><path d="M16 12h4"/><polyline points="17 18 21 14 17 10"/><line x1="21" y1="14" x2="13" y2="14"/></svg>',
...
function buildCategoryChips() {
  ...
  sorted.forEach((cat) => {
    ...
    btn.className =
      'memory-cat-chip' + (cat === activeCategory ? ' active' : '');
    btn.dataset.cat = cat;
    btn.textContent = cat;
...
async function syncToggles() {
  ...
  await syncPrefToggle(
    'auto-memory-toggle',
    'auto_memory',
    'Auto-extract memories enabled',
    'Auto-extract memories disabled',
    false,
  );
  await syncPrefToggle(
    'auto-skills-toggle',
    'auto_skills',
    'Auto-extract skills enabled',
    'Auto-extract skills disabled',
    false,
  );
  await syncPrefToggle(
    'auto-approve-skills-toggle',
    'auto_approve_skills',
    'Auto-approve skills enabled',
    'Auto-approve skills disabled',
    false,
  );
  await syncPrefSlider(
  ...
  if (headerToggle) {
    ...
    if (!headerToggle.dataset.boundUx) {
      ...
      headerToggle.addEventListener('change', () => {
        if (modalBody)
          modalBody.style.opacity = headerToggle.checked ? '' : '0.3';
        reflectMemoryToggleInSidebar(headerToggle.checked);
      });
    }
  }
  ...
  if (skillsToggle) {
    ...
    const applyDim = () => {
      if (skillsPanel)
        skillsPanel.style.opacity = skillsToggle.checked ? '' : '0.3';
    };
    applyDim();
    ...
    slider.addEventListener('change', async () => {
      ...
      try {
        const res = await fetch(
          `${window.location.origin}/api/prefs/${prefKey}`,
          {
            method: 'PUT',
...
function enterSelectMode() {
  ...
  if (selectBtn) {
    ...
    selectBtn.innerHTML = _SELECT_BTN_X_SVG + 'Cancel';
  }
  updateBulkCount();
  renderMemoryList();
}
...
function exitSelectMode() {
  ...
  if (selectBtn) {
    ...
    selectBtn.innerHTML = _SELECT_BTN_DOT_SVG + 'Select';
  }
...
async function bulkDelete() {
  ...
  for (const id of selectedIds) {
    try {
      const res = await fetch(`${window.location.origin}/api/memory/${id}`, {
        method: 'DELETE',
      });
      if (res.ok) {
        deleted++;
  ...
  try {
    ...
    if ((data.removed || 0) === 0) {
      ...
      if (tidyBtn) {
        ...
        tidyBtn.innerHTML =
          '<svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" style="vertical-align:-1px;margin-right:2px;color:var(--accent, var(--red));"><path d="M12 0L14.59 8.41L23 12L14.59 15.59L12 24L9.41 15.59L1 12L9.41 8.41Z"/></svg> Tidy';
      }
    ...
    showToast(
      `Tidied: ${data.removed} removed (${data.before} \u2192 ${data.after})`,
    );
  } catch (error) {
...
async function animateMemoryRemoval(ids) {
  ...
  const items = Array.from(
    memoryList.querySelectorAll('.memory-item[data-memory-id]'),
...
function getFilteredMemories() {
  const searchTerm =
    document.getElementById('memory-search')?.value?.toLowerCase().trim() || '';

  let filtered = searchTerm
    ? memories.filter(
        (m) => m.text && m.text.toLowerCase().includes(searchTerm),
      )
    : [...memories];

  if (activeCategory !== 'all') {
    filtered = filtered.filter(
      (m) => (m.category || 'fact') === activeCategory,
    );
  }
...
export function renderMemoryList() {
  ...
  if (filtered.length === 0) {
    ...
    if (searchTerm || activeCategory !== 'all') {
      ...
      memoryList
        .querySelector('[data-mem-goto-add]')
  ...
  filtered.forEach((memory) => {
    ...
    if (selectMode) {
      ...
      cb.addEventListener('change', () => {
        ...
        const selectAllEl = document.getElementById('memory-select-all');
        if (selectAllEl)
          selectAllEl.checked = filtered.every((m) => selectedIds.has(m.id));
      });
      item.appendChild(cb);
    ...
    if (!selectMode) {
      ...
      selectItem.innerHTML =
        '<span class="dropdown-icon"><span style="font-size:16px;line-height:1;">●</span></span><span>Select</span>';
      ...
      menuBtn.addEventListener('click', (e) => {
        ...
        document
          .querySelectorAll('.memory-item-dropdown')
          .forEach((d) => d.remove());
        ...
        if (dr.left < 6) {
          dropdown.style.right =
            Math.max(6, window.innerWidth - 6 - dr.width) + 'px';
        }
        const dr2 = dropdown.getBoundingClientRect();
        if (dr2.bottom > window.innerHeight - 6) {
          dropdown.style.maxHeight =
            Math.max(80, window.innerHeight - 12 - dr2.top) + 'px';
      ...
      // checkbox, or another button (those have their own click handlers).
      {
        ...
        item.addEventListener('pointerdown', (e) => {
          if (
            e.target.closest(
              '.memory-menu-btn, .memory-select-cb, button, input',
            )
          )
            return;
...
function startInlineEdit(item, memory) {
  ...
  saveBtn.addEventListener('click', () =>
    saveInlineEdit(memory.id, input.value, catSelect.value),
  );
...
async function saveInlineEdit(id, newText, newCategory) {
  ...
  const catChanged =
    newCategory && newCategory !== (memory?.category || 'fact');
...
export function updateMemoryCount() {
  ...
  if (h2Count)
    h2Count.textContent = `${num} ${scopeTotal === 1 && visible.length === scopeTotal ? 'memory' : 'memories'}`;
...
export async function deleteMemory(id) {
  ...
  if (
    !(await uiModule.styledConfirm(`Delete this memory?\n"${memory.text}"`, {
      confirmText: 'Delete',
      danger: true,
    }))
  )
...
export async function extractMemory(sessionId) {
  ...
  if (suggestions.length === 0) {
    body.innerHTML =
      '<div class="memory-empty">No useful information detected.</div>';
  } else {
...
async function handleImportFile(file) {
  ...
  if (importBtn) {
    ...
    importSpin.element.style.cssText =
      'width:12px;height:12px;margin:0 5px 0 0;display:inline-flex;vertical-align:-2px;transform:translateY(-1px);';
  ...
  try {
    ...
    if (suggestions.length === 0) {
      body.innerHTML =
        '<div class="memory-empty">No useful information found in file.</div>';
    } else {
      ...
      saveAllBtn.addEventListener('click', async () => {
        ...
        await loadMemories();
        document
          .querySelector('.memory-tab[data-memory-tab="browse"]')
          ?.click();
