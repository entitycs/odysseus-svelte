...
function _displayHistoryContent(content) {
  ...
  const head = text.slice(
    0,
    HISTORY_DISPLAY_CHAR_LIMIT - HISTORY_DISPLAY_TAIL_CHARS,
  );
...
function _renderHistoryMessage(msg, modelName) {
  ...
  if (typeof msg.content === 'string') {
    ...
    displayContent = _displayHistoryContent(
      msg.content
  ...
  return _addHistoryMessageWithFullRenderer(
    msg.role,
    displayContent,
    modelName,
    meta,
  ...
  const loadOlder = async () => {
    ...
    try {
      const res = await fetch(
        _historyUrl(_historyPager.sessionId, {
          limit: nextLimit,
          offset: nextOffset,
        }),
      );
      const data = await res.json();
      if (!_historyPager || _historyPager.sessionId !== currentSessionId)
        return;
      const newEls = [];
...
function _markIncognito(sid) {
  ...
  if (!ids.includes(sid)) {
    ids.push(sid);
    sessionStorage.setItem(_INCOGNITO_SESSIONS_KEY, JSON.stringify(ids));
  }
}
function _isIncognitoSession(sid) {
  return _getIncognitoIds().includes(sid);
}
  ...
  try {
    ...
    if (savedOrder) {
      ...
      if (Array.isArray(orderIds) && orderIds.some((x) => String(x) === id)) {
        Storage.set(
          'session-order',
          JSON.stringify(orderIds.filter((x) => String(x) !== id)),
        );
      }
    }
  } catch (e) {
...
function createSessionItem(s) {
  ...
  const _isFork =
    s.name && (s.name.startsWith('Fork:') || s.name.startsWith('\u2ADD'));
  ...
  if (_isGroup) {
    ...
    icon.innerHTML =
      '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>';
  } else {
    icon.innerHTML =
  ...
  if (s.is_important && !isOpenClaw) {
    ...
    icon.innerHTML =
      '<svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>';
    icon.addEventListener('click', async (e) => {
      e.stopPropagation();
      const fd = new FormData();
      fd.append('important', false);
      await fetch(`${API_BASE}/api/session/${s.id}/important`, {
        method: 'POST',
        body: fd,
      });
  ...
  if (_isFork)
    chatTitle = chatTitle.replace(/^Fork:\s*/, '').replace(/^\u2ADD\s*/, '');
        ...
        if (dd) {
          ...
          requestAnimationFrame(() => {
            ...
            if (mr.bottom > window.innerHeight - 8)
              dd.style.top = rect.top - mr.height - 4 + 'px';
  ...
  div.addEventListener('click', (e) => {
    if (
      ...
      e.target.closest('.session-rename-input') ||
      e.target.closest('.session-select-cb')
    )
      return;
    if (_touchMoved || _longPressed) {
      _touchMoved = false;
      _longPressed = false;
      return;
    }
  ...
  deleteItem.innerHTML =
    _icon(_deleteIcon) +
    '<span>Delete</span><span class="dropdown-shortcut">' +
    _mod +
    '+Alt+D</span>';
  ...
  if (!isOpenClaw) {
    ...
    starItem.innerHTML =
      _icon(_favIcon) +
      '<span>' +
      (s.is_important ? 'Unfavorite' : 'Favorite') +
      '</span><span class="dropdown-shortcut">' +
      _mod +
      '+Alt+F</span>';
  ...
  if (!isOpenClaw) {
    ...
    selectMoreItem.innerHTML =
      _icon('<span style="font-size:16px;line-height:1;">●</span>') +
      '<span>Select</span>';
  ...
  _sep.style.cssText =
    'height:1px;margin:3px 0;background:color-mix(in srgb,var(--border) 40%,transparent)';
  ...
  renameItem.addEventListener('click', () => {
    ...
    const sessionEl = document.querySelector(
      `.list-item[data-session-id="${s.id}"]`,
    );
    if (!sessionEl) return;
  ...
  deleteItem.addEventListener('click', async () => {
    ...
    dropdown.style.display = 'none';
    if (
      !(await uiModule.styledConfirm('Delete this session?', {
        confirmText: 'Delete',
        danger: true,
      }))
    ) {
    ...
    try {
      ...
      /* network error — session may still exist server-side */
    }
    await loadSessions();
  });
  ...
  var _isProcessing =
    _researchingSessions.has(s.id) || _streamingSessions.has(s.id);
...
function _renderSessionListImpl() {
  ...
  document
    .querySelectorAll('.session-dropdown, .folder-submenu')
    .forEach((d) => d.remove());
  ...
  if (_sortMode && _sortMode !== 'group') {
    ...
    if (allFlat.length > SIDEBAR_MAX_VISIBLE) {
      ...
      toggleBtn.textContent = _showAllSessions
        ? 'Show less'
        : `Show ${remaining} more`;
  ...
  orderedFolderNames.forEach((folderName) => {
    ...
    deleteBtn.addEventListener('click', async (e) => {
      ...
      if (
        !(await uiModule.styledConfirm(
          `Delete folder "${folderName}" and all ${count} session(s) inside it?`,
    ...
    header.addEventListener('click', (e) => {
      e.stopPropagation();
      if (
        e.target.closest('.folder-drag-handle') ||
        e.target.closest('.folder-delete-btn')
      )
        return;
      if (_folderTouchMoved) {
        _folderTouchMoved = false;
        return;
      }
    ...
    header.addEventListener('dblclick', async (e) => {
      ...
      const promises = folders[folderName].map((s) =>
        moveToFolder(s.id, newName.trim()),
      );
  ...
  if (hasFolders && unfiled.length > 0) {
    ...
    deleteBtn.addEventListener('click', async (e) => {
      ...
      if (
        !(await uiModule.styledConfirm(
          `Delete all ${unfiled.length} unsorted session(s)?`,
          { confirmText: 'Delete', danger: true },
        ))
      )
        return;
...
function _postRenderSessionList(list) {
  if (currentSessionId) {
    const activeEl = document.querySelector(
      `.list-item[data-session-id="${currentSessionId}"]`,
    );
    if (activeEl) {
...
function _showSwipeHint(list) {
  if (
    ...
    !localStorage.getItem('ody-swipe-hint-shown')
  ) {
    ...
    if (firstItem) {
      ...
      hint.innerHTML =
        '<span class="swipe-hint-arrow">\u2190</span> swipe to delete';
...
function _enterSelectMode() {
  ...
  document.querySelectorAll('.list-item[data-session-id]').forEach((item) => {
    ...
    dot.innerHTML = '○';
    dot.style.cssText =
      'cursor:pointer;font-size:16px;flex-shrink:0;opacity:0.4;transition:opacity 0.1s;user-select:none;';
    dot._checked = false;
...
function _updateBulkCount() {
  ...
  if (deleteBtn) {
    ...
    deleteBtn.style.opacity = count === 0 ? '0.2' : '';
  }
}
...
function _initBulkSelect() {
  ...
  const selectFromDropdown = document.getElementById(
  ...
  if (selectAll && selectAllDot) {
    const _toggleAll = () => {
      ...
      selectAllDot.style.color = selectAll.checked
        ? 'var(--accent, var(--red))'
        : '';
  ...
  if (archiveBtn) {
    archiveBtn.addEventListener('click', async () => {
      ...
      const count = _selectedIds.size;
      if (
        !(await uiModule.styledConfirm(`Archive ${count} session(s)?`, {
          confirmText: 'Archive',
        }))
      )
        return;
  ...
  if (deleteBtn) {
    deleteBtn.addEventListener('click', async () => {
      ...
      await _animateSessionRowsRemoving(
        deletedIds,
        '#session-list .list-item[data-session-id]',
      );
      _exitSelectMode();
      if (window._suppressSidebarClose !== undefined) {
        window._suppressSidebarClose = true;
...
function _animateSessionRowsRemoving(ids, selector) {
  ...
  const rows = Array.from(
    document.querySelectorAll(selector || '.list-item[data-session-id]'),
  ).filter((row) =>
    idSet.has(String(row.dataset.sessionId || row.dataset.sid)),
  );
...
export async function loadSessions() {
  try {
    ...
    const _isTransient = (s) =>
      !!s && (s.folder === 'Assistant' || s.folder === 'Tasks');
    ...
    if (hasPendingChat) {
      ...
      currentSessionId &&
      activeSessions.some((s) => s.id === currentSessionId)
    ) {
    ...
    if (!targetId && !hasPendingChat) {
      ...
      if (activeSessions.length === 0 && !_autoCreateInProgress) {
        ...
        try {
          ...
          /* no default model — that's fine, user can /setup */
        }
...
export async function selectSession(
  id,
  { keepSidebar = false, showLoading = true, immediateLoading = false } = {},
) {
  ...
  try {
    ...
    if (prevSessionId !== id && window.documentModule?.clearSelection) {
      try {
        window.documentModule.clearSelection();
      } catch {}
    }
    currentSessionId = id;
    ...
    if (window.compareModule) {
      if (window.compareModule.isActive())
        window.compareModule.deactivate(true);
      else if (window.compareModule.hasVisibleResults())
        window.compareModule.cleanupResults();
    }
    const msgInput = document.getElementById('message');
    if (msgInput) {
      msgInput.disabled = false;
    ...
    document
      .querySelectorAll('.list-item.active-session')
      .forEach((el) => el.classList.remove('active-session'));
    const activeEl = document.querySelector(
      `.list-item[data-session-id="${id}"]`,
    );
    ...
    const hasExistingBubbles =
      chatHistory && chatHistory.querySelectorAll('.msg').length > 0;
    const wouldWipe =
      !isOC && !msgHistory.length && isSameSession && hasExistingBubbles;
    if (wouldWipe) {
    ...
    if (isOC) {
      ...
      const isDocScopedEmptySession = !!(
        meta &&
        (meta.has_documents || /^Email:|^New Email$/i.test(meta.name || ''))
      );
    ...
    if (
      window.groupModule &&
      window.groupModule.restoreState &&
      window.groupModule.restoreState(id)
    ) {
    ...
    if (window.documentModule) {
      ...
      if (hasDocs) {
        ...
        setTimeout(
          () =>
            window.documentModule.loadSessionDocs(id, { restoreMode: true }),
          300,
        );
      } else if (!shouldOpen) {
...
async function _onSessionListKeydown(e) {
  ...
  if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
    ...
    const allItems = Array.from(
      document.querySelectorAll('#session-list .list-item[data-session-id]'),
    );
  ...
  if (e.key === 'Delete' || e.key === 'Backspace') {
    ...
    const ok = await uiModule.styledConfirm('Delete this session?', {
      confirmText: 'Delete',
      danger: true,
    });
    if (!ok) return;
...
function _updateResearchDots() {
  document
    .querySelectorAll('.session-star[data-session-id]')
    .forEach(function (star) {
      var sid = star.dataset.sessionId;
      var isRunning =
        _researchingSessions.has(sid) || _streamingSessions.has(sid);
...
function _updateRailNotifs() {
  ...
  if (railResearch) {
    ...
    const researching =
      _researchingSessions.size > 0 || !!window._researchJobsActive;
  ...
  if (railChats) {
    ...
    railChats.classList.toggle(
      'rail-notify-success',
      hasCompleted && sidebarHidden,
    );
...
async function _checkServerStream(sessionId) {
  try {
    ...
    if (
      window.chatModule &&
      window.chatModule.hasActiveStream &&
    ...
    const pollId = setInterval(async () => {
      ...
      if (
        !_resumeRetried &&
        window.chatModule &&
        window.chatModule.resumeStream
      ) {
        _resumeRetried = true;
        const attached = await window.chatModule.resumeStream(sessionId);
        if (attached) {
      ...
      try {
        const r = await fetch(
          `${API_BASE}/api/chat/stream_status/${sessionId}`,
        );
...
export function clearStreamComplete(sessionId) {
  ...
  var star = document.querySelector(
    `.session-star[data-session-id="${sessionId}"]`,
  );
  if (star) {
    star.classList.remove('notify', 'processing');
    star.style.opacity = '';
  }
  _updateResearchDots();
  _updateRailNotifs();
...
function _initDropdownDismiss() {
  document.addEventListener('click', (e) => {
    if (e.target.closest('.session-dropdown-menu, .session-folder-submenu'))
      return;
    document
      .querySelectorAll('.session-dropdown-menu, .session-folder-submenu')
...
function _showDropdown(anchorEl, items) {
  ...
  document
    .querySelectorAll('.session-dropdown-menu.archive-dd')
    .forEach((d) => d.remove());
...
async function _arcPeekOpen(sid) {
  try {
    ...
    if (window.chatModule && window.chatModule.hideWelcomeScreen)
          ...
          const content =
            typeof msg.content === 'string'
              ...
              : Array.isArray(msg.content)
                ...
                : String(msg.content || '');
          try {
            addMsg(msg.role, content, model, msg.metadata || null);
          } catch (e) {
            console.warn('Failed to render message:', e);
          }
        }
      }
    }
...
async function _arcDelete(sid) {
  if (
    !(await window.styledConfirm('Delete this session permanently?', {
      confirmText: 'Delete',
      danger: true,
    }))
  )
  ...
  try {
    ...
    await _animateSessionRowsRemoving(
      [sid],
      '#archive-grid .archive-row[data-session-id]',
    );
    _arcRemove(sid);
    _arcRefreshUI();
    uiModule.showToast('Session deleted');
...
async function _arcBulkRestore() {
  ...
  _arcRefreshUI();
  uiModule.showToast(
    `${ids.length} session${ids.length > 1 ? 's' : ''} restored`,
  );
  loadSessions();
}
...
async function _arcBulkDelete() {
  ...
  const ok = await uiModule.styledConfirm(
    `Delete ${ids.length} session${ids.length > 1 ? 's' : ''} permanently?`,
    { confirmText: 'Delete', danger: true },
  );
  if (!ok) return;
  const deletedIds = [];
...
function _arcRenderChips() {
  ...
  const mkChip = (label, value, count) => {
    ...
    chip.addEventListener('click', () => {
      _arc.model = _arc.model === value ? '' : value;
      _arcFetch(false);
    });
    el.appendChild(chip);
  };
...
function _arcRenderCard(s) {
  ...
  card.className =
    'memory-item archive-row' + (_arc.selected.has(s.id) ? ' selected' : '');
...
export function openLibrary(defaultTab) {
  ...
  modal.querySelectorAll('.lib-tab').forEach((tab) => {
    tab.addEventListener('click', () => {
      ...
      } else if (_lib.tab === 'chats') {
        action1.textContent = 'Archive';
      } else if (_lib.tab === 'research') {
        action1.textContent = 'Open Report';
      } else {
        action1.textContent = 'Export';
      }
      _renderLibGrid();
    });
  });
  ...
  document.getElementById('lib-sort').addEventListener('change', () => {
    _lib.sort = document.getElementById('lib-sort').value;
    _renderLibGrid();
  });
  document.getElementById('lib-search').addEventListener('input', (e) => {
    clearTimeout(_lib.debounce);
    _lib.debounce = setTimeout(() => {
      _lib.search = e.target.value.trim().toLowerCase();
      _renderLibGrid();
    }, 200);
  });
  ...
  document.getElementById('lib-select-btn').addEventListener('click', () => {
    ...
    _lib.selected.clear();
    document
      .getElementById('lib-bulk-bar')
      .classList.toggle('hidden', !_lib.selectMode);
    _renderLibGrid();
  });
  document.getElementById('lib-select-all').addEventListener('change', (e) => {
    const checked = e.target.checked;
    document.querySelectorAll('#lib-grid .memory-select-cb').forEach((cb) => {
      cb.checked = checked;
    });
    document.querySelectorAll('#lib-grid .doclib-card').forEach((card) => {
      const id = card.dataset.sessionId || card.dataset.docId;
      if (id) {
        if (checked) _lib.selected.add(id);
        else _lib.selected.delete(id);
      }
    });
    _updateLibCount();
  });
  ...
  document
    .getElementById('lib-bulk-action1')
    .addEventListener('click', async () => {
      if (_lib.tab === 'chats') {
        for (const sid of _lib.selected)
          await fetch(`${API_BASE}/api/session/${sid}/archive`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          });
        ...
        for (const sid of _lib.selected)
          await fetch(`${API_BASE}/api/session/${sid}/restore`, {
            method: 'POST',
          });
  ...
  // Bulk delete
  document
    .getElementById('lib-bulk-delete')
    .addEventListener('click', async () => {
      if (
        !(await uiModule.styledConfirm(`Delete ${_lib.selected.size} items?`, {
          confirmText: 'Delete',
          danger: true,
        }))
      )
      ...
      if (_lib.tab === 'chats' || _lib.tab === 'archive') {
        ...
        for (const did of _lib.selected)
          await fetch(`${API_BASE}/api/document/${did}`, { method: 'DELETE' });
      } else if (_lib.tab === 'research') {
        for (const rid of _lib.selected)
          await fetch(`${API_BASE}/api/research/${rid}`, { method: 'DELETE' });
      }
      _lib.selected.clear();
      _lib.selectMode = false;
      document.getElementById('lib-bulk-bar').classList.add('hidden');
...
function _renderLibChats(grid) {
  ...
  else if (_lib.sort === 'most-messages')
    filtered.sort((a, b) => (b.message_count || 0) - (a.message_count || 0));
  else if (_lib.sort === 'alpha')
    ...
    card.querySelector('.archive-menu-btn').addEventListener('click', (e) => {
      ...
      _showDropdown(e.currentTarget, [
        ...
        },
        {
          ...
          action: async () => {
            if (
              !(await uiModule.styledConfirm('Delete?', {
                confirmText: 'Delete',
                danger: true,
              }))
            )
...
async function _renderLibArchive(grid) {
  ...
  try {
    const params = new URLSearchParams({
      limit: '50',
      sort: _lib.sort === 'most-messages' ? 'messages' : _lib.sort,
    });
        ...
        _showDropdown(e.currentTarget, [
          {
            label: 'Restore',
            action: async () => {
              await fetch(`${API_BASE}/api/session/${s.id}/restore`, {
                method: 'POST',
              });
              await loadSessions();
              _renderLibGrid();
    ...
    console.error('Library archive error:', e);
    grid.innerHTML = '<div class="doclib-empty">Failed to load archive</div>';
  }
}
...
async function _renderLibDocuments(grid) {
  ...
  try {
    ...
    if (!docs.length) {
      grid.innerHTML = '<div class="doclib-empty">No documents found</div>';
      return;
    }
    grid.innerHTML = '';
    for (const d of docs) {
      ...
      card.addEventListener('click', (e) => {
        ...
        if (d.session_id && window.documentModule) {
          ...
          setTimeout(() => {
            if (window.documentModule.loadSessionDocs)
              window.documentModule.loadSessionDocs(d.session_id);
          }, 300);
        }
      });
      card.querySelector('.archive-menu-btn').addEventListener('click', (e) => {
    ...
    console.error('Library documents error:', e);
    grid.innerHTML = '<div class="doclib-empty">Failed to load documents</div>';
  }
}
  ...
  try {
    ...
    for (const item of items) {
      ...
      const card = _buildLibCard(
        ...
        meta,
        item.completed_at
          ? new Date(item.completed_at * 1000).toISOString()
          : '',
        false,
        false,
      );
      ...
      card.addEventListener('click', (e) => {
        if (
          e.target.closest('.archive-menu-btn') ||
          ...
          _showDropdown(e.currentTarget, [
            {
              ...
              action: () =>
                window.open(
                  `${API_BASE}/api/research/report/${item.id}`,
                  '_blank',
                ),
            },
            {
            ...
            },
            {
              ...
              action: async () => {
                if (
                  ...
                  return;
                await fetch(`${API_BASE}/api/research/${item.id}`, {
                  method: 'DELETE',
                });
                _renderLibGrid();
              },
...
function _buildLibCard(id, title, count, meta, time, isActive, isDoc) {
  ...
  const cbHtml = _lib.selectMode
    ? `<input type="checkbox" class="memory-select-cb"${_lib.selected.has(id) ? ' checked' : ''}>`
    : '';
  const metaParts = [];
  if (meta) metaParts.push(uiModule.esc(meta));
  metaParts.push(
    isDoc ? 'v' + count : count + ' msg' + (count !== 1 ? 's' : ''),
  );
  ...
  if (cb) {
    ...
    cb.addEventListener('change', () => {
      if (cb.checked) _lib.selected.add(id);
      else _lib.selected.delete(id);
      _updateLibCount();
    });
  }
...
export function openArchive() {
  ...
  document
    .getElementById('archive-close')
    .addEventListener('click', closeArchive);
  document.getElementById('archive-sort').addEventListener('change', (e) => {
    _arc.sort = e.target.value;
    _arcFetch(false);
  });
  document.getElementById('archive-search').addEventListener('input', (e) => {
    clearTimeout(_arc.debounce);
    _arc.debounce = setTimeout(() => {
      _arc.search = e.target.value.trim();
      _arcFetch(false);
    }, 300);
  });
  document.getElementById('archive-load-more').addEventListener('click', () => {
    _arc.offset = _arc.data.length;
    _arcFetch(true);
  });
  document
    .getElementById('archive-select-btn')
    .addEventListener('click', _arcToggleSelectMode);
  document
    .getElementById('archive-bulk-restore')
    .addEventListener('click', _arcBulkRestore);
  document
    .getElementById('archive-bulk-delete')
    .addEventListener('click', _arcBulkDelete);
  document
    .getElementById('archive-select-all')
