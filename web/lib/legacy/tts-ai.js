...
class AITTSManager {
  ...
  async checkAvailability() {
    try {
      ...
      if (stats.provider === 'browser') {
        ...
        if (!this.available) {
          console.warn(
            'TTS: browser mode selected but speechSynthesis not supported',
          );
        }
  ...
  async _playQueueItem(item) {
    ...
    const ICON_LOADING =
      '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="9" stroke-dasharray="42" stroke-dashoffset="12" stroke-linecap="round"><animateTransform attributeName="transform" type="rotate" from="0 12 12" to="360 12 12" dur="0.8s" repeatCount="indefinite"/></circle></svg>';
    var ICON_STOP =
  ...
  streamingAttachButton(button, resetFn) {
    ...
    for (var i = 0; i < this._queue.length; i++) {
      if (
        this._queue[i].button &&
        this._queue[i].button.classList.contains('streaming-placeholder')
      ) {
...
export function addAITTSButton(messageElement, text) {
  ...
  playButton.style.cssText =
    'background:none;border:none;color:#6b7280;cursor:pointer;padding:2px 6px;border-radius:4px;transition:color .15s;line-height:1;display:inline-flex;align-items:center;';
  ...
  playButton.addEventListener('mouseleave', () => {
    if (
      ...
      !playButton.classList.contains('loading')
    )
